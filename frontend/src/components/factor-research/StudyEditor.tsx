  ...
  return <form ref={form} onSubmit={event => { event.preventDefault(); save(false) }}>
    <fieldset disabled={busy} className="space-y-5">
      <div className="grid min-w-0 gap-5 lg:grid-cols-2">
        <Card title="1 · 研究对象与样本">
          <div className="space-y-4">
            ...
            {products.length > 0 && <div className="max-h-44 overflow-y-auto rounded-lg border border-slate-200">{products.map(product => <button type="button" className="block min-h-10 w-full px-3 py-2 text-left text-sm hover:bg-indigo-50" key={product.ts_code} onClick={() => { setTargets(parseCodes(targets + ', ' + product.ts_code).join(', ')); setQuery('') }}>{product.ts_code} · {product.name}</button>)}</div>}
        ...
        <Card title="2 · 基准、模型与数据">
          <div className="space-y-4">
            ...
            <p className="rounded-lg bg-indigo-50 p-3 text-sm leading-6 text-indigo-900">研究市场：中国；币种：人民币。比较基准用于衡量表现。FF3 与 RBSA 属于收益归因模型，请在“因子收益率 → 收益归因”中选择对应数据。</p>
            <p className="text-xs leading-5 text-slate-500">按所选日、周或月频形成信号，下一交易日净值模拟入场。缺失不补零，公告日只有日期时延后一交易日使用；研究结果保留数据修订局限。</p>
          </div>
        </Card>
      </div>
      <Card title="3 · 因子组合">
        ...
        <div className="grid gap-3 md:grid-cols-2">{factors.filter(f => f.product_kinds.includes(draft.product_kind)).map(factor => {
          ...
          return <div key={factor.id} className={'flex flex-wrap items-center gap-3 rounded-lg border p-3 ' + (chosen ? 'border-indigo-200 bg-indigo-50/40' : 'border-slate-200')}>
            <label className="flex min-h-10 min-w-0 flex-1 items-center gap-2 text-sm"><input type="checkbox" checked={Boolean(chosen)} onChange={e => change('factors', e.target.checked ? [...draft.factors, { factor_id: factor.id, revision: factor.revision, weight: 1 }] : draft.factors.filter(f => f.factor_id !== factor.id))} /><span>{factor.name}<span className="ml-1 text-xs text-slate-500">v{chosen?.revision ?? factor.revision}</span></span></label>
        ...
        <p className="mt-3 text-xs text-slate-500">权重在计算时归一化。方案锁定因子修订号；单个因子缺失时该产品不获得组合得分。</p>
      </Card>
      <Card title="4 · 检验与净值模拟">
        ...
        <p className="mt-3 text-xs leading-5 text-slate-500">多头等权，边界同分产品共同纳入，买卖均扣费；不足三个有效产品时持现金，现金收益假设为 0。净值模拟不代表实际成交。</p>
      </Card>
      <div className="flex flex-wrap items-center gap-3"><button className={secondaryClass} type="submit">保存方案</button><button className={buttonClass} type="button" onClick={() => save(true)}>保存并运行检验</button>{current && <span className="break-all text-xs text-slate-500">当前方案 v{current.revision}</span>}</div>
    </fieldset>
  </form>
}
