...
export default function RegimeTemporalPanel({ report, stale = false, busy = false, disabled = false, onAudit, onNode }: {
  ...
  return <section aria-label="时点能力与因果性审计" className={'min-w-0 space-y-3 rounded-xl border p-4 ' + (verified ? 'border-emerald-200 bg-emerald-50/40' : 'border-slate-200 bg-slate-50')}>
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">{title}</h3><p className="mt-1 text-xs leading-5 text-slate-600">{hindsight ? '这是事后研究能力，不是算法错误；不能用它重建当时的交易信号。' : verified ? '仅验证本次定义、模式与数据；正式发布还需通过完整门禁。' : '先检查实际依赖与时间契约，再用真实计算检验未来数据是否改变过去。'}</p></div>{onAudit && <button type="button" disabled={busy || disabled} onClick={onAudit} className="min-h-10 rounded-lg border border-accent-200 bg-white px-3 text-sm font-semibold text-accent-700 disabled:opacity-40">{busy ? '计算与审计中…' : '因果性审计'}</button>}</div>
    {!stale && report?.reasons?.slice(0, 2).map((r, i) => <div key={r.node_id + i} className="flex flex-wrap items-start justify-between gap-2 text-xs leading-5"><span>{r.message}</span>{onNode && <button type="button" onClick={() => onNode(r.node_id)} className="min-h-9 shrink-0 text-accent-700 underline">定位节点</button>}</div>)}
    {report && <details className="rounded-xl border border-slate-200 bg-white p-3"><summary className="cursor-pointer text-xs font-semibold">查看审计依据与限制</summary>
      <div className="mt-3 grid gap-3 text-xs sm:grid-cols-2"><div><p className="text-slate-600">计算因果性</p><p>{report.numerical_verdict === 'causal' ? '已覆盖探针未发现未来依赖' : report.numerical_verdict === 'leak' ? '发现未来依赖反例' : '数值证据尚未充分验证'}</p></div><div><p className="text-slate-600">数据与信息时点</p><p>{report.data_checks ? report.data_checks.passed ? '本次来源时点契约已检查（日期级，不证明日内可交易）。' : '来源时点尚未充分核验，不能认定为实时可用。' : '运行后核对真实数据快照、公布时间与版本。'}</p></div><div><p className="text-slate-600">历史重绘</p><p>{report.may_repaint ? '存在后续确认或全样本回标依赖' : '未声明重绘；仍需前缀重放核对'}</p></div><div><p className="text-slate-600">事后语义</p><p>{report.semantic_hindsight ? '人工事后认定：必须用于事后研究' : '没有人工事后标签强制限制'}</p></div></div>
