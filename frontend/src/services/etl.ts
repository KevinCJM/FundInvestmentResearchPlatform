...
export type EtlKind = 'download' | 'map' | 'resolve' | 'snapshot'
...
export interface EtlRunOptions { mode: 'full' | 'incremental'; parameters: Record<string, string> }
export interface EtlStep {
  id: string; name: string; kind: EtlKind; inputs: string[]
...
export interface EtlDefinition { name: string; description: string; max_runtime_seconds: number; steps: EtlStep[]; parameters?: EtlParameter[] }
export interface EtlWorkflow { id: string; revision: number; definition: EtlDefinition; updated_at: string }
export interface EtlValidation { valid: boolean; errors: { code: string; message: string }[]; steps: { id: string; name: string; kind: EtlKind; inputs: string[]; tables: string[] }[] }
export interface EtlRun {
  ...
  template_definition?: EtlDefinition; options?: EtlRunOptions
  steps: { id: string; name: string; kind: EtlKind; status: string; rows?: number; pages?: number; error?: string; started_at?: string; finished_at?: string; output?: Record<string, unknown> }[]
}
...
export const fetchTushareEtlTemplate = () => request<{ definition: EtlDefinition; expected_revision: number }>('/templates/tushare-funds')
...
export const runEtl = (definition: EtlDefinition, request_id: string, options: EtlRunOptions = { mode: 'incremental', parameters: {} }) => post<EtlRun>('/runs', { definition, request_id, options, confirm: true })
export const listEtlRuns = () => request<EtlRun[]>('/runs')
export const resumeEtl = (id: string) => post<EtlRun>(`/runs/${id}/resume`, { confirm: true })
export const cancelEtl = (id: string) => post<EtlRun>(`/runs/${id}/cancel`, {})
export const stepLabels: Record<EtlKind, string> = { download: '下载原始数据', map: '字段映射', resolve: '多源取值', snapshot: '指标快照计算' }
