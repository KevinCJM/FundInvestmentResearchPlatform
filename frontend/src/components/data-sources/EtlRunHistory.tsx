...
export default function EtlRunHistory({ runs, busy, connected = true, onResume, onCancel, onReuse, focusedRun, onShowRun }: {
  ...
  const renderStep = (s: EtlRun['steps'][number], n: number) => <li key={s.id} className={`min-w-0 rounded-lg border p-3 ${s.status === 'FAILED' ? 'border-rose-200 bg-rose-50' : s.status === 'RUNNING' ? 'border-indigo-300 bg-indigo-50' : 'border-slate-100 bg-slate-50'}`}>
  ...
  return <section className="space-y-4" aria-label="ETL 运行记录"><h2 className="text-lg font-bold">下载任务</h2>
    <p className="text-xs text-slate-500">每个任务显示当前进度；之前的失败与续跑记录收在任务内，不会重复列出。</p>
    ...
    {visibleRuns.map((run, index) => {
      ...
      <summary className="cursor-pointer break-words text-sm font-semibold focus-visible:outline-indigo-600">
        <span>{title}</span><span className={`ml-2 inline-block rounded-full px-2 py-1 text-xs ${recovering || run.status === 'RUNNING' ? 'bg-indigo-50 text-indigo-700' : run.status === 'FAILED' ? 'bg-rose-50 text-rose-700' : 'bg-slate-100 text-slate-600'}`}>{recovering ? '正在校验并续跑' : run.execution?.state === 'WORKER_OBSERVED' ? '后台有下载进度（调度中断）' : stateLabels[run.status] ?? run.status}</span>
        <span className="mt-2 block text-xs font-normal text-slate-500">已完成 {complete} / {run.steps.length} 个步骤 · 最近更新 {new Date(updatedAt).toLocaleString('zh-CN')}</span>
      </summary>
      ...
      <p className="mt-3 text-xs text-slate-500">{run.options ? `${runModeLabels[run.options.mode]} · ` : ''}候选未发布{run.cancel_requested && run.status === 'RUNNING' ? ' · 已请求取消，正在等待安全停止' : ''}</p>
      {run.history && (run.history.records.length > 0 || run.history.resume_count > 0 || run.history.lineage_warning) ? <details className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs">
        <summary className="cursor-pointer text-slate-600 focus-visible:outline-indigo-600">历史记录 · 已续跑 {run.history.resume_count} 次，保留 {run.history.records.length} 条原记录</summary>
        <p className="mt-2 text-slate-500">下面是之前的执行记录，不代表当前失败。已完成数据经校验后复用；采集时点和原执行版本不变。</p>
        ...
        <ol aria-label="历次执行记录" className="mt-2 max-h-64 space-y-3 overflow-y-auto">
          {run.history.records.map(entry => <li key={entry.run_id} className="break-words border-t border-slate-200 pt-2">
            ...
            <p className="mt-1 break-all text-slate-400">记录编号：{entry.run_id}</p>
          </li>)}
        </ol>
      </details> : run.recovered_from ? <p className="mt-2 break-all text-xs text-slate-500">原任务 {run.recovered_from} 保留审计记录；已完成数据经校验后复用。</p> : null}
      ...
      {run.steps.length > 0 ? <details className="mt-4"><summary className="cursor-pointer text-xs text-indigo-700">查看全部步骤（共 {run.steps.length} 个，已完成 {complete} 个）</summary><ol aria-label="全部工作步骤" className="mt-3 space-y-2">{run.steps.map(renderStep)}</ol></details> : null}
    </details>})}
  </section>
}
