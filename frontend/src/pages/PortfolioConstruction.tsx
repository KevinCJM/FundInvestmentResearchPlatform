...
function PortfolioConstructionEditor() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6" aria-busy={running || saving || universeLoading}>
    <header className="rounded-2xl bg-slate-900 px-5 py-6 text-white sm:px-7">
      ...
      <p className="mt-2 max-w-3xl text-sm text-slate-300">把大类资金预算分配到具体产品，检查类内权重后运行历史研究。</p>
    </header>
    ...
    {(draft || inputsChanged) && !result && <p className="text-xs text-slate-500">输入自动保留；修改后请重新运行，历史结果不会沿用。</p>}
    <section id="portfolio-step-1" className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      <div className="space-y-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div className="space-y-2"><div className="grid gap-3 sm:grid-cols-[1fr_180px]"><label className="block text-sm font-medium" htmlFor="portfolio-search">从可投资域搜索产品<input id="portfolio-search" disabled={!universe || !assetClasses.length} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="输入代码或名称" className="mt-1 w-full rounded border border-slate-300 px-3 py-2 disabled:bg-slate-100" /></label><label className="block text-sm font-medium">加入大类<select aria-label="候选产品所属大类" disabled={!assetClasses.length} value={selectedAssetClassId} onChange={(event) => setSelectedAssetClassId(event.target.value)} className="mt-1 w-full rounded border border-slate-300 bg-white px-3 py-2 disabled:bg-slate-100"><option value="">请选择</option>{assetClasses.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>
          {!assetClasses.length && <p className="rounded bg-amber-50 px-3 py-2 text-xs text-amber-800">请先在<Link to={universe ? `/pre-investment/saa/asset-classes?universe=${encodeURIComponent(universe.id)}` : '/pre-investment/product-pool'} className="mx-1 font-medium underline">大类构建</Link>中建立大类并导入产品。</p>}
          <p aria-live="polite" className="text-xs text-slate-500">{searching ? '正在检索可投资域产品…' : searchError || (query && !candidates.length ? '没有匹配产品。' : '')}</p>
          <ul className="divide-y rounded border border-slate-200" aria-label="产品搜索结果">{candidates.map((item) => <li key={`${item.kind}-${item.product_id}`} className="flex items-center justify-between gap-3 px-3 py-2 text-sm"><span><b>{item.name}</b> <span className="text-slate-500">{item.code ?? item.product_id} · {item.kind === 'etf' ? 'ETF' : '公募基金'}</span></span><button type="button" disabled={!selectedAssetClassId} onClick={() => addInstrument(item)} className="rounded border border-emerald-600 px-2 py-1 text-emerald-700 disabled:border-slate-300 disabled:text-slate-400">加入</button></li>)}</ul>
        </div>
        <div><h2 className="text-base font-semibold">已选产品（至少 2 只）</h2>{!constituents.length ? <p className="mt-2 text-sm text-slate-500">搜索并加入产品后开始构建。</p> : <ul className="mt-2 space-y-2">{constituents.map((item, index) => <li key={`${item.kind}-${item.product_id}`} className="grid grid-cols-[1fr_auto] gap-2 rounded border border-slate-200 p-2"><span className="text-sm"><b>{item.name}</b><br /><span className="text-xs text-slate-500">{item.code ?? item.product_id} · {item.asset_class_name || '未指定大类'}</span></span><button type="button" onClick={() => setConstituents((current) => current.filter((_, currentIndex) => currentIndex !== index))} className="text-sm text-rose-700 underline">移除</button>{method === 'manual' && <label className="col-span-2 text-sm">权重 (%)<input aria-label={`${item.name} 权重`} type="number" min="0" max="100" value={item.weight ?? 0} onChange={(event) => updateNumber(index, 'weight', event.target.value)} className="ml-2 w-24 rounded border border-slate-300 px-2 py-1" /></label>}{method === 'risk_budget' && <label className="col-span-2 text-sm">风险预算 (%)<input aria-label={`${item.name} 风险预算`} type="number" min="0" max="100" value={item.risk_budget ?? 0} onChange={(event) => updateNumber(index, 'risk_budget', event.target.value)} className="ml-2 w-24 rounded border border-slate-300 px-2 py-1" /></label>}</li>)}</ul>}
      ...
      <div className="space-y-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <fieldset id="portfolio-step-2"><legend className="text-base font-semibold">权重方法</legend><div className="mt-2 grid gap-2">{METHODS.map((item) => <label key={item.value} className={`cursor-pointer rounded border p-3 ${method === item.value ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200'}`}><input type="radio" name="method" disabled={Boolean(allocationSource) && item.value !== 'manual'} checked={method === item.value} onChange={() => setMethod(item.value)} /> <b className="ml-1">{item.label}</b><span className="block pl-5 text-xs text-slate-600">{item.help}</span></label>)}</div></fieldset>
        ...
        {allocationSource && <p className="text-xs text-slate-500">当前承接 TAA 预算，使用手工权重做类内调整。</p>}
