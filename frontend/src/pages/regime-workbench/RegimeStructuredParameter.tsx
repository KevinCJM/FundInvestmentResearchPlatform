...
function ScalarField({ label, value, onChange }: { label: string; value: unknown; onChange: (value: unknown) => void }) {
  ...
  return <label className="block min-w-0 text-[11px] font-semibold text-slate-600">{label}<input aria-label={label} type={typeof value === 'number' ? 'number' : 'text'} step="any" value={text} onChange={(event) => { const next = event.target.value; setText(next); if (typeof value !== 'number') onChange(next); else if (next.trim() && Number.isFinite(Number(next))) onChange(Number(next)) }} onBlur={() => { if (typeof value === 'number' && (!text.trim() || !Number.isFinite(Number(text)))) setText(String(value)) }} className="mt-1 min-h-9 w-full min-w-0 rounded-md border border-slate-300 bg-white px-2 font-normal" /></label>
}
...
export default function RegimeStructuredParameter({ label, value, onChange }: { label: string; value: unknown; onChange: (value: unknown) => void }) {
  if (Array.isArray(value)) return <div className="space-y-2">{value.map((item, index) => <div key={index} className="rounded-lg border border-slate-200 bg-slate-50 p-2"><div className="mb-2 flex items-center justify-between gap-2"><span className="text-[10px] font-bold text-slate-500">第 {index + 1} 项</span><div className="flex gap-2"><button type="button" onClick={() => onChange([...value.slice(0, index + 1), JSON.parse(JSON.stringify(item)), ...value.slice(index + 1)])} className="text-[10px] font-bold text-indigo-700">复制</button><button type="button" aria-label={`删除${label}第${index + 1}项`} onClick={() => onChange(value.filter((_, row) => row !== index))} className="text-[10px] font-bold text-rose-600">删除</button></div></div><RegimeStructuredParameter label={`${label}第${index + 1}项`} value={item} onChange={(next) => onChange(value.map((entry, row) => row === index ? next : entry))} /></div>)}{!value.length ? <p className="text-xs text-slate-500">当前列表为空。可从模板载入规则，或使用高级编辑添加结构。</p> : null}</div>
