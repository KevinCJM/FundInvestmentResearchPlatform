...
function initialRequest(baseline: TaaBaseline, catalog: TaaCatalog | null, platformDay: string | null = null): TaaPreviewRequest {
  ...
  return { baseline_id: baseline.id, start_date: start, end_date: end, as_of: asOf, train_end_date: split, signal_mode: 'momentum', lookback: 60, manual_tilts: zeroes(baseline), max_abs_tilt: .1, transaction_cost_bps: 10, risk_penalty: 3, max_tracking_error: baseline.policy?.mandate.max_tracking_error ?? .1, max_turnover: 1, confidence_floor: .6, max_signal_age_days: 31, search: true, objective: 'active_utility', review_days: 30, note: '' }
}
