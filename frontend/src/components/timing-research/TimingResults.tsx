...
const tableClass = 'w-full text-left text-xs [&_th]:whitespace-nowrap [&_th]:bg-slate-50 [&_th]:px-3 [&_th]:py-3 [&_th]:font-medium [&_th]:text-slate-500 [&_td]:whitespace-nowrap [&_td]:border-t [&_td]:border-slate-100 [&_td]:px-3 [&_td]:py-3 [&_td]:text-slate-700'
function Periods({ rows, label }: { rows: TimingPeriod[]; label: string }) {
  return <div className="max-h-[420px] overflow-auto rounded-lg border border-slate-200"><table aria-label={label} className={tableClass}><thead><tr><th>时期</th><th>策略收益</th><th>买入持有</th><th>最大回撤</th><th>已平仓交易</th><th>胜率</th><th>持仓比例</th></tr></thead><tbody>{rows.map(row => <tr key={row.period}><td>{row.period}</td><td>{timingPercent(row.total_return)}</td><td>{timingPercent(row.buy_hold_return)}</td><td>{timingPercent(row.max_drawdown)}</td><td>{timingNumber(row.trade_count, 0)}</td><td>{timingPercent(row.win_rate)}</td><td>{timingPercent(row.exposure)}</td></tr>)}</tbody></table>{!rows.length && <p className="p-4 text-sm text-slate-500">此区间没有可用统计。</p>}</div>
}
function Summary({ metrics }: { metrics?: TimingMetrics }) {
  ...
  return <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">{items.map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 bg-white p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-xl font-semibold tabular-nums text-slate-900">{value}</p></div>)}</div>
}
...
export default function TimingResults({ run }: { run: TimingRun }) {
  ...
  if (!product) return <p className="p-6 text-sm text-slate-500">此研究没有产品结果。</p>
  return <section aria-label="择时研究结果" className="min-w-0 space-y-4">
    <div className="flex flex-wrap items-end justify-between gap-3"><label className="min-w-[180px] text-xs text-slate-600">查看产品<select className={timingField} value={productId} onChange={event => setProductId(event.target.value)}>{run.products.map(item => <option key={item.product_id} value={item.product_id}>{item.product_id}{item.status === 'error' ? ' · 数据不足' : ''}</option>)}</select></label><span className="text-xs text-slate-500">运行于 {run.created_at?.replace('T', ' ').slice(0, 19)}</span></div>
    {product.status === 'error' ? <p role="alert" className="rounded-xl bg-amber-50 p-4 text-sm text-amber-800">{product.error || '此产品无法完成研究，请检查数据和区间。'}</p> : <>
      <div className="flex flex-wrap gap-2" aria-label="统计样本">{(['out_of_sample', 'in_sample', 'all'] as const).map(value => <button key={value} type="button" aria-pressed={sample === value} className={`min-h-9 rounded-lg px-3 text-sm ${sample === value ? 'bg-indigo-50 font-semibold text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`} onClick={() => setSample(value)}>{({ out_of_sample: '样本外', in_sample: '样本内', all: '全区间' })[value]}</button>)}</div>
      ...
      ].map(([label, value]) => <div key={label} className="rounded-lg bg-slate-50 p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-lg font-semibold tabular-nums text-slate-900">{value}</p></div>)}</div><p className="mt-3 text-xs leading-5 text-slate-500">有信号月份 {timingNumber(diagnostics.active_month_count, 0)} / {timingNumber(diagnostics.total_month_count, 0)}；含不完整首尾月，非综合评分。候选信号可能重复，不等于成交次数。</p></details>}
      <div className="flex flex-wrap gap-2 border-b border-slate-200" role="tablist" aria-label="研究结果视图">{(['overview', 'periods', 'trades', 'nodes'] as const).map(value => <button key={value} id={`timing-tab-${value}`} type="button" role="tab" aria-selected={tab === value} aria-controls="timing-result-panel" onClick={() => setTab(value)} className={`min-h-11 border-b-2 px-3 text-sm ${tab === value ? 'border-indigo-600 font-semibold text-indigo-700' : 'border-transparent text-slate-500'}`}>{({ overview: '净值与信号', periods: '年度 / 月度', trades: '逐笔交易', nodes: '步骤预览' })[value]}</button>)}</div>
      <div id="timing-result-panel" role="tabpanel" aria-labelledby={`timing-tab-${tab}`} className="min-w-0 space-y-4">
        {loadError ? <div role="alert" className="rounded-lg bg-rose-50 p-4 text-sm text-rose-700">{loadError}<button type="button" className="ml-3 underline" onClick={() => setRetry(value => value + 1)}>重新加载</button></div> : needsDetail && !loaded ? <p role="status" className="p-5 text-sm text-slate-500">正在读取该产品的曲线与步骤…</p> : <>
          {tab === 'overview' && <><p className="text-xs leading-5 text-slate-500">下方图表展示完整研究区间。样本外从 {run.request_snapshot.holdout_start} 开始；上方统计按所选样本切换。</p><ReactECharts option={chartOption} notMerge style={{ height: 330, width: '100%' }} onEvents={chartEvents} />
            <div role="group" aria-label="买卖点图表" className="min-w-0">
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-slate-600"><h3 className="mr-auto text-sm font-medium text-slate-800">价格与买卖点</h3><span className="inline-flex items-center gap-1.5"><span className="rounded border border-rose-200 bg-rose-50 px-1.5 py-0.5 font-bold text-rose-700">B</span>买入</span><span className="inline-flex items-center gap-1.5"><span className="rounded border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 font-bold text-emerald-700">S</span>卖出</span></div>
              <ReactECharts option={priceOption} notMerge style={{ height: 300, width: '100%' }} onEvents={chartEvents} />
              <p className="text-xs leading-5 text-slate-500">B 在成交点下方，S 在上方。拖动滑块放大区间；密集文字自动避让，成交点全部保留。悬停查看成交详情，点击标记定位日期。</p>
            </div>
    ...
    {!!run.restrictions?.length && <details className="rounded-lg bg-slate-50 p-3"><summary className="cursor-pointer text-xs font-medium text-slate-600">研究边界与数据口径</summary><div className="mt-2 space-y-1 text-xs leading-5 text-slate-500">{run.restrictions.map((restriction, index) => <p key={index}>{restriction}</p>)}</div></details>}
  </section>
}

function Pager({ page, total, onChange }: { page: number; total: number; onChange: (page: number) => void }) {
  if (total <= 100) return null
  return <div className="mt-3 flex items-center justify-between gap-2 text-xs text-slate-500"><button type="button" className="min-h-9 rounded-lg border px-3 disabled:opacity-40" disabled={page === 0} onClick={() => onChange(page - 1)}>上一页</button><span>{page + 1} / {Math.ceil(total / 100)} 页 · {total} 条</span><button type="button" className="min-h-9 rounded-lg border px-3 disabled:opacity-40" disabled={(page + 1) * 100 >= total} onClick={() => onChange(page + 1)}>下一页</button></div>
}
