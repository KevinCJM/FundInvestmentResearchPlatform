...
export default function RegimeIndicatorSourceFields({ node, series, onPatchNode }: {
  ...
  const input = 'mt-1 min-h-10 w-full min-w-0 rounded-xl border border-slate-300 bg-white px-2 text-xs disabled:opacity-50'
  return <section aria-label="指标计算对象" className="space-y-3 rounded-xl border border-slate-200 p-3">
    ...
    {loading ? <p role="status" className="text-xs text-slate-600">正在搜索计算对象…</p> : error ? <p role="alert" className="text-xs text-rose-700">{error}<button type="button" className="ml-2 underline" onClick={() => setRetry(value => value + 1)}>重试</button></p> : kind && result?.key === key && <p className="text-xs text-slate-600">{result.total ? `找到 ${result.total} 个对象。` : '没有匹配的对象，请换一个名称或代码。'}</p>}
    {result?.key === key && items.length < result.total && <button type="button" disabled={loading} onClick={() => setPage(value => value + 1)} className="min-h-9 text-xs text-accent-700">加载更多计算对象</button>}
