...
export default function ImplementationMappingEditor({ universe, catalog, domainId, requestedMapping, onSaved }: {
  ...
  function readMapping() {
    ...
    void historyRead.run(signal => getImplementationMap(requestedMapping, signal), result => {
      if (result.id !== requestedMapping) throw new Error('读取的映射身份不一致。')
  ...
  return <section className="space-y-4 border-t border-slate-200 pt-5" aria-label="战略实施映射">
    ...
    {historyRead.busy && <p role="status" className="text-sm text-slate-600">正在读取不可变实施映射…</p>}
    {historyRead.error && <Button onClick={readMapping}>重试读取实施映射</Button>}
    {saved && <p className="text-sm text-slate-700">只读映射：{saved.name}。<Button onClick={() => { historyRead.invalidate(); operation.invalidate(); setSaved(null); setPreview(null) }}>复制映射为新研究</Button></p>}
    ...
    <div className="flex flex-wrap gap-3"><Button disabled={Boolean(issue) || operation.busy || historyRead.busy || Boolean(saved)} onClick={() => void operation.run(signal => previewImplementationMap(draft, signal), setPreview)}>检查映射覆盖</Button><Button tone="primary" disabled={Boolean(issue) || operation.busy || historyRead.busy || !preview || Boolean(saved)} onClick={() => preview && void operation.run(signal => confirmImplementationMap(draft, preview.preview_hash, signal), result => { setSaved(result); setPreview(result); updateAllocationJourney({ strategicUniverseId: universe.id, universeId: result.definition.universe_snapshot_id, implementationMappingId: result.id }); onSaved(result) })}>确认保存实施映射</Button></div>
