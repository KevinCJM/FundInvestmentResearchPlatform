...
export function RiskImpactResult({ result }: { result: RiskImpact }) {
  ...
  return <section className={`${sectionClass} space-y-5`} aria-label="本次压测结果">
    <div><h3 className="font-semibold">{result.name}</h3><p className="mt-1 text-xs text-slate-600">研究日期 {result.as_of} · {frequencyLabels[result.frequency]} · {result.transient ? '本次计算结果未写入磁盘' : '旧版历史结果 · 已保存'}</p></div>
    <div className="grid gap-5 sm:grid-cols-3"><div><p className="text-xs text-slate-600">模型解释的情景影响</p><p className={`mt-1 text-2xl font-semibold tabular-nums ${result.summary.terminal_return < 0 ? 'text-rose-700' : 'text-accent-800'}`}>{percentText(result.summary.terminal_return)}</p></div><div><p className="text-xs text-slate-600">所填假设本金的损益</p><p className="mt-1 text-xl font-semibold tabular-nums">{numberText(result.summary.pnl_amount, 2)} 元</p></div><div><p className="text-xs text-slate-600">{result.path.length > 1 ? '设定路径内最大回撤' : '冲击后相对价值'}</p><p className="mt-1 text-xl font-semibold tabular-nums">{result.path.length > 1 ? percentText(result.summary.max_drawdown) : numberText(result.summary.terminal_nav, 4)}</p></div></div>
    {result.path.length > 1 && <div><p className="mb-2 text-xs text-slate-600">以 1 为起点，仅展示冲击解释部分，未加入基线收益。</p><ReactECharts option={option} style={{ height: 250 }} notMerge /></div>}
    <div className="grid gap-5 lg:grid-cols-2"><div className="overflow-auto"><table className="w-full text-sm"><caption className="mb-2 text-left font-medium">哪些产品贡献了损益</caption><thead className="text-xs text-slate-600"><tr><th scope="col" className="p-2 text-left">产品</th><th scope="col" className="p-2 text-right">起始权重</th><th scope="col" className="p-2 text-right">终值贡献</th></tr></thead><tbody className="divide-y divide-slate-100">{result.by_asset.map(item => <tr key={item.key}><td className="p-2">{item.name}</td><td className="p-2 text-right tabular-nums">{percentText(item.weight)}</td><td className="p-2 text-right tabular-nums">{percentText(item.contribution)}</td></tr>)}</tbody></table></div><div className="overflow-auto"><table className="w-full text-sm"><caption className="mb-2 text-left font-medium">哪些风险来源贡献了损益</caption><thead className="text-xs text-slate-600"><tr><th scope="col" className="p-2 text-left">风险来源</th><th scope="col" className="p-2 text-right">终值贡献</th></tr></thead><tbody className="divide-y divide-slate-100">{result.by_factor.map(item => <tr key={item.id}><td className="p-2">{item.name}</td><td className="p-2 text-right tabular-nums">{percentText(item.contribution)}</td></tr>)}</tbody></table></div></div>
    <details className="border-t border-slate-100 pt-3"><summary className="cursor-pointer text-sm font-medium text-slate-700">路径数据、限制与来源</summary><div className="mt-3 space-y-2 text-xs leading-5 text-slate-600">{result.limitations.map(item => <p key={item}>{item}</p>)}{result.target.holdings_date && <p>使用的期末持仓：{result.target.holdings_date}；未重新运行组合策略。</p>}<p>资产贡献和因子贡献均已由后端与组合终值对账；未将单期百分比直接相加。</p><p className="break-all">情景：{result.request.scenario_release_id}</p><p className="break-all">风险模型成果：{result.request.exposure_release_id}</p><p className="break-all">结果：{result.id}</p></div><div className="mt-3 max-h-64 overflow-auto"><table className="w-full text-xs"><thead className="text-slate-600"><tr><th scope="col" className="p-2 text-left">期数</th><th scope="col" className="p-2 text-right">当期影响</th><th scope="col" className="p-2 text-right">相对价值</th><th scope="col" className="p-2 text-right">回撤</th></tr></thead><tbody>{result.path.map(item => <tr key={item.step}><td className="p-2">{item.step}</td><td className="p-2 text-right">{percentText(item.return)}</td><td className="p-2 text-right">{numberText(item.nav, 5)}</td><td className="p-2 text-right">{percentText(item.drawdown)}</td></tr>)}</tbody></table></div></details>
  </section>
}
...
export default function PublishedRiskPanel({ productKey, productName, portfolioRunId, portfolioOnly = false }: { productKey?: string; productName?: string; portfolioRunId?: string; portfolioOnly?: boolean }) {
  ...
  return <div className="min-w-0 space-y-4" data-testid="published-risk-panel"><section className={`${sectionClass} space-y-4`}>
    <div><h3 className="text-lg font-semibold">{productKey ? '这只产品怕什么？' : '使用已发布成果做压测'}</h3><p className="mt-1 text-sm leading-6 text-slate-600">这里只读取已发布敏感度与情景。需要更改算法或重新训练，请前往风险模型中心。</p></div>
    ...
    <fieldset disabled={busy} className="min-w-0 space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="风险研究日期" hint="与历史表现区间独立；不能使用该日以后才发布的模型。"><input className={inputClass} type="date" max={today()} value={asOf} onChange={event => setAsOf(event.target.value)} /></Field>{!productKey && !portfolioRunId && !portfolioOnly && <Field label="测试什么对象"><select className={inputClass} value={mode} onChange={event => setMode(event.target.value as typeof mode)}><option value="product">单个产品</option><option value="portfolio_run">已保存的产品组合</option></select></Field>}</div>
      {loading ? <p role="status" className="py-3 text-sm text-slate-600">正在读取本地已发布成果，不会重新计算敏感度…</p> : !exposures.length ? <Empty title={productKey ? '这只产品尚无已发布风险模型' : '还没有已发布风险模型'}><p>先在风险模型中心选择对象、计算并验证，然后发布成果。</p><Link className={`${primaryClass} mt-3`} to={riskLink}>到风险模型中心研究</Link></Empty> : <>
...
export function RiskImpactComparison({ primary, comparison }: { primary: RiskImpact; comparison: RiskImpact }) {
  return <section className={`${sectionClass} space-y-4`} aria-label="组合情景对比结果">
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[420px] text-sm"><thead className="text-left text-slate-600"><tr><th scope="col" className="p-2">比较项目</th><th scope="col" className="p-2">当前方案</th><th scope="col" className="p-2">对照方案</th></tr></thead><tbody className="divide-y divide-slate-100">
      <tr><th scope="row" className="p-2 text-left font-medium">组合</th>{[primary, comparison].map(value => <td className="p-2" key={value.id}>{value.target.name}</td>)}</tr>
      <tr><th scope="row" className="p-2 text-left font-medium">持仓截至</th>{[primary, comparison].map(value => <td className="p-2" key={value.id}>{value.target.holdings_date ?? '未提供'}</td>)}</tr>
      <tr><th scope="row" className="p-2 text-left font-medium">情景影响</th>{[primary, comparison].map(value => <td className="p-2 font-semibold tabular-nums" key={value.id}>{percentText(value.summary.terminal_return)}</td>)}</tr>
      <tr><th scope="row" className="p-2 text-left font-medium">假设本金损益</th>{[primary, comparison].map(value => <td className="p-2 tabular-nums" key={value.id}>{numberText(value.summary.pnl_amount, 2)} 元</td>)}</tr>
    </tbody></table></div>
  </section>
}
