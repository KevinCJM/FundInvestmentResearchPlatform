...
export default function PortfolioConstruction() {
  ...
  const [universeId, setUniverseId] = useState(() => searchParams.get('universe') ?? '')
  ...
  useEffect(() => {
    ...
    evaluateNumericControls([
      ...
      .catch((reason) => {
        if ((reason as DOMException)?.name !== 'AbortError') setNumericControlError('NJIT 权重校验暂不可用')
      })
    return () => controller.abort()
  }, [constituents])
  ...
  const readyToRun = Boolean(universe) && constituents.length >= 2 && minWeight <= maxWeight &&
    ...
    <section className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      <div className="space-y-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div><h2 className="text-base font-semibold">已选产品（至少 2 只）</h2>{!constituents.length ? <p className="mt-2 text-sm text-slate-500">搜索并加入产品后开始构建。</p> : <ul className="mt-2 space-y-2">{constituents.map((item, index) => <li key={`${item.kind}-${item.product_id}`} className="grid grid-cols-[1fr_auto] gap-2 rounded border border-slate-200 p-2"><span className="text-sm"><b>{item.name}</b><br /><span className="text-xs text-slate-500">{item.code ?? item.product_id} · {item.asset_class_name || '未指定大类'}</span></span><button type="button" onClick={() => setConstituents((current) => current.filter((_, currentIndex) => currentIndex !== index))} className="text-sm text-rose-700 underline">移除</button>{method === 'manual' && <label className="col-span-2 text-sm">权重 (%)<input aria-label={`${item.name} 权重`} type="number" min="0" max="100" value={item.weight ?? 0} onChange={(event) => updateNumber(index, 'weight', event.target.value)} className="ml-2 w-24 rounded border border-slate-300 px-2 py-1" /></label>}{method === 'risk_budget' && <label className="col-span-2 text-sm">风险预算 (%)<input aria-label={`${item.name} 风险预算`} type="number" min="0" max="100" value={item.risk_budget ?? 0} onChange={(event) => updateNumber(index, 'risk_budget', event.target.value)} className="ml-2 w-24 rounded border border-slate-300 px-2 py-1" /></label>}</li>)}</ul>}
          {(method === 'manual' || method === 'risk_budget') && (() => { const control = method === 'manual' ? weightControl : budgetControl; return <p className={`mt-2 text-sm ${control?.within_tolerance ? 'text-emerald-700' : 'text-amber-700'}`}>{method === 'manual' ? '权重' : '风险预算'}合计：{control ? `${control.total.toFixed(2)}%（需为 100%）` : numericControlError || 'NJIT 校验中…'}</p> })()}</div>
      </div>
      <div className="space-y-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
