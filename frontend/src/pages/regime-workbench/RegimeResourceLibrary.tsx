...
export default function RegimeResourceLibrary({ schemas, onAdd, onOpenDataLab, busy = false }: {
  ...
  return <aside className="min-w-0 rounded-xl border border-slate-200 bg-white p-3 shadow-sm" aria-label="历史情景节点资源库">
    <div className="flex items-center justify-between gap-2"><div><h3 className="text-sm font-bold text-slate-950">节点资源库 <RegimeHelpTip label="节点资源库说明" text="基础算子只做一件事；组合模板会展开为真实步骤；耦合内核保留必要的递推或联合求解。选择模板时可一次撤销。" /></h3><p className="mt-1 text-xs text-slate-600">选择数据、计算步骤和市场状态规则</p></div><button type="button" onClick={onOpenDataLab} className="min-h-9 shrink-0 rounded-lg bg-accent-50 px-2 text-xs font-bold text-accent-700">数据实验室</button></div>
    <label className="mt-3 block text-xs font-semibold text-slate-700">节点类型<select aria-label="节点类型" value={category} onChange={event => setCategory(event.target.value)} className="mt-1 min-h-10 w-full rounded-xl border border-slate-300 bg-white px-2 font-normal">
    ...
    })}{!visible.length && <p className="rounded-xl border border-dashed border-slate-300 p-4 text-center text-xs text-slate-600">没有匹配节点，请调整类型或搜索词。</p>}</div>
  </aside>
}
