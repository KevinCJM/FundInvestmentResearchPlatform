...
function ComputationNode({ id, data, selected }: NodeProps<FlowNode>) {
  ...
  if (data.appearance === 'constant') return <div data-node-kind="constant" className={`relative min-w-[132px] border px-4 py-3 shadow-sm ${appearance!.className} ${selected ? '!border-accent-600 ring-2 ring-accent-200' : !data.ready ? '!border-rose-500 ring-1 ring-rose-200' : ''}`}>
    <p className="text-xs font-bold text-amber-950"><span aria-hidden="true" className="mr-1.5">●</span>{data.label}</p>
    <p className="mt-1 text-xs text-amber-800">{data.schema?.category_label || s('graph.fixedNumber')}</p>
  ...
  return <div data-node-kind={data.appearance} className={`min-w-[210px] border px-3 py-3 shadow-sm ${appearance?.className || 'rounded-xl border-slate-300 bg-white'} ${selected ? '!border-accent-600 ring-2 ring-accent-200' : !data.ready ? '!border-rose-500 ring-1 ring-rose-200' : ''}`}>
    ...
    <p className="mt-1 text-xs font-semibold tracking-wide text-slate-600">{data.schema?.category_label || s('graph.calculationNode', {}, '计算节点')}</p>
    {data.statusLabel ? <p className="mt-1 text-xs text-slate-600">{data.statusLabel}</p> : null}
    <div className="mt-3 grid grid-cols-2 gap-x-5 gap-y-1 border-t border-slate-100 pt-2 text-xs text-slate-600">
...
function NodeCard({ node, index, count, selected, incoming, schema, onNodesChange, portLabel, readOnly }: {
  ...
  return <article className={`group relative min-w-0 border p-3 text-left shadow-sm transition ${appearance?.className || 'rounded-xl border-slate-200 bg-white'} ${selected ? '!border-accent-600 ring-2 ring-accent-200' : node.ready === false ? '!border-rose-500 ring-1 ring-rose-200' : ''}`} data-node-id={node.id} data-node-kind={node.appearance}>
    ...
    <div className="mt-3 flex items-center justify-end gap-1 border-t border-slate-100 pt-2">
      <button type="button" aria-label={s('graph.moveLeft', { name: label })} disabled={readOnly || index === 0} onClick={() => move(-1)} className="min-h-8 rounded-lg px-2 text-xs font-bold text-slate-600 hover:bg-slate-100 disabled:opacity-30">←</button>
      <button type="button" aria-label={s('graph.moveRight', { name: label })} disabled={readOnly || index === count - 1} onClick={() => move(1)} className="min-h-8 rounded-lg px-2 text-xs font-bold text-slate-600 hover:bg-slate-100 disabled:opacity-30">→</button>
      <button type="button" aria-label={s('graph.deleteNamed', { name: label })} disabled={readOnly} onClick={() => onNodesChange([{ type: 'remove', id: node.id }])} className="min-h-8 rounded-lg px-2 text-xs font-bold text-rose-700 hover:bg-rose-50 disabled:opacity-30">{s('common.delete')}</button>
    </div>
  </article>
}
...
export default function GraphCanvas({ nodes, edges, schemas, selectedNodeId, onNodesChange, onConnect, onDuplicate, onSelectionChange,
  ...
  return <section aria-label={ariaLabel} data-testid={testIds.canvas} tabIndex={0} onKeyDown={handleKeyboard} className="relative min-h-[420px] overflow-auto rounded-xl border border-slate-200 bg-slate-50 p-3 sm:p-4" style={{ backgroundImage: 'radial-gradient(#cbd5e1 1px, transparent 1px)', backgroundSize: '18px 18px' }}>
