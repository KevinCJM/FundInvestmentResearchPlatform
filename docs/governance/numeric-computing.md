...
- 经明确授权接入的 CalMetricsEngine C++ AOT 数值路径属于正式执行后端，与固定签名 NJIT 并列；本节 NJIT 专属签名与预热要求适用于仍使用 NJIT 的路径。此规则不自动切换已有服务，也不豁免 Python 数值回调或回退。
- C++ AOT 必须来自预先构建的原生包，并提供可验证的引擎版本、构建身份、算子/IR 契约、计划身份、CPU 预算和结果所有权；缺少原生包或有效凭据时失败关闭。构建机器码不得发生在服务启动或正式请求期间；读取公式与构造原生执行图不属于 JIT 编译。
- C++ AOT 与 NJIT 遵守相同的数据轴、逐指标错误、参数、历史定义、dtype、只读输入、零拷贝与生命周期约束；正式计算结果须通过前后端各自的对应执行门禁，禁止伪造 NJIT 签名为 C++ 放行。
