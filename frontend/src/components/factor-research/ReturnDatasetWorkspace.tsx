...
export default function ReturnDatasetWorkspace({ action, busy, datasetId, onSelect, onAttribute }: { action: Action; busy: boolean; datasetId?: string; onSelect: (id: string) => void; onAttribute: (id: string) => void }) {
  ...
  return <div className="min-w-0 space-y-5">
    <Card title="收益率数据集">
      ...
      {datasets.length ? <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[630px] text-left text-sm" aria-label="收益率数据集目录"><thead className="text-xs text-slate-500"><tr><th className="p-2">数据集</th><th>算法 / 来源</th><th>市场</th><th>因子列</th><th>覆盖区间</th><th>天数</th></tr></thead><tbody>{datasets.map(dataset => <tr key={dataset.id} className="border-t border-slate-100"><th className="p-2 font-medium"><button className="text-left text-indigo-700 hover:underline disabled:opacity-50" disabled={busy} onClick={() => onSelect(dataset.id)}>{dataset.name}</button></th><td className="p-2">{returnMethodName(dataset.source_method)}</td><td className="p-2">{dataset.market}/{dataset.currency}</td><td className="p-2">{(dataset.factor_names || ['MKT_RF', 'SMB', 'HML']).join(', ')}</td><td className="whitespace-nowrap p-2 text-xs">{dataset.start_date}<br />{dataset.end_date}</td><td className="p-2">{dataset.observations}</td></tr>)}</tbody></table></div> : <p className="mt-4 text-sm text-slate-500">尚无数据集。可在“构建工作台”生成，或在下方导入已有的真实因子收益。</p>}
    </Card>
    <Card title="导入已有因子收益">
