...
Git 候选模式也从对应 Git 内容加载检查器自身，在内存中执行该版本的校验入口；候选检查器缺失、语法或初始化失败时直接报错，不用工作区版本替代。检查器代码同样绑定审核指纹。本地启动入口能够运行是执行前提。默认工作区范围合并结构化的暂存/未暂存差异和未跟踪路径，改名必须保留原路径与新路径；显式 `--changed-file` 仍只使用指定范围。
