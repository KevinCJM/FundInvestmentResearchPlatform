...
export default function ResearchIndicatorPanel({
  ...
  return <section className="min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm" aria-labelledby="research-indicators-title">
    ...
    {catalog.length === 0
      ...
      : <>
        {/* 一条工具栏：三个控件底边对齐、靠左挨在一起，说明合并成一行放在下面。 */}
        <div className="flex flex-wrap items-end gap-x-5 gap-y-3 border-t border-slate-100 bg-slate-50 px-5 py-4">
          <MetricSelector
            indicators={catalog}
          ...
          <div className="min-w-0">
            ...
            <div role="group" aria-labelledby="research-period-columns" className="mt-1 flex flex-wrap items-center gap-2">
              {periods.map((period) => <span key={period} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-accent-200 bg-accent-50 py-1 pl-3 pr-1 text-sm font-medium text-accent-700">
              ...
              <select
                ...
                className="min-h-11 rounded-lg border border-dashed border-slate-300 bg-white px-3 text-sm text-slate-700 focus:border-accent-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 disabled:cursor-not-allowed disabled:opacity-50"
              >
          ...
          <label className="block">
            <span className="block text-xs font-medium text-slate-600">截止日（可选）</span>
            <input type="date" value={asOf} onChange={(event) => onAsOfChange(event.target.value)} className="mt-1 block min-h-11 w-44 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 focus:border-accent-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500" />
          </label>
          <p className="w-full text-xs leading-5 text-slate-600">{columnHint} 截止日：{asOfHint}</p>
        </div>
