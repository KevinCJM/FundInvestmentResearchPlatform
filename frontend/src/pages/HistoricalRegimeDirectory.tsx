  ...
  return <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2"><h3 className="text-base font-bold text-slate-950">{definition.name}</h3><span className="rounded-full bg-accent-50 px-2 py-1 text-xs font-bold text-accent-700">V2 · r{definition.revision || 1}</span></div>
    ...
    <dl className="mt-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
      <div className="rounded-xl bg-slate-50 p-2"><dt className="text-xs font-bold text-slate-600">计算节点</dt><dd className="mt-1 font-bold text-slate-900">{definition.graph.nodes.length}</dd></div>
      <div className="rounded-xl bg-slate-50 p-2"><dt className="text-xs font-bold text-slate-600">状态语义</dt><dd className="mt-1 font-bold text-slate-900">{definition.states.length}</dd></div>
      <div className="rounded-xl bg-slate-50 p-2"><dt className="text-xs font-bold text-slate-600">正式运行</dt><dd className="mt-1 font-bold text-slate-900">{runs.length}</dd></div>
      <div className="rounded-xl bg-slate-50 p-2"><dt className="text-xs font-bold text-slate-600">最近运行</dt><dd className="mt-1 truncate font-bold text-slate-900">{latestRun ? formatTime(latestRun.created_at) : '尚无'}</dd></div>
    </dl>
    <div className="mt-3 flex flex-wrap items-center gap-1.5 text-xs"><span className="font-bold text-slate-600">下游应用</span>{usages.length ? usages.map((usage) => <span key={usage} className="rounded-full bg-emerald-50 px-2 py-1 font-bold text-emerald-800">{usageLabel(usage)}</span>) : <span className="text-slate-600">尚无已发布绑定</span>}<span className="ml-auto text-slate-600">更新 {formatTime(definition.updated_at || definition.created_at)}</span></div>
  </article>
}
...
export default function HistoricalRegimeDirectory() {
  ...
  return <div className="space-y-5" data-testid="historical-regime-directory">
    ...
    {notice ? <p role="status" className="rounded-xl border border-accent-200 bg-accent-50 px-4 py-3 text-sm text-accent-950">{notice}</p> : null}
    ...
    <section id="regime-templates" tabIndex={-1} aria-labelledby="templates-heading" className="scroll-mt-5 rounded-xl border border-slate-200 bg-white p-4 shadow-sm focus:outline-accent-500 sm:p-5">
      <div className="flex items-end justify-between gap-3"><div><h2 id="templates-heading" className="text-lg font-black text-slate-950">选择研究模板</h2><p className="mt-1 text-xs leading-5 text-slate-600">先选要识别的市场状态，再设置数据与参数、运行看结果。模板中的规则都可以查看和修改，也可以随时进入自由画板。</p></div><span className="shrink-0 text-xs font-bold text-slate-600">{templates.length} 个</span></div>
      {(['realtime', 'retrospective'] as const).map(mode => {
        ...
        return <section key={mode} aria-label={label} className="mt-5">
          ...
          <p className="mt-1 text-xs text-slate-600">{mode === 'realtime' ? '使用当时可得数据；也可用于历史回放。' : '用于解释历史区间；需要之后的数据，不能用作当时的交易信号。'}</p>
    ...
    <section id="regime-saved" tabIndex={-1} aria-labelledby="saved-v2-heading" className="scroll-mt-5 focus:outline-accent-500">
      <div className="mb-3 flex items-end justify-between gap-3"><div><h2 id="saved-v2-heading" className="text-lg font-black text-slate-950">已有识别方案</h2><p className="mt-1 text-xs text-slate-600">打开保存的版本继续研究，已有结果和发布引用保留原版本。</p></div><span className="text-xs font-bold text-slate-600">{definitions.length} 个方案</span></div>
      ...
      {!loading && !definitions.length ? <EmptyState title="还没有识别方案" hint="选择上方模板开始第一次研究，保存后可在这里继续。" action={<a href="#regime-templates" className="inline-flex min-h-10 items-center rounded-lg bg-accent-600 px-4 text-sm font-semibold text-white hover:bg-accent-700">选择第一个模板</a>} /> : null}
    </section>

    <details className="rounded-xl border border-amber-200 bg-amber-50/50 p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 space-y-2">{legacyDefinitions.map((definition) => {
        const key = `${definition.id}:${definition.revision}`
        return <article key={key} className="flex flex-col gap-3 rounded-xl border border-amber-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-bold text-slate-950">{definition.name}</h3><span className="rounded-lg bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-900">V1 · r{definition.revision || 1}</span>{definition.status ? <span className="text-xs font-bold text-slate-500">{definition.status}</span> : null}</div><p className="mt-1 text-xs text-slate-500">{definition.target?.name || definition.target?.series_id || '未命名数据源'} · {definition.algorithm?.family || '未知模型'}</p></div><button type="button" disabled={!definition.id || !definition.revision || copying === key} onClick={() => void copyLegacy(definition)} className="min-h-9 shrink-0 rounded-lg bg-amber-900 px-3 text-xs font-bold text-white disabled:opacity-40">{copying === key ? '迁移中…' : '复制为 V2 图谱'}</button></article>
      })}{!loading && !legacyDefinitions.length ? <p className="rounded-xl bg-white p-4 text-xs text-slate-600">没有需要迁移的 V1 定义。</p> : null}</div>
    </details>
  </div>
}
