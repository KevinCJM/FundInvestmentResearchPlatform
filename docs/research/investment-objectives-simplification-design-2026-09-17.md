...
目标类型切换只调整目标专属成功条件。已填写的现金预算（本金、组合外储备、日期及现金流）在三种目标间保留；绝对/相对目标间保留兼容的现金保护。只有用户明确关闭可选现金计划时才清除预算。未填写预算的新绝对/相对目标继续默认关闭，切入资金目标才按需创建空预算。

产品优先入口读取 `mandate` URL 参数并写入当前研究旅程，按既有失效规则清除旧 SAA/TAA；产品快照创建/恢复及大类构建继续保留该目标，使没有 mandate 参数的后续 SAA 页面也使用已选择目标。

非资金目标的现金计划可明确选择“仅资金算账”“所有必要支付完成”或“完成支付并保护期末余额”。仅支付保护提交 `cash_protection.mode=payments_only`，不伪造期末目标；后端沿用参考/SAA 的资金成功概率和独立验证。未选择保护时继续只做确定性预算约束。

用户修改投资期限时，按既有循环计划的开始月及频率重新推导末次支付月；单次支付保持原月份。非整数、空值或超出1–30年的输入不会改写计划，须先修正期限；开始月超出新期限时仍按原校验阻止提交，不静默删除支付。

若缩短后的期限不足以容纳循环计划的第二次支付（包括开始月已越界），保留原循环末月与频率，并由越界校验阻止提交；重新延长期限后再重算。不得通过把末月压成开始月而把循环计划变成单次；需要单次时由用户明确更改频率。

上述循环意图保护同样覆盖开始月和频率编辑：暂时无法容纳第二次发生时，以越界末月保留循环状态并禁止提交，不通过日期相等静默改成单次。仅支付保护必须至少存在一笔 withdrawal；空计划、仅投入或移除最后一笔支付后，在填写页说明原因并禁用诊断，不等到后端才拒绝。

支付加期末底线模式须填写有限且大于零的保护金额；空值、非有限值、零或负数在填写页阻止诊断。清空金额保留已选保护模式并提示补齐，不自动降为无保护。

历史季度/年度记录即使 first_month=last_month，every_months=3/12 仍表示已保存的循环意图，编辑和延长期限须按该频率恢复后续发生；本界面显式单次使用端点相等且every_months=1。后端 CashProtection 同样拒绝零金额期末保护，保留 payments_only 以及含支付的 funding_goal 零期末目标原契约。
