...
export const timingField = 'mt-1 min-h-10 w-full min-w-0 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-300 disabled:bg-slate-50'
const actionClass = 'min-h-9 rounded-lg border border-slate-300 bg-white px-3 py-1 text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-40'
...
function FlowStep({ data }: NodeProps) {
  ...
  return <div className={`w-56 rounded-xl border bg-white p-3 shadow-sm ${item.selected ? 'border-indigo-500 ring-2 ring-indigo-100' : 'border-slate-300'}`}>
    <strong className="block truncate text-sm text-slate-900">{item.label}</strong><span className="text-xs text-slate-500">{item.operator?.label || '未知步骤'}</span>
    <div className="mt-3 space-y-2">{item.operator?.inputs.map(port => <div key={port.name} className="relative text-left text-xs text-slate-500" title={typeLabels[port.type]}><Handle type="target" id={port.name} position={Position.Left} style={{ left: -18, top: 7 }} />{port.label} · {typeLabels[port.type]}</div>)}</div>
    <div className="mt-3 space-y-2">{item.operator?.outputs.map(port => <div key={port.name} className="relative text-right text-xs text-indigo-600" title={typeLabels[port.type]}>{port.label} · {typeLabels[port.type]}<Handle type="source" id={port.name} position={Position.Right} style={{ right: -18, top: 7 }} /></div>)}</div>
  </div>
}
  ...
  const renderStep = (node: TimingNode, index: number) => {
    ...
    return <section key={node.id} className="min-w-0 rounded-xl border border-slate-200 bg-white">
      <button type="button" aria-expanded={expanded === node.id} onClick={() => setExpanded(expanded === node.id ? '' : node.id)} className="flex w-full items-start gap-3 p-4 text-left">
        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-50 text-xs font-semibold text-indigo-700">{index + 1}</span>
        <span className="min-w-0 flex-1"><strong className="block break-words text-sm text-slate-900">{node.label}</strong><span className="mt-1 block text-xs leading-5 text-slate-500">{operator?.description || '此步骤未在当前目录中注册。'}</span>{missing.length > 0 && <span className="mt-1 block text-xs text-amber-700">待连接：{missing.map(port => port.label).join('、')}</span>}</span>
        <span aria-hidden="true" className="text-slate-400">{expanded === node.id ? '−' : '+'}</span>
      </button>
      {expanded === node.id && <div className="space-y-3 border-t border-slate-100 p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">{operator?.inputs.map(port => <label key={port.name} className="min-w-0 text-xs font-medium text-slate-600">{port.label}<span className="ml-1 font-normal text-slate-500">· {typeLabels[port.type]}</span>{inputSelect(node.inputs[port.name] || '', port.type, node.id, value => patchNode(node.id, { inputs: { ...node.inputs, [port.name]: value } }), `${node.label} ${port.label}`, !port.required)}</label>)}
