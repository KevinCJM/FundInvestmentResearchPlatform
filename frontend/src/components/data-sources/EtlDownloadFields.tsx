...
export default function EtlDownloadFields({ step, catalog, onChange, chooseInterface = true }: {
  ...
  return <div className="min-w-0 space-y-3">
    ...
    {!fields.length && record ? <p className="text-xs text-slate-600">此接口尚未声明请求字段；可在下方填写参数，或在接口配置中补充请求字段定义。</p> : null}
    {record ? <p className="text-xs leading-5 text-slate-600">接口修订 {step.interface_revision} · 目标：{record.config.mappings.filter(m => m.enabled).map(m => catalog.targets.tables.find(t => t.table_id === m.target_table)?.label ?? m.target_table).join('、')}。<Link className="text-accent-700 underline" to={`/settings/source-center?source=${step.source_id}&interface=${step.interface_id}`}>配置来源与映射</Link></p> : null}
