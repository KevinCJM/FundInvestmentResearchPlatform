...
export default function PortfolioOnboardingWorkspace() {
  ...
  return <div className="space-y-5" data-testid="portfolio-onboarding-workspace">
    <StaticDemoBanner />
    <header className="rounded-xl bg-gradient-to-r from-amber-800 via-orange-700 to-slate-900 p-6 text-white shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-100">Approved research → actual portfolio master</p><h2 className="mt-2 text-2xl font-bold">组合落地与启用</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-amber-50">把平台外已批准研究方案登记为真实组合主数据，或为存量真实组合关联新的目标版本；后续投中、会计与投后统一引用同一个 portfolio_id。</p></header>
    ...
    <nav className="grid gap-2 rounded-xl border border-slate-200 bg-white p-2 shadow-sm sm:grid-cols-2" aria-label="组合落地模式">
      ...
      <button type="button" aria-pressed={mode === 'adjustment'} onClick={() => { setMode('adjustment'); setNotice('') }} className={`min-h-11 rounded-lg px-4 py-3 text-left text-sm font-semibold ${mode === 'adjustment' ? 'bg-amber-800 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><span className="block">存量组合调整</span><span className={`mt-1 block text-xs font-normal ${mode === 'adjustment' ? 'text-amber-100' : 'text-slate-600'}`}>沿用 portfolio_id，登记新目标版本</span></button>
    </nav>

    {mode === 'new' ? <form onSubmit={submitNewPortfolio} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div><h3 className="font-semibold text-slate-900">研究方案落地登记</h3><p className="mt-1 text-sm text-slate-600">仅可选择已记录平台外批准结果的研究方案。</p></div><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <label className="text-sm text-slate-600">已批准研究方案<select aria-label="已批准研究方案" value={newDraft.planId} onChange={(event) => setNewDraft((current) => ({ ...current, planId: event.target.value }))} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2">{approvedResearchPlans.map((plan) => <option key={plan.planId} value={plan.planId}>{plan.name} · {plan.version}</option>)}</select></label>
      ...
      <label className="text-sm text-slate-600">管理人主体<select aria-label="管理人主体" value={newDraft.managerId} onChange={(event) => setNewDraft((current) => ({ ...current, managerId: event.target.value }))} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2">{managerEntities.map((manager) => <option key={manager.entityId} value={manager.entityId}>{manager.name} · {manager.entityId}</option>)}</select></label>
      ...
      <label className="text-sm text-slate-600">记账本位币<select aria-label="记账本位币" value={newDraft.baseCurrency} onChange={(event) => setNewDraft((current) => ({ ...current, baseCurrency: event.target.value }))} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2"><option>CNY</option><option>USD</option><option>HKD</option></select></label>
      ...
      <label className="text-sm text-slate-600">会计政策版本<select aria-label="会计政策版本" value={newDraft.accountingPolicyVersion} onChange={(event) => setNewDraft((current) => ({ ...current, accountingPolicyVersion: event.target.value }))} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2">{accountingPolicyVersions.map((version) => <option key={version}>{version}</option>)}</select></label>
      <label className="text-sm text-slate-600">共享执行通道<select aria-label="共享执行通道" value={newDraft.executionAccountId} onChange={(event) => setNewDraft((current) => ({ ...current, executionAccountId: event.target.value }))} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2">{externalAccounts.filter((account) => account.kind === '执行通道账户').map((account) => <option key={account.accountId} value={account.accountId}>{account.name} · {account.accountId}</option>)}</select></label>
      ...
      <label className="text-sm text-slate-600">首任基金经理<select aria-label="首任基金经理" value={newDraft.portfolioManagerId} onChange={(event) => setNewDraft((current) => ({ ...current, portfolioManagerId: event.target.value }))} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2">{Array.from(new Map(portfolioManagerAssignments.map((assignment) => [assignment.portfolioManagerId, assignment])).values()).map((assignment) => <option key={assignment.portfolioManagerId} value={assignment.portfolioManagerId}>{assignment.portfolioManagerName} · {assignment.portfolioManagerId}</option>)}</select></label>
      <div className="rounded-xl bg-slate-50 p-4 text-sm"><p className="text-xs text-slate-600">自动带入的版本关系</p><p className="mt-2 font-semibold text-slate-900">批准 {selectedPlan.approvalRecordId}</p><p className="mt-1 text-slate-600">初始目标 {selectedPlan.targetVersion}</p></div>
    ...
    {mode === 'adjustment' ? <form onSubmit={submitAdjustment} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div><h3 className="font-semibold text-slate-900">存量组合目标版本调整</h3><p className="mt-1 text-sm text-slate-600">调整研究关系不会新建组合，也不会覆盖历史目标版本。</p></div><div className="mt-5 grid gap-4 md:grid-cols-2">
      <label className="text-sm text-slate-600">存量真实组合<select aria-label="存量真实组合" value={adjustmentDraft.portfolioId} onChange={(event) => setAdjustmentDraft((current) => ({ ...current, portfolioId: event.target.value }))} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2">{actualPortfolioDemoData.map((portfolio) => <option key={portfolio.portfolioId} value={portfolio.portfolioId}>{portfolio.name} · {portfolio.portfolioId}</option>)}</select></label>
