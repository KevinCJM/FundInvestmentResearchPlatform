...
- 列内指标窗口不一致时，单元格必须展示各自起止日期，不能仅以“口径随指标不同”替代可追溯日期。（原文要求同时展示观察值数，已被第 6 节取代。）浏览器回归使用区间列的添加/移除控件及指标名称入口，不再定位已删除的卡片控件。
...
## 6. 取消指标条数上限与行内瘦身（2026-09-17）

### 6.1 条数上限

`MAX_RESEARCH_INDICATORS = 8` 是产品层面的限制，没有技术理由，删除；
`MetricSelector` 不传 `maxSelected` 即为不限，计数显示成「已选 44」而不是「已选 8/8」。

后端把两个本来同值、含义不同的上限拆开：

| 常量 | 管什么 | 值 |
| --- | --- | --- |
| `MAX_INDICATORS` | 预热时 NJIT 批编译的分批宽度、评价方案的指标条数 | 10（不变） |
| `MAX_EVALUATION_INDICATORS` | 一次 evaluate 请求里的指标条数（路由模型与服务端校验共用） | 200（原来复用 10） |

拆开的理由：研究看板的条数是读者自己的清单，和批编译宽度、评价方案规模不是同一件事；
原来三处共用一个 10，改其中一个就会误改另外两个。实测本项目标量目录已有 44 个指标，
全选后一次请求全部算出，无错误。
...
| --- | --- |
| `内置 v1` | 移入指标名的悬停提示 |
| `高优先 / 低优先` | 改为 `越高越好 / 越低越好`；`neutral` 写全 `仅展示，不判断优劣`。定义抽屉里的「数值高优先」同步改口径，避免同一屏两种说法 |
| `246 个观察值`（列头与单元格） | 删除。走势图标题里的同一串也一并删除 |
| 数值 `text-base font-semibold` | 改为 `text-sm`（仍 `tabular-nums` 右对齐）。一屏几十个数时，每个都加粗放大等于都不突出 |

顺带：`windowKey` 原来把 `observation_count` 计入"同一口径"的判定，
既然不再展示，判定键就改成只按起止日期——键与显示的内容一致，
不会出现"列头说口径不同、格子里却显示同样的日期"。

### 6.3 工具栏

原来是 `lg:grid-cols-[17rem_1fr_15rem]`：选择器没有标题、和另外两个字段的基线对不齐，
截止日被推到卡片最右侧，两条说明文字各自挂在不同 x 位置。改为一条 flex 工具栏：

- `items-end`：三个控件底边对齐，控件高度统一 `min-h-11`。
- 靠左依次排列，不再横跨整张卡片。
- 两条说明合并成一行放在工具栏底部（`w-full` 的 flex 项，自然换行到第二行）。
- 工具栏底色 `bg-slate-50`，读起来是一条筛选栏而不是浮在卡片里的散控件。

### 6.4 自测

| 检查 | 结果 |
| --- | --- |
| `pytest tests/test_custom_indicator_routes.py tests/test_custom_indicator_service.py tests/test_evaluation_run_results.py` | 44 passed |
| `vitest run`（全量） | 147 files / 1156 tests passed |
| `tsc --noEmit`、`design:check`、`check_i18n` | 通过 / 无回归 |
| 浏览器（真实后端，1440 / 768 / 320） | 目录 44 个标量指标全选、全部算出、无错误横幅、无控制台报错；页面无「个观察值」字样；工具栏三个控件底边同为 522px；三个视口无横向溢出 |
