...
export default function SourceEditor({ record, editingEnabled, onSaved, onDirty, onCredentialChanged, onNext, onBusy }: {
  ...
  return <form ref={form} noValidate className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5" onSubmit={event => {
    ...
    <fieldset disabled={busy || !editingEnabled} className="space-y-5">
      <details open={record.revision === 0} className="rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer text-sm font-semibold">连接地址与协议{record.revision > 0 ? '（已有配置通常不用修改）' : ''}</summary><div className="mt-3 grid gap-3 sm:grid-cols-2">
