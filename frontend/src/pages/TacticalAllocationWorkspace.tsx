...
export default function TacticalAllocationWorkspace() {
  ...
  useEffect(() => {
    ...
    getHistoricalRegimeRun(id).then(async value => {
      ...
      if (binding) {
        ...
        )) throw new Error('历史参考的状态轴与绑定版本不一致，请重新核验引用。')
  ...
  const applyBlocked = (preview?.request.decision_policy && !preview.application ? '缺少执行资格证据，请重新计算后交接。' : '') || (preview?.request.decision_policy && preview.request.current_weights_as_of !== today() ? '实际持仓快照不是今天，请更新持仓并重新研究后交接。' : '') || (preview?.application?.eligible === false ? preview.application.reasons.join('；') : '') || pitConflict || (preview?.policy_check?.current_application_eligible === false ? '政策已到复核日或不具备当前应用资格，请重新确认政策；历史研究可保留。' : '') || (preview?.policy_check?.within_limits === false ? preview.policy_check.violations.join('；') : '') || (preflight?.quality.status === 'blocked' ? '数据存在数量级跳变，请先返回大类检查产品。' : selectedCandidate?.validation_feasible === false ? '所选候选在独立验证区间超出约束，请先复核。' : preview?.recommendation.turnover_from_current != null && preview.recommendation.turnover_from_current > preview.request.max_turnover + 1e-8 ? '从参考持仓调整到当前方案的换手超过上限。' : '')
  ...
  return <div className="min-h-screen min-w-0 max-w-full break-words bg-slate-50 pb-12 text-slate-800"><main className="mx-auto max-w-[1500px] min-w-0 space-y-4 px-3 py-4 sm:px-7">
    ...
    {loadingBaseline && <p role="status" className="rounded-xl bg-white p-5 text-sm text-slate-600">正在加载锁定的基准与资产范围…</p>}
    {baseline && request && <>
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        <section className={`${sectionClass} space-y-4`} aria-label="本次资产配置"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-semibold text-slate-950">本次准备怎么配？</h2><p className="mt-1 text-xs leading-5 text-slate-600">{baseline.name} · 方案日期 {baseline.as_of.slice(0, 10)}</p></div><span className="rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs text-amber-900">{preview?.data.pit.status === 'verified' ? '查看逐时点验证范围' : '研究结果 · 历史 PIT 待核验'}</span></div>
          <div className="space-y-3 sm:hidden">{baseline.assets.map(asset => <article key={asset.id} className="rounded-lg border border-slate-200 p-3"><h3 className="text-sm font-semibold">{asset.name}</h3><dl className="mt-3 grid grid-cols-3 gap-2 text-xs"><div><dt className="text-slate-600">SAA</dt><dd className="mt-1 font-medium tabular-nums">{percentText(asset.base_weight)}</dd></div><div><dt className="text-slate-600">拟议权重</dt><dd className="mt-1 font-semibold tabular-nums text-accent-900">{preview ? percentText(preview.recommendation.weights[asset.id]) : '待计算'}</dd></div><div><dt className="text-slate-600">偏离百分点</dt><dd className="mt-1 font-medium tabular-nums">{preview ? `${(preview.recommendation.tilts[asset.id] * 100).toFixed(2)}` : '—'}</dd></div></dl><p className="mt-3 text-xs leading-5 text-slate-600">允许 {percentText(asset.min_weight)}–{percentText(asset.max_weight)}；{preview?.recommendation.trade_deltas ? `持仓差额 ${points(preview.recommendation.trade_deltas[asset.id])}` : '未提供参考持仓'}</p></article>)}</div>
          <div className="hidden overflow-auto sm:block"><table className="w-full min-w-[540px] text-sm"><caption className="sr-only">SAA 与拟议战术权重，偏离以百分点表示</caption><thead className="text-xs text-slate-600"><tr><th scope="col" className="px-2 py-3 text-left">资产</th><th scope="col" className="px-2 py-3 text-right">SAA</th><th scope="col" className="px-2 py-3 text-right">拟议权重</th><th scope="col" className="px-2 py-3 text-right">相对 SAA</th><th scope="col" className="px-2 py-3 text-right">参考持仓差额</th></tr></thead><tbody className="divide-y divide-slate-100">{baseline.assets.map(asset => <tr key={asset.id}><th scope="row" className="px-2 py-3 text-left font-medium">{asset.name}<span className="mt-1 block text-xs font-normal text-slate-600">允许 {percentText(asset.min_weight)}–{percentText(asset.max_weight)}</span></th><td className="px-2 py-3 text-right tabular-nums">{percentText(asset.base_weight)}</td><td className="px-2 py-3 text-right font-semibold tabular-nums text-accent-900">{preview ? percentText(preview.recommendation.weights[asset.id]) : '待计算'}</td><td className="px-2 py-3 text-right tabular-nums">{preview ? points(preview.recommendation.tilts[asset.id]) : '—'}</td><td className="px-2 py-3 text-right text-xs tabular-nums">{preview?.recommendation.trade_deltas ? points(preview.recommendation.trade_deltas[asset.id]) : '未提供参考持仓'}</td></tr>)}</tbody></table></div><p className="text-xs leading-5 text-slate-600">偏离是与 SAA 的差值；参考持仓差额用于比较拟议目标，不代表实际订单。拟议权重由后端约束计算后展示。</p>
        </section>
        <aside className={`${sectionClass} space-y-4`} aria-label="本次决策摘要"><p className="text-xs font-semibold text-accent-800">本次决策</p><h2 className="text-lg font-semibold text-slate-950">{preview?.application ? ({ maintain: '维持持仓', waiting_execution: '等待执行', adjustment_proposal: '形成调整建议', ineligible: '不可交接' }[preview.application.state]) : preview ? preview.recommendation.is_saa ? '维持长期配置' : '已生成偏离方案' : '先验证，再决定'}</h2><p className="text-sm leading-6 text-slate-600">{preview?.recommendation.reason ?? '选择信号与偏离边界，查看扣费后是否仍有改善。保持 SAA 始终参与比较。'}</p>{preview?.application && <div role="status" className="space-y-1 text-sm text-amber-800">{preview.application.reasons.map(reason => <p key={reason}>{reason}</p>)}<p>可执行目标的决策日：{preview.application.decision_date ?? '尚未形成'}；本次执行机会：{preview.application.execution_opportunity ? '是' : '否'}。仅为研究建议。</p>{preview.application.pending_decision && <p>最新决策 {preview.application.latest_decision_date} 仍在等待滞后，尚未用于执行目标。</p>}</div>}{preview?.recommendation.signal_details && <details className="text-xs leading-5"><summary className="cursor-pointer font-medium text-accent-800">为什么这样调整？</summary><div className="mt-2 space-y-2">{preview.recommendation.signal_details.map(item => <div key={item.asset_id}><p className="font-medium">{baseline.assets.find(asset => asset.id === item.asset_id)?.name ?? item.asset_id} · {item.direction}</p><p>信号 {item.value == null ? '不可用' : percentText(item.value)} · {item.signal_date ?? '无可用日期'}</p><p>{item.window?.window_start && <>观察 {item.window.window_start} — {item.window.window_end}；最晚公布 {item.window.available_at}；距决策 {item.window.lag_days} 天。</>}</p><p>原始偏离 {points(item.raw_tilt)} → 约束后 {points(item.applied_tilt)}</p>{item.constraint_reason && <p className="text-amber-800">{item.constraint_reason}</p>}</div>)}</div></details>}{preview && <dl className="space-y-2 text-xs"><div className="flex justify-between gap-2"><dt className="text-slate-600">决策日期</dt><dd>{preview.request.as_of}</dd></div><div className="flex justify-between gap-2"><dt className="text-slate-600">信号日期</dt><dd>{preview.recommendation.signal_date ?? '暂无可用信号'}</dd></div><div className="flex justify-between gap-2"><dt className="text-slate-600">复核到期</dt><dd className={expired ? 'text-amber-800' : ''}>{preview.recommendation.expires_on}{expired ? ' · 已过期' : ''}</dd></div></dl>}{formIssue && <p role="status" className="rounded-lg bg-amber-50 p-3 text-xs leading-5 text-amber-900">{formIssue}</p>}<button type="button" className={`${primaryClass} w-full`} disabled={busy || checking || !preflight?.can_calculate || Boolean(formIssue)} onClick={() => void calculate()}>{busy ? '正在计算与验证…' : checking ? '正在检查研究条件…' : preview ? '重新计算研究' : '计算并比较方案'}</button><p className="text-xs leading-5 text-slate-600">使用本地真实数据。保存研究版本、进入产品配置均需显式操作。</p></aside>
      </div>
