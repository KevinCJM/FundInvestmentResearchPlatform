...
const button = 'min-h-10 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500'
...
const IndicatorGraphEditor = forwardRef<IndicatorGraphEditorHandle, Props>(function IndicatorGraphEditor(props, ref) {
  ...
  return <div ref={rootRef} tabIndex={-1} role={fullscreen ? 'dialog' : 'region'} aria-modal={fullscreen || undefined} aria-label={s(fullscreen ? 'graph.fullscreenAria' : 'graph.editorAria')} className={fullscreen ? 'fixed inset-0 z-[100] overflow-auto bg-slate-50 p-3 sm:p-5' : 'mt-4 min-w-0'} onKeyDown={event => {
    ...
    <div className={`mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3 ${fullscreen ? 'sticky top-0 z-30' : ''}`}>
      <div><h3 className="text-base font-semibold text-slate-900">{fullscreen ? draft.name : s('graph.title')}</h3><p className="mt-1 text-xs text-slate-600">{s('graph.hint')}</p></div>
      <div className="flex flex-wrap gap-2"><button type="button" className={button} disabled={!editor.loaded || editor.busy || disabled || importing} onClick={() => void editor.check(false)}>{s('graph.check')}</button><button type="button" disabled={!editor.loaded || editor.busy || disabled || importing} onClick={() => void editor.check(true)} className={`${button} !border-accent-600 !bg-accent-600 !text-white`}>{s(editor.busy ? 'graph.checking' : 'graph.apply')}</button><button ref={fullButtonRef} type="button" className={button} aria-pressed={fullscreen} onClick={() => { editor.change({ ...editor.document, viewport: null }); setLayoutKey(value => value + 1); if (fullscreen) setPanel(null); setFullscreen(value => !value) }}>{s(fullscreen ? 'graph.exitFullscreen' : 'graph.fullscreen')}</button></div>
    </div>
    ...
    {editor.document.graph.nodes.some(node => node.kind === 'operator' && node.operator_id === 'rolling_apply') && <p className="mb-3 rounded-lg bg-accent-50 px-3 py-2 text-xs leading-5 text-accent-900">{s('rollingScope.graphHint')}</p>}
    ...
    <div className={`grid min-w-0 gap-3 ${panel ? 'lg:grid-cols-[minmax(0,1fr)_280px]' : ''}`}>
      ...
      </>} /> : <div className="flex min-h-[400px] items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white p-5 text-sm text-slate-600">{s(editor.busy ? 'graph.preparing' : 'graph.restoreHint')}</div>}</div>
