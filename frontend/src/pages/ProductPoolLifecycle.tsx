...
export default function ProductPoolLifecycle() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6" aria-busy={loading || comparing}>
    <header className="rounded-2xl bg-slate-900 px-5 py-6 text-white sm:px-7">
      ...
      <p className="mt-2 text-sm text-slate-300">已发布版本不可修改；产品变更通过新版本生效，并保留完整差异和评价方案来源。</p>
    </header>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 overflow-x-auto"><table className="min-w-[920px] w-full text-sm"><thead className="bg-slate-50 text-left text-xs text-slate-500"><tr><th className="px-3 py-3">产品池</th><th className="px-3 py-3">版本</th><th className="px-3 py-3">有效期</th><th className="px-3 py-3">评价方案</th><th className="px-3 py-3">可投资产品</th><th className="px-3 py-3">发布时间</th><th className="px-3 py-3">查看</th></tr></thead><tbody className="divide-y divide-slate-100">{versions.map((version) => <tr key={version.id} className={selectedId === version.id ? 'bg-emerald-50/50' : ''}><td className="px-3 py-3 font-medium text-slate-900">{version.pool_name}</td><td className="px-3 py-3">V{version.version}</td><td className="px-3 py-3">{version.effective_from} ～ {version.effective_to || '持续有效'}</td><td className="px-3 py-3">{version.evaluation_plans.length}</td><td className="px-3 py-3">{version.investable_count}</td><td className="px-3 py-3 text-xs text-slate-500">{version.created_at.slice(0, 19).replace('T', ' ')}</td><td className="px-3 py-3"><button type="button" onClick={() => { setSelectedId(version.id); setDiff(null) }} className="text-emerald-700 underline">查看</button></td></tr>)}</tbody></table>{!loading && versions.length === 0 && <p className="p-6 text-center text-sm text-slate-500">尚未发布产品池版本。</p>}</div>
    </section>

    {selected && <section className="grid gap-5 xl:grid-cols-[1fr_1fr]">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-semibold text-slate-900">{selected.pool_name} · V{selected.version}</h2><p className="mt-1 font-mono text-xs text-slate-400">{selected.id}</p></div><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">不可变</span></div>
        <dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-500">生效日</dt><dd className="mt-1 font-medium">{selected.effective_from}</dd></div><div className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-500">可投资产品</dt><dd className="mt-1 font-medium">{selected.investable_count}</dd></div><div className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-500">已准入</dt><dd className="mt-1 font-medium">{selected.member_counts.approved ?? 0}</dd></div><div className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-500">观察 / 排除</dt><dd className="mt-1 font-medium">{selected.member_counts.watch ?? 0} / {selected.member_counts.rejected ?? 0}</dd></div></dl>
        <h3 className="mt-5 text-sm font-semibold text-slate-800">评价方案即产品分组</h3><ul className="mt-2 space-y-2">{selected.evaluation_plans.map((plan) => <li key={`${plan.plan_id}:${plan.plan_revision}`} className="rounded-lg border border-slate-200 px-3 py-2 text-sm"><b>{plan.plan_name}</b><span className="ml-2 text-xs text-slate-500">v{plan.plan_revision} · {plan.imported_count} 个候选 · 截止 {plan.as_of || '—'}</span></li>)}</ul>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {!diff && <p className="mt-5 rounded-lg bg-slate-50 p-4 text-sm text-slate-500">选择历史版本后查看新增、移除和状态变化。</p>}
        ...
        {diff && <div className="mt-4 max-h-80 space-y-2 overflow-auto text-sm">{diff.added.map((item) => <p key={`add-${item.key}`} className="rounded bg-emerald-50 px-3 py-2 text-emerald-900">新增：{item.name || item.code}</p>)}{diff.removed.map((item) => <p key={`remove-${item.key}`} className="rounded bg-rose-50 px-3 py-2 text-rose-900">移除：{item.name || item.code}</p>)}{diff.changed.map((item) => <div key={`change-${item.key}`} className="rounded bg-amber-50 px-3 py-2 text-amber-950"><b>变更：{item.name || item.key}</b><p className="mt-1 text-xs">{Object.keys(item.changes).join('、')}</p></div>)}</div>}
      </div>
    </section>}
  </div>
}
