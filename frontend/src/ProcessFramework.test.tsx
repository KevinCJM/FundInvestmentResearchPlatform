...
describe('投研流程框架', () => {
  ...
  it('主页展示五阶段闭环和独立设置入口，不显示节点数量标签', () => {
    ...
    expect(screen.getByRole('heading', { name: '完整的投研流程' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '核心能力' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '精选功能' })).toBeInTheDocument()
    expect(screen.getByRole('region', { name: '完整的投研流程' }).querySelectorAll('a')).toHaveLength(5)
    expect(within(screen.getByRole('navigation', { name: '页脚导航' })).getByRole('link', { name: '设置' })).toHaveAttribute('href', '/settings')
    expect(screen.getByText(/组合登记和会计核算贯穿投中与投后/)).toBeInTheDocument()
