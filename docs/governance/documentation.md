...
候选隔离同时覆盖所复用的 Hermes 辅助代码：暂存检查从 index 加载，指定提交检查从该提交加载，工作区修复不能代替候选中的版本。辅助代码也纳入审核回执指纹，版本变化使旧回执失效；候选缺少辅助代码或加载失败时直接报错；三种检查模式均将辅助代码导入期间的 `SystemExit`（包括退出码 0）转为失败。

Git 候选模式也从对应 Git 内容加载检查器自身，在内存中执行该版本的校验入口；候选检查器缺失、语法或初始化失败时直接报错，不用工作区版本替代。候选初始化或入口通过 `SystemExit` 提前退出（包括退出码 0）同样按失败处理，避免未执行检查却报告成功。检查器代码同样绑定审核指纹。本地启动入口能够运行是执行前提。默认工作区范围合并结构化的暂存/未暂存差异和未跟踪路径，改名必须保留原路径与新路径；显式 `--changed-file` 仍只使用指定范围。

影响归属同时读取候选和比较基线的模块/文档目录，删除源码、测试、配置或撤销模块登记不能消除原契约的审核义务。工作区与暂存检查以 HEAD 为基线，PR 检查以 merge-base 为基线；首次提交前没有基线时只使用当前目录。审核指纹绑定基线提交，基线变化后须重新核对。计划表按同一规则去除行首尾空白及可选首尾竖线，合法的 1–3 空格缩进、表头/分隔行/内容行省略边界竖线不能绕过状态和证据检查；代码块中的示例仍不作为活动计划。
