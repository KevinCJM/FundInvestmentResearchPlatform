...
export default function ResearchResults({ run, onPublish, onBuildReturns }: { run: FactorRun; onPublish: () => void; onBuildReturns?: () => void }) {
  ...
  return <div className="space-y-5" aria-label="因子检验结果">
    <Card title={run.name + ' · 检验结果'}>
      ...
      ].map(([label, value]) => <div key={label} className="rounded-lg bg-slate-50 p-3"><p className="text-xs text-slate-600">{label}</p><p className="mt-2 text-xl font-bold tabular-nums text-slate-900">{value}</p></div>)}</div>
      <p className="mt-3 text-xs leading-5 text-slate-600">收益统计基于每日净值路径，费用已计入；累计费用比率之和 {percentText(performance.fee_sum)}。样本内检验剔除标签跨界的信号。ICIR 为逐期均值 ÷ 标准差，未年化。</p>
    </Card>
    <Card title="组合净值与比较基准">
      <p className="mb-3 text-xs text-slate-600">{run.study_snapshot.benchmark.label} · {run.study_snapshot.benchmark.code} · 图形展示完整研究区间</p>
      <ReactECharts option={curve} style={{ height: 320, width: '100%' }} notMerge />
    </Card>
    <div className="grid min-w-0 gap-5 xl:grid-cols-2">
      <Card title="因子检验"><div className="overflow-x-auto"><table className="w-full min-w-[440px] text-left text-sm" aria-label="因子检验统计"><thead className="text-xs text-slate-600"><tr><th scope="col" className="p-2">因子</th><th scope="col">IC</th><th scope="col">RankIC</th><th scope="col">ICIR</th><th scope="col">正值比例</th></tr></thead><tbody>{summary.factors.map(item => <tr className="border-t border-slate-100" key={item.factor_id}><th scope="row" className="p-2 font-medium">{item.name}</th><td>{numberText(item.ic.mean)}</td><td>{numberText(item.rank_ic.mean)}</td><td>{numberText(item.rank_ic.icir)}</td><td>{percentText(item.rank_ic.positive_rate)}</td></tr>)}</tbody></table></div></Card>
      <Card title="分组远期收益">
        ...
        <p className="text-xs leading-5 text-slate-600">Q1 为低分组，末组为高分组。展示 {run.study_snapshot.horizon} 日标签平均收益（小数）；这是因子检验，不是可直接连乘的组合净值。</p>
      </Card>
    </div>
    <RollingICResults key={run.id} run={run} sample={sample} />
    <Card title="因子相关性"><div className="overflow-x-auto"><table className="w-full min-w-[400px] text-left text-sm" aria-label="因子相关性"><thead><tr><th scope="col" className="p-2">因子</th>{factors.map(f => <th scope="col" key={f.id} className="p-2 font-medium">{f.name}</th>)}</tr></thead><tbody>{factors.map((factor, i) => <tr className="border-t border-slate-100" key={factor.id}><th scope="row" className="p-2 font-medium">{factor.name}</th>{summary.factor_correlation[i].map((value, j) => <td key={j} className="p-2 tabular-nums">{numberText(value)}</td>)}</tr>)}</tbody></table></div><p className="mt-2 text-xs text-slate-600">所选样本区间的标准化因子面板相关性，用于识别信息重复。</p></Card>
    <Card title="最新产品得分与因子贡献">
      <div className="overflow-x-auto"><table className="w-full min-w-[640px] text-left text-sm" aria-label="最新因子得分"><thead className="text-xs text-slate-600"><tr><th scope="col" className="p-2">排名 / 产品</th><th scope="col">组合分数</th>{factors.map(f => <th scope="col" className="p-2" key={f.id}>{f.name}<span className="block font-normal">原始值 / 分数贡献</span></th>)}</tr></thead><tbody>{run.latest_scores.map(row => <tr key={row.code} className="border-t border-slate-100"><th scope="row" className="p-2 font-medium"><span className="text-accent-600">{numberText(row.rank, 1)}</span> · <a className="hover:underline" href={'/product-research/products/' + encodeURIComponent(row.product_id) + '?kind=' + row.kind}>{row.name}</a><span className="block text-xs font-normal text-slate-600">{row.code}{row.exclusion_reason && ' · ' + row.exclusion_reason}</span></th><td className="font-semibold">{numberText(row.score, 2)}</td>{row.factors.map(f => <td className="p-2" key={f.factor_id}><span>{numberText(f.raw_value, 4)}</span><span className="block text-xs text-accent-600">{numberText(f.contribution, 2)}</span></td>)}</tr>)}</tbody></table></div>
      <p className="mt-3 text-xs text-slate-600">这是 {run.as_of} 的可用数据得分快照；最新分数尚没有完整远期标签。空值保持缺失。</p>
    </Card>
    <details className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold text-slate-800">逐期检验与数据证据</summary>
      <div className="mt-4 max-h-72 overflow-auto"><table className="w-full min-w-[560px] text-left text-xs"><thead><tr><th scope="col" className="p-2">信号日</th><th scope="col">模拟入场日</th><th scope="col">标签终点</th><th scope="col">样本</th><th scope="col">组合 RankIC</th><th scope="col">配对数</th></tr></thead><tbody>{run.periods.map(row => <tr key={row.date} className="border-t border-slate-100"><td className="p-2">{row.date}</td><td>{row.entry_date || '—'}</td><td>{row.label_end || '—'}</td><td>{row.sample === 'in_sample' ? '样本内' : row.sample === 'out_of_sample' ? '样本外' : '跨界剔除'}</td><td>{numberText(row.rank_ic[row.rank_ic.length - 1])}</td><td>{row.pair_counts[row.pair_counts.length - 1]}</td></tr>)}</tbody></table></div>
      <p className="mt-3 break-all text-xs text-slate-600">运行：{run.id}<br />输入哈希：{run.input_checksum}<br />数据快照：{run.data_lineage.snapshot}<br />计算版本：{run.engine_version}</p>
      <pre className="mt-3 max-h-48 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-slate-50 p-3 text-xs">{JSON.stringify({ quality: run.data_quality, lineage: run.data_lineage, execution: run.execution }, null, 2)}</pre>
    </details>
