...
export default function EtlStepProgress({ step, now }: { step: Step; now: number }) {
  ...
  return <section aria-label={`${step.name}实时进度`} className="mt-3 min-w-0 space-y-2 rounded-lg border border-accent-100 bg-white/80 p-3 text-xs text-slate-700">
    ...
    {active ? <div role="progressbar" aria-label={`${step.name}当前阶段进度`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent ?? undefined} aria-valuetext={percent === null ? '总量尚未确定，正在执行' : `当前阶段 ${percent}%`} className="h-2 overflow-hidden rounded-full bg-accent-100">
      <div className={`h-full rounded-full bg-accent-500 ${percent === null ? 'w-1/3 motion-safe:animate-pulse' : 'transition-[width]'}`} style={percent === null ? undefined : { width: `${percent}%` }} />
    </div> : null}
    <p className="break-words">{p?.message || '当前执行器尚未上报详细进度；页面会持续查询，日志不可见不代表任务停止。'}</p>
    {p?.event_coverage ? <p className="rounded-lg bg-accent-50 p-2">本次查询 {p.event_coverage.queried_days} 天 · 复用已查区间 {p.event_coverage.reused_days} 天 · 修订复核 {p.event_coverage.revision_days} 天 · 复用检查点 {p.event_coverage.checkpoints} 个 · 实际请求 {p.event_coverage.requests} 次</p> : null}
    {p?.received_rows !== undefined ? <p className="text-slate-600">接收数量包含历史重叠、分页和复核；新增/修订行数未经逐键比较时不作推断。</p> : null}
    ...
    {active && percent !== null ? <p className="text-slate-600">百分比仅代表当前阶段，不代表整个节点；之后仍可能合并、校验文件。</p> : null}
    {active && quiet !== null && quiet >= 60 ? <p className="rounded-lg bg-amber-50 p-2 text-amber-800">已有 {duration(quiet)} 未收到新进度。可能在等待接口、限频或处理本地文件，不能据此判定卡死。</p> : null}
    {p?.logs?.length ? <details><summary className="cursor-pointer text-accent-700">最近日志（{p.logs.length} 条，已脱敏）</summary>
      <ol aria-label="最近执行日志" className="mt-2 max-h-48 space-y-1 overflow-y-auto rounded-lg bg-slate-900 p-3 font-mono text-slate-100">{p.logs.map((line, i) => <li key={`${line.at}-${i}`} className="whitespace-pre-wrap break-all"><time className="mr-2 text-slate-400">{new Date(line.at).toLocaleTimeString('zh-CN')}</time>{line.message}</li>)}</ol>
    </details> : null}
  </section>
}
