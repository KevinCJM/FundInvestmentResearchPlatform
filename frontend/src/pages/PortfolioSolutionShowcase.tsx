...
function MetricCard({ label, value, hint }: { label: string; value: string; hint: string }) {
  return <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-xl font-bold text-slate-950">{value}</p><p className="mt-1 text-xs text-slate-400">{hint}</p></article>
}
  ...
  return (
    <figure className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-3 text-xs"><span className="text-fuchsia-700">● 组合</span><span className="text-slate-500">● 基准</span></div>
      </div>
      ...
      <p className="mt-2 text-xs leading-5 text-slate-500">虚线左侧为历史回测，右侧为实盘跟踪示例。两段数据必须分别标识，不拼接为未经说明的连续实盘业绩。</p>
    </figure>
  )
}
...
function ProfileView({ solution }: { solution: (typeof portfolioSolutions)[number] }) {
  return <div className="space-y-5">
    ...
    <section className="border-y border-slate-200 bg-slate-50/60 px-4 py-4" aria-label="组合适用性说明（非交互）">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">组合说明 · 非交互</p>
...
function PerformanceView({ solution }: { solution: (typeof portfolioSolutions)[number] }) {
  ...
  return <div className="space-y-5">
    ...
    <PerformanceLineChart />
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><table className="w-full min-w-[640px] text-sm"><thead className="bg-slate-50 text-left text-xs text-slate-500"><tr>{['区间', '组合收益', '基准收益', '相对收益'].map((item) => <th key={item} className="px-4 py-3">{item}</th>)}</tr></thead><tbody>{periodPerformance.map((row) => <tr key={row[0]} className="border-t border-slate-100">{row.map((cell) => <td key={cell} className="px-4 py-3 text-slate-700">{cell}</td>)}</tr>)}</tbody></table></section>
  </div>
}
...
function BacktestView() {
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="font-semibold text-slate-900">冻结回测口径</h3><dl className="mt-4 grid gap-3 text-sm">{[
      ...
      ].map(([term, value]) => <div key={term} className="grid grid-cols-[100px_1fr] gap-3"><dt className="text-slate-500">{term}</dt><dd className="font-medium text-slate-800">{value}</dd></div>)}</dl></section>
...
function ScenarioView() {
  ...
  return <div className="space-y-5">
    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{scenarioResults.map((scenario) => <button type="button" key={scenario.id} onClick={() => setSelectedId(scenario.id)} className={`rounded-xl border p-4 text-left transition ${selectedId === scenario.id ? 'border-fuchsia-400 bg-fuchsia-50 ring-2 ring-fuchsia-100' : 'border-slate-200 bg-white hover:border-fuchsia-200'}`}><span className="text-sm font-semibold text-slate-900">{scenario.name}</span><span className="mt-1 block text-xs leading-5 text-slate-500">{scenario.description}</span></button>)}</section>
    <section className="rounded-2xl bg-slate-950 p-6 text-white"><div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-fuchsia-300">Selected scenario</p><h3 className="mt-2 text-2xl font-bold">{selected.name}</h3><p className="mt-2 text-sm text-slate-300">{selected.description}</p></div><span className="rounded-full bg-white/10 px-3 py-1 text-xs">{selected.confidence}</span></div><div className="mt-6 grid gap-4 sm:grid-cols-3">{[['组合模拟', selected.portfolio], ['基准模拟', selected.benchmark], ['恢复时间', selected.recovery]].map(([label, value]) => <div key={label} className="rounded-xl bg-white/[0.07] p-4"><p className="text-xs text-slate-400">{label}</p><p className="mt-1 text-xl font-bold">{value}</p></div>)}</div></section>
    <section className="grid gap-5 xl:grid-cols-2"><article className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="font-semibold text-slate-900">周期矩阵</h3><div className="mt-4 grid grid-cols-2 gap-3">{[['衰退', '-4.8%'], ['滞胀', '-3.6%'], ['复苏', '+9.3%'], ['扩张', '+7.1%']].map(([label, value]) => <div key={label} className="rounded-xl bg-slate-50 p-4"><p className="text-sm text-slate-500">{label}</p><p className="mt-1 font-bold text-slate-900">{value}</p></div>)}</div></article><aside className="border-l-2 border-slate-300 bg-slate-50/70 px-4 py-4" role="note" aria-label="情景口径说明（非交互）"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">非交互说明</p><h3 className="mt-1 font-semibold text-slate-800">口径边界</h3><ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-slate-600"><li>情景由“设置 / 情景算法中心”的冻结版本计算。</li><li>历史情景、因子冲击和周期模型必须区分展示。</li><li>结果是条件模拟，不是概率预测，也不是收益承诺。</li></ul></aside></section>
  </div>
}

function RulesView({ solution }: { solution: (typeof portfolioSolutions)[number] }) {
  return <div className="space-y-5">
    <section className="border-y border-slate-200 bg-slate-50/60 px-4 py-4" aria-label="组合规则链说明（非交互）"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">规则链说明 · 非交互</p><ol className="mt-3 flex flex-col gap-2 text-sm text-slate-700 lg:flex-row lg:flex-wrap lg:items-center">{['目标与约束', 'SAA 配置中枢', 'TAA 状态偏离', '产品选择与替代', '再平衡与风控'].map((item, index) => <li key={item} className="flex items-center gap-2"><span className="font-mono text-xs text-fuchsia-700">0{index + 1}</span><span className="font-medium">{item}</span>{index < 4 ? <span className="hidden text-slate-300 lg:inline" aria-hidden="true">→</span> : null}</li>)}</ol></section>
    <div className="grid gap-5 xl:grid-cols-[0.7fr_1.3fr]"><section className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="font-semibold text-slate-900">资产配置边界</h3><div className="mt-4 space-y-4">{solution.allocation.map((item) => <div key={item.label}><div className="flex justify-between text-sm"><span>{item.label}</span><span className="font-semibold">中枢 {item.value}% · 区间 ±5%</span></div><div className="mt-1 h-2 rounded-full bg-slate-100"><div className={`h-2 rounded-full ${item.color}`} style={{ width: `${item.value}%` }} /></div></div>)}</div></section><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><table className="w-full min-w-[720px] text-sm"><thead className="bg-slate-50 text-left text-xs text-slate-500"><tr>{['模块', '方法', '运行频率', '版本'].map((item) => <th key={item} className="px-4 py-3">{item}</th>)}</tr></thead><tbody>{algorithmRules.map((row) => <tr key={row[0]} className="border-t border-slate-100">{row.map((cell) => <td key={cell} className="px-4 py-3 text-slate-700">{cell}</td>)}</tr>)}</tbody></table></section></div>
  </div>
}
...
function PublishingView() {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]"><section className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="font-semibold text-slate-900">展示前披露检查</h3><ul className="mt-4 space-y-3">{disclosureItems.map((item) => <li key={item} className="flex gap-3 rounded-xl bg-emerald-50 px-4 py-3 text-sm leading-6 text-emerald-950"><span aria-hidden="true" className="font-bold">✓</span><span>{item}</span></li>)}</ul></section><section className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="font-semibold text-slate-900">版本链</h3><dl className="mt-4 space-y-3 text-sm">{[['研究方案', 'PF-RESEARCH-R18'], ['产品池', 'POOL-FOF-R12'], ['回测档案', 'BT-STRICT-20260831'], ['情景算法', 'SCENARIO-R4'], ['指标方案', 'METRIC-R9'], ['展示快照', 'SHOWCASE-V3.2']].map(([term, value]) => <div key={term} className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-500">{term}</dt><dd className="mt-1 font-mono text-xs font-semibold text-slate-800">{value}</dd></div>)}</dl><button type="button" onClick={() => setNotice('已生成当前会话内的静态预览；未发布、未保存。')} className="mt-5 w-full rounded-lg bg-fuchsia-700 px-4 py-2 text-sm font-semibold text-white hover:bg-fuchsia-600">生成静态预览</button>{notice ? <p className="mt-3 rounded-lg bg-fuchsia-50 p-3 text-sm text-fuchsia-800" aria-live="polite">{notice}</p> : null}</section></div>
  </div>
}
...
export default function PortfolioSolutionShowcase({ view }: { view: ShowcaseView }) {
  ...
  return (
    <div className="space-y-5" data-testid={`portfolio-solution-${view}`}>
      <StaticDemoBanner />
      <header className="rounded-2xl bg-gradient-to-r from-fuchsia-900 via-violet-800 to-indigo-800 p-6 text-white shadow-sm">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-fuchsia-200">Published solution prototype</p><h2 className="mt-2 text-2xl font-bold">{page.title}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-white/80">{page.description}</p></div><label className="text-xs font-semibold text-fuchsia-100">展示组合<select value={solution.id} onChange={(event) => updateSolution(event.target.value)} className="mt-1 block w-full min-w-64 rounded-lg border border-white/20 bg-white px-3 py-2 text-sm text-slate-900">{portfolioSolutions.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.version}</option>)}</select></label></div>
      </header>

      <section className="rounded-2xl border border-fuchsia-200 bg-fuchsia-50 p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap gap-2 text-xs font-semibold"><span className="rounded-full bg-fuchsia-200 px-2.5 py-1 text-fuchsia-900">{solution.riskLevel}</span><span className="rounded-full bg-white px-2.5 py-1 text-slate-600">{solution.horizon}</span><span className="rounded-full bg-white px-2.5 py-1 text-slate-600">{solution.status}</span></div><h3 className="mt-3 text-xl font-bold text-slate-950">{solution.name} · {solution.version}</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{solution.subtitle}</p></div><Link to="/portfolio-solutions/catalog" className="shrink-0 text-sm font-semibold text-fuchsia-800">返回方案目录 →</Link></div><p className="mt-4 border-t border-fuchsia-200 pt-3 text-xs leading-5 text-slate-600"><span className="font-semibold">业绩比较基准：</span>{solution.benchmark}</p></section>
