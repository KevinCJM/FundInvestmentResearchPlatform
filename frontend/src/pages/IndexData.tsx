...
function CheckCard({ check }: { check: QualityCheck }) {
  return (
    <article className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-slate-600">{dimensionLabels[check.dimension] ?? check.dimension}</p><h3 className="mt-1 font-semibold text-slate-900">{check.label}</h3></div><StatusBadge tone={checkTone(check.status)} /></div>
      ...
      {check.threshold && <p className="mt-3 border-t border-slate-200 pt-3 text-xs leading-5 text-slate-600"><span className="font-semibold text-slate-700">规则：</span>{check.threshold}</p>}
    </article>
  );
}
    ...
    <li className={`border-l-4 p-4 sm:p-5 ${style.border}`}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${style.badge}`}>{style.label}</span><span className="rounded-lg bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">{dimensionLabels[issue.dimension] ?? issue.dimension}</span><span className="text-xs text-slate-600">{issue.scope}</span></div>
        ...
        {issue.actionTo ? <Link to={issue.actionTo} className="shrink-0 self-start text-sm font-semibold text-accent-700 hover:text-accent-900">{issue.actionLabel} →</Link> : <button type="button" onClick={onReload} className="shrink-0 self-start text-sm font-semibold text-accent-700 hover:text-accent-900 focus:outline-none focus:ring-2 focus:ring-accent-500">重新检查</button>}
      </div>
      ...
      {issue.samples.length > 0 && <div className="mt-3"><p className="text-xs font-semibold text-slate-600">异常样本（最多展示 5 个）</p><ul className="mt-2 flex flex-wrap gap-2">{issue.samples.map((sample) => <li key={`${issue.id}-${sample.kind}-${sample.ts_code}`} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs"><span className="font-semibold text-slate-800">{sample.name || sample.ts_code}</span>{sample.name && <span className="ml-1 text-slate-600">{sample.ts_code}</span>}<span className="mt-0.5 block text-slate-600">{[sample.observed, sample.latest_date].filter(Boolean).join(' · ')}</span></li>)}</ul></div>}
    </li>
  );
}
...
export default function DataQuality() {
  ...
  return (
    <div className="mx-auto min-w-0 max-w-7xl space-y-6">
      ...
      <section aria-labelledby="quality-checks-title" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h2 id="quality-checks-title" className="text-lg font-semibold text-slate-950">深度检查矩阵</h2><p className="mt-1 text-sm text-slate-600">每项结果均来自当前已生效数据版本；未检查项不会被算作通过。</p></div><span className="text-xs text-slate-600">{deepQuality?.validation.status === 'passed' ? '全量验收记录已绑定' : '缺少全量验收记录'}</span></div>
      ...
      <section aria-labelledby="quality-issues-title" className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 p-5 sm:p-6 lg:flex-row lg:items-end lg:justify-between"><div><h2 id="quality-issues-title" className="text-lg font-semibold text-slate-950">问题工作台</h2><p className="mt-1 text-sm text-slate-600">按严重级别处理；每条问题都说明证据、研究影响和异常样本。</p></div><div className="flex flex-wrap gap-2" aria-label="问题风险筛选">{([['all', '全部问题'], ['critical', '阻断'], ['high', '高风险'], ['medium', '关注']] as const).map(([value, label]) => { const count = value === 'all' ? issues.length : severityCounts[value]; const selected = severityFilter === value; return <button key={value} type="button" aria-pressed={selected} onClick={() => setSeverityFilter(value)} className={`min-h-10 rounded-lg border px-3 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-accent-500 ${selected ? 'border-accent-300 bg-accent-50 text-accent-800' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}>{label} <span className="tabular-nums">{count}</span></button>; })}</div></div>
