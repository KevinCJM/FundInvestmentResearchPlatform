...
## PR 43 审核修复：忙碌验证不产生冻结版本（2026-09-20）

原验证先追加 `candidate_frozen` 再获取计算名额；忙碌错误会留下多余版本，使原 revision 失效。新增回归在修复前失败。现共用 `compute_slot` 上下文，预览、优化和验证统一释放名额；验证取得名额后才冻结。忙碌路径不改变研究包、历史或运行尝试，原 revision/key 直接重试并保持幂等；冻结写入失败也释放名额。已真正开始计算后的失败仍保留原审计尝试及冻结候选。

首轮实施/共同配置两模块 49 项通过，真实 Chrome 实施完整流程 3 项通过（1440/768/320）；扩展后端相关回归 641 项通过（146.12 秒，非全仓库 pytest），多 CMA 浏览器另有 12 项通过；路由 validate/evolve 和 diff 检查通过。未改变产品风险、费用或资金算法，未迁移历史研究包。
