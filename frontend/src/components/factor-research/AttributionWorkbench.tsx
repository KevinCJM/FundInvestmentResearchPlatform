      ...
      <form onSubmit={event => { event.preventDefault(); setResult(undefined); void action('运行收益归因', async () => { const value = await factorApi.attribution(request); setResult(value); setHistory((await factorApi.attributions()).items); return value }) }}>
        <fieldset disabled={busy} className="space-y-4" onChange={() => setResult(undefined)}>
          ...
          {model === 'rbsa' ? <Field label="风格代理指数" hint="2–8个本地指数代码。优先选择覆盖充分、风格区分清楚的代理。"><input className={inputClass} required value={indices} onChange={e => setIndices(e.target.value)} /></Field> : <>
            ...
            {selectedDataset && <p className="rounded-lg bg-indigo-50 p-3 text-sm leading-6 text-indigo-900">因子列：{(selectedDataset.factor_names || ['MKT_RF', 'SMB', 'HML']).join('、')}。{selectedDataset.dependent_return === 'total' ? '本次解释产品总收益，没有 RF；截距不是风险调整 Alpha。' : '本次解释产品扣除 RF 后的超额收益。'} 数据覆盖 {selectedDataset.start_date} — {selectedDataset.end_date}。</p>}
            <p className="text-xs leading-6 text-slate-500">没有合适数据时，先在“构建工作台”生成，或在 <Link className="text-indigo-700 underline" to="?module=returns&tab=datasets">收益率数据集</Link> 导入；ETF 动量差额不能作为 FF3 使用。</p>
          </>}
          ...
          {exposureMode === 'rolling' ? <div className="rounded-lg border border-indigo-200 bg-indigo-50/30 p-4">
          ...
          </div> : <p className="text-xs leading-6 text-slate-500">样本内是一组事后拟合暴露，不代表当时已经知道；样本外使用固定系数。两段贡献分别独立对账。</p>}
