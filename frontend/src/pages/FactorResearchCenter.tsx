...
export default function FactorResearchCenter() {
  ...
  return <div className="min-w-0 space-y-5" data-testid="factor-research-center">
    <header className="rounded-xl border border-slate-200 bg-white p-5 sm:p-6">
      ...
      <div role="tablist" aria-label="因子研究模块" className="mt-5 grid gap-3 md:grid-cols-2">{modules.map((item, index) => <button key={item.id} type="button" role="tab" id={'factor-module-' + item.id} aria-label={item.name} aria-selected={module === item.id} aria-controls="factor-module-panel" tabIndex={module === item.id ? 0 : -1} disabled={Boolean(busy)} onKeyDown={event => tabKeyDown(event, index)} onClick={() => navigate(item.id, item.id === 'returns' ? 'construct' : 'workbench')} className={'min-h-24 rounded-xl border p-4 text-left transition focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 ' + (module === item.id ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50')}><span className="block text-base font-bold">{item.name}</span><span className={'mt-2 block text-xs leading-5 ' + (module === item.id ? 'text-slate-300' : 'text-slate-500')}>{item.description}</span></button>)}</div>
    </header>
    {busy && <p className="rounded-lg border border-accent-200 bg-accent-50 p-3 text-sm text-accent-900" role="status">{busy}…{busy.includes('运行') ? '正在校验数据并计算，请保留此页面。' : ''}</p>}
    ...
    <section id="factor-module-panel" role="tabpanel" aria-labelledby={'factor-module-' + module} className="min-w-0 space-y-5">
      ...
      {catalog && <div id="factor-tab-panel" role="tabpanel" aria-labelledby={'factor-tab-' + tab} className="min-w-0">
        ...
        {tab === 'workbench' && <div className="space-y-5">
          ...
          {run ? <ResearchResults run={run} onPublish={() => navigate('characteristics', 'releases')} onBuildReturns={() => navigate('returns', 'construct', { source: run.id })} /> : <EmptyState title="尚未运行检验" hint="设置研究对象、比较基准和因子权重后，点击“保存并运行检验”。" />}
        </div>}
