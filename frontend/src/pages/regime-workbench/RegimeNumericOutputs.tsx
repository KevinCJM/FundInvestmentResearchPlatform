...
export default function RegimeNumericOutputs({ result }: { result: RegimeResultData }) {
  ...
  return <details className="rounded-xl border border-slate-200 bg-white p-4" aria-label="数值输出通道"><summary className="cursor-pointer text-sm font-semibold text-slate-800">数值输出 · {entries.length} 个通道</summary>
    <div className="mt-3 flex flex-wrap gap-2" role="tablist" aria-label="数值结果通道">{entries.map(([key, item]) => <button type="button" key={key} role="tab" aria-selected={key === id} onClick={() => setSelected(key)} className={`min-h-10 rounded-lg border px-3 text-xs ${key === id ? 'border-violet-400 bg-violet-50 text-violet-800' : 'border-slate-200'}`}>{item.label}</button>)}</div>
