...
function LabHeader({ embedded, onClose }: Pick<ResearchDataLabProps, 'embedded' | 'onClose'>) {
  return (
    <header className="flex flex-col gap-3 border-b border-slate-200 bg-white px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-accent-600">Research data lab</p>
        <h2 className="mt-1 text-xl font-bold text-slate-950">研究数据实验室</h2>
        <p className="mt-1 text-xs leading-5 text-slate-600">只展示接口返回的真实数据画像；指数、宏观和指标均保留时点与版本信息。</p>
      </div>
      {embedded && onClose ? <button type="button" onClick={onClose} className="min-h-10 rounded-xl border border-slate-300 px-4 text-sm font-bold text-slate-700 hover:border-accent-300 hover:text-accent-700">返回计算图</button> : null}
    </header>
  )
}
...
function CatalogCard({
  ...
  return (
    <button
      ...
      className={`w-full rounded-xl border p-3 text-left transition focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 ${selected ? 'border-accent-500 bg-accent-50 shadow-sm' : 'border-slate-200 bg-white hover:border-accent-200'}`}
    >
      <span className="flex items-start justify-between gap-2">
        <span className="min-w-0">
          <span className="block truncate text-sm font-bold text-slate-900">{item.name}</span>
          <span className="mt-0.5 block truncate text-xs text-slate-600">{item.code || item.id}</span>
        </span>
        <span className={`shrink-0 rounded-full px-2 py-1 text-xs font-bold ${available ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>{available ? '可研究' : '尚未下载'}</span>
      </span>
      <span className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600">
...
function UploadPanel({
  ...
  return <section className="mt-4 rounded-xl border border-accent-200 bg-accent-50/60 p-3" aria-label="上传研究时序">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h4 className="text-sm font-bold text-accent-950">上传 CSV / JSON 时序</h4><p className="mt-1 text-xs leading-5 text-accent-900">必填 date、value；可选 available_at、vintage、revision。前端先校验，后端仍执行最终校验与完整样本 NJIT 画像。</p></div><label className="inline-flex min-h-10 cursor-pointer items-center justify-center rounded-lg bg-accent-600 px-3 text-xs font-bold text-white">{parsing ? '解析中…' : '选择文件'}<input aria-label="选择研究时序文件" type="file" accept=".csv,.json,text/csv,application/json" disabled={parsing} onChange={(event) => void handleFile(event.target.files?.[0])} className="sr-only" /></label></div>
    <div className="mt-3 grid gap-3 sm:grid-cols-3">
      <label className="text-xs font-bold text-slate-600">序列名称<input aria-label="上传序列名称" value={name} onChange={(event) => onName(event.target.value)} maxLength={160} className="mt-1 min-h-10 w-full rounded-xl border border-slate-300 bg-white px-2 font-normal" /></label>
      <label className="text-xs font-bold text-slate-600">频率<select aria-label="上传序列频率" value={frequency} onChange={(event) => onFrequency(event.target.value as UploadFrequency)} className="mt-1 min-h-10 w-full rounded-xl border border-slate-300 bg-white px-2 font-normal"><option value="daily">日频</option><option value="weekly">周频</option><option value="monthly">月频</option><option value="quarterly">季频</option><option value="annual">年频</option><option value="irregular">不定期</option></select></label>
      <label className="text-xs font-bold text-slate-600">可得性口径<select aria-label="上传可得性口径" value={availabilityMode} onChange={(event) => onAvailabilityMode(event.target.value as AvailabilityMode)} className="mt-1 min-h-10 w-full rounded-xl border border-slate-300 bg-white px-2 font-normal"><option value="point_in_time">按当时可得</option><option value="latest">使用最新修订</option></select></label>
    </div>
    {rows.length ? <div className="mt-3 rounded-lg border border-emerald-200 bg-white p-3"><p role="status" className="text-xs font-bold text-emerald-800">已解析 {rows.length.toLocaleString('zh-CN')} 行 · {fileName}</p><div className="mt-2 overflow-x-auto"><table className="min-w-full text-left text-xs" aria-label="上传数据预览"><thead className="text-slate-500"><tr><th scope="col" className="pr-4">date</th><th scope="col" className="pr-4">value</th><th scope="col" className="pr-4">available_at</th><th scope="col">vintage / revision</th></tr></thead><tbody>{rows.slice(0, 3).map((row, index) => <tr key={`${row.date || row.observation_date}-${index}`} className="border-t border-slate-100"><td className="py-1 pr-4">{row.date || row.observation_date}</td><td className="pr-4">{row.value == null ? 'null' : row.value}</td><td className="pr-4">{row.available_at || '同 date'}</td><td>{row.vintage || '—'} / {row.revision || 1}</td></tr>)}</tbody></table></div></div> : <p className="mt-3 rounded-lg border border-dashed border-accent-200 bg-white/70 px-3 py-2 text-xs text-slate-600">单次最多 20,000 行；文件只随本次画像请求传输，不会在浏览器伪造数据或宣称已经持久化。</p>}
  </section>
}
...
function ProfileSummary({ profile }: { profile: ResearchSeriesProfile }) {
  ...
  return <dl className="grid grid-cols-2 gap-2 lg:grid-cols-4">{stats.map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><dt className="text-xs font-semibold text-slate-600">{label}</dt><dd className="mt-1 text-sm font-bold text-slate-900">{value}</dd></div>)}</dl>
}
...
function ProfileTable({ profile }: { profile: ResearchSeriesProfile }) {
  ...
  return (
    <section aria-label="研究序列数据表" className="rounded-xl border border-slate-200 bg-white">
      <div className="border-b border-slate-200 px-4 py-3"><h4 className="text-sm font-bold text-slate-900">抽样数据表</h4><p className="mt-1 text-xs text-slate-600">统计使用完整样本，表格仅展示后端返回的等距抽样点。</p></div>
      <div className="space-y-2 p-3 md:hidden" data-testid="research-series-mobile-list">
        {rows.slice(0, 80).map((row) => <article key={row.date} className="rounded-lg bg-slate-50 p-3"><p className="text-xs font-bold text-slate-800">{row.date}</p><dl className="mt-2 grid grid-cols-2 gap-2">{columns.map((metric, index) => <div key={metric}><dt className="truncate text-xs text-slate-600">{metric}</dt><dd className="text-xs font-semibold text-slate-800">{typeof row.values[index] === 'number' || row.values[index] == null ? compactNumber(row.values[index] as number | null) : String(row.values[index])}</dd></div>)}</dl></article>)}
      </div>
      <div className="hidden max-h-80 overflow-auto md:block">
        <table className="min-w-full text-left text-xs" aria-label="研究序列抽样数据">
          <thead className="sticky top-0 bg-slate-50 text-slate-600"><tr><th scope="col" className="px-3 py-2 font-semibold">日期</th>{columns.map((metric) => <th scope="col" key={metric} className="px-3 py-2 text-right font-semibold">{metric}</th>)}</tr></thead>
...
function MultiSeriesComparison({
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4" aria-label="多序列研究比较">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="text-sm font-bold text-slate-950">多序列研究比较</h3><p className="mt-1 text-xs leading-5 text-slate-500">选择 2–4 条序列。共同日期、标准化、相关性与散点样本全部由后端固定签名 NJIT 计算。</p></div><button type="button" disabled={selections.length < 2 || loading} onClick={onRun} className="min-h-10 rounded-xl bg-slate-950 px-4 text-xs font-bold text-white disabled:opacity-40">{loading ? '正在比较…' : `比较 ${selections.length} 条序列`}</button></div>
    <div className="mt-3 flex flex-wrap gap-2">{selections.map((item) => <span key={item.key} className="inline-flex items-center gap-2 rounded-full bg-accent-50 px-3 py-1.5 text-xs font-bold text-accent-900">{item.label}<button type="button" aria-label={`移除对比${item.label}`} onClick={() => onRemove(item.key)} className="text-accent-600 hover:text-rose-600">×</button></span>)}{!selections.length ? <span className="rounded-lg border border-dashed border-slate-300 px-3 py-2 text-xs text-slate-600">从上方当前数据资源加入对比。</span> : null}</div>
    {comparison ? <div className="mt-4 space-y-4">
      <dl className="grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-600">共同日期</dt><dd className="mt-1 text-sm font-bold text-slate-900">{comparison.alignment.intersected_observations}</dd></div><div className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-600">全部有效</dt><dd className="mt-1 text-sm font-bold text-slate-900">{comparison.common_valid.observation_count}</dd></div><div className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-600">共同有效起点</dt><dd className="mt-1 text-xs font-bold text-slate-900">{comparison.common_valid.start_date || '—'}</dd></div><div className="rounded-lg bg-slate-50 p-3"><dt className="text-xs text-slate-600">共同有效终点</dt><dd className="mt-1 text-xs font-bold text-slate-900">{comparison.common_valid.end_date || '—'}</dd></div></dl>
      ...
      <div className="grid min-h-0 gap-4 p-3 sm:p-4 xl:grid-cols-[minmax(280px,0.72fr)_minmax(0,1.28fr)]">
        <aside className="min-w-0 space-y-3 rounded-xl border border-slate-200 bg-white p-3" aria-label="研究数据目录">
          <label className="block text-xs font-bold text-slate-600">搜索数据
            <input aria-label="搜索研究数据" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="指数代码、宏观指标或自定义指标" className="mt-1 min-h-10 w-full rounded-lg border border-slate-300 px-3 text-sm font-normal focus:border-accent-500 focus:outline-none" />
          </label>
          ...
          <label className="block text-xs font-bold text-slate-600">下载状态 <select aria-label="下载状态" value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)} className="ml-2 min-h-9 rounded-xl border border-slate-300 bg-white px-2 font-normal"><option value="available">仅可用</option><option value="not_downloaded">尚未下载</option><option value="all">全部</option></select></label>
          <div className="flex items-center justify-between text-xs text-slate-600"><span>{loading ? '正在读取目录…' : `${catalog?.total ?? 0} 条数据资源`}</span><span>{catalog?.snapshot?.generation || catalog?.snapshot?.directory || catalog?.snapshot?.id || '快照待确认'}</span></div>
          <div className="max-h-[58vh] space-y-2 overflow-y-auto pr-1 xl:max-h-[680px]">{catalog?.items.map((item) => <CatalogCard key={item.id} item={item} selected={item.id === selectedId} onSelect={() => setSelectedId(item.id)} />)}{!loading && !catalog?.items.length ? <p className="rounded-xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-600">没有匹配的数据资源。</p> : null}</div>
        </aside>

        <main className="min-w-0 space-y-4">
          {selected ? <section className="rounded-xl border border-slate-200 bg-white p-4" aria-label="数据画像参数">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-bold text-accent-600">{selected.category_label || selected.kind}</p><h3 className="mt-1 text-lg font-bold text-slate-950">{selected.name}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{selected.description || `${selected.source_api || selected.dataset || '数据快照'} · ${selected.frequency || '频率未知'}`}</p><div className="mt-2 flex flex-wrap gap-1 text-xs font-bold"><span className={`rounded-full px-2 py-1 ${selected.pit?.supported ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>{selected.pit?.supported ? '支持时点约束' : '无时点元数据'}</span><span className={`rounded-full px-2 py-1 ${selected.vintage?.supported ? 'bg-accent-100 text-accent-800' : 'bg-slate-100 text-slate-600'}`}>{selected.vintage?.supported ? '支持数据版本' : '单版本序列'}</span></div></div><div className="flex flex-wrap gap-2"><button type="button" disabled={!canAddComparison || comparisonSelections.some((item) => item.key === comparisonKey)} onClick={addComparison} className="min-h-10 rounded-xl border border-slate-300 px-4 text-sm font-bold text-slate-700 disabled:opacity-40">加入多序列对比</button><button type="button" disabled={!canBind || isBound} onClick={() => { if (bindCandidate && onBindSeries) { onBindSeries(bindCandidate); setLocallyBound((current) => [...current, bindingIdentity]) } }} className="min-h-10 rounded-xl border border-accent-200 px-4 text-sm font-bold text-accent-700 disabled:cursor-not-allowed disabled:opacity-40">{isBound ? '已加入计算图' : '加入计算图'}</button><button type="button" disabled={profiling || selected.status === 'not_downloaded' || (isUpload && uploadParsing)} onClick={() => void requestProfile()} className="min-h-10 rounded-xl bg-accent-600 px-4 text-sm font-bold text-white disabled:opacity-40">{profiling ? '正在计算画像…' : '计算数据画像'}</button></div></div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 2xl:grid-cols-6">
              <label className="text-xs font-bold text-slate-600">字段 <select aria-label="研究字段" value={field} onChange={(event) => setField(event.target.value)} className="mt-1 min-h-10 w-full rounded-xl border border-slate-300 bg-white px-2 font-normal">{(selected.fields || []).map((item) => <option key={fieldId(item)} value={fieldId(item)}>{fieldLabel(item)}</option>)}</select></label>
          ...
          </section> : <section className="grid min-h-64 place-items-center rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-600">请从左侧选择一个真实数据资源。</section>}
          ...
          {!profile && !profiling && selected ? <section className="grid min-h-60 place-items-center rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center"><div><p className="font-bold text-slate-800">尚未计算数据画像</p><p className="mt-2 text-sm text-slate-600">设置研究区间后点击“计算数据画像”；这里不会生成示例走势。</p></div></section> : null}
        </main>
      </div>
    </div>
  )
