...
export default function PitSnapshots() {
  ...
  const onApply = (releaseId: string | null, mode: RunMode) => {
    setApplying(true)
    setApplyMessage('')
    applyPitSettings({ activeReleaseId: releaseId, runMode: mode })
      .then((payload) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-600">
            每张表能证明什么、公告滞后多久、在严格 PIT 模式下是否可用。PIT 口径是<strong>系统级</strong>设置，在本页选定后全平台默认按同一版本展示；各功能页可在顶栏临时切换只影响自己的查看口径。
          </p>
        </div>
        <button
          type="button"
      ...
      {/* Applying口径 comes first: it is the one thing on this page that changes
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-black/5 px-3 py-2.5">
          <div>
            <h3 className="text-sm font-semibold text-slate-900">应用口径（系统级）</h3>
          ...
          {!pitSettings?.can_apply && (
            <p className="max-w-md text-xs text-slate-700">
              还没有任何数据版本，系统默认<strong>无 PIT 口径</strong>。先在下方封一个基线版本，才能应用。
            </p>
          )}
        </div>
        <div className="flex flex-wrap items-end gap-2 px-3 py-2.5">
          <label className="text-xs">
            <span className="mb-1 block font-medium text-slate-600">数据版本</span>
            <select
              className="w-72 rounded border border-slate-300 px-2 py-1 text-xs disabled:bg-slate-100 disabled:text-slate-400"
              value={draftRelease}
              disabled={!pitSettings?.can_apply || applying}
              onChange={(event) => setDraftRelease(event.target.value)}
              aria-label="应用的数据版本"
            >
              <option value="">不应用（无 PIT 口径）</option>
          ...
          <label className="text-xs">
            <span className="mb-1 block font-medium text-slate-600">运行模式</span>
            <select
              className="w-40 rounded border border-slate-300 px-2 py-1 text-xs disabled:bg-slate-100 disabled:text-slate-400"
              value={draftMode}
              disabled={!draftRelease || applying}
              onChange={(event) => setDraftMode(event.target.value as RunMode)}
              aria-label="运行模式"
            >
              <option value="RESEARCH">研究模式</option>
              <option value="STRICT_PIT">严格 PIT</option>
            </select>
          </label>
          <button
            type="button"
            className="rounded bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-700 disabled:opacity-50"
            onClick={() => onApply(draftRelease || null, draftRelease ? draftMode : 'RESEARCH')}
            disabled={applying}
          >
            {applying ? '应用中…' : '应用到全平台'}
          </button>
          {!draftRelease && (
            <span className="text-[11px] text-slate-600">
              未应用版本时不能启用严格 PIT —— 没有截止日可执行，等同研究模式。
            </span>
          )}
        </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-3 py-2">
          ...
          <p className="mt-0.5 text-[11px] text-slate-600">
            研究日不足以保证可复现：同一个研究日，上周与本周的底层数据可能已被供应商回溯修订。封版把文件本身钉住。
          </p>
        </div>
