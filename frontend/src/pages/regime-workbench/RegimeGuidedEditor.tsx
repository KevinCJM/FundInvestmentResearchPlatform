...
export default function RegimeGuidedEditor(props: RegimeGuidedEditorProps) {
  ...
  return <div className="mx-auto w-full max-w-6xl space-y-5 p-3 sm:p-5" aria-label="历史情景构建引导">
    {!props.builderOnly && <><div><h2 className="text-lg font-semibold text-slate-950">时序计算逻辑</h2><p className="mt-1 text-xs leading-5 text-slate-500">选择输入，组合计算步骤，再指定输出。与画布和高级公式同步保存。</p></div>
    <details open={!nodes.length} className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">从模板开始</summary><div className="mt-3 flex flex-wrap gap-2"><select aria-label="图谱模板" value={props.selectedTemplate} onChange={event => props.onTemplate(event.target.value)} className="min-h-10 min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-2 text-sm"><option value="">选择模板</option>{props.templates.map(template => <option key={template.id} value={template.id}>{template.default_mode === 'retrospective' ? '事后 · ' : ''}{template.name}</option>)}</select><button type="button" disabled={!props.selectedTemplate || props.loadingTemplate} onClick={props.onLoadTemplate} className="min-h-10 rounded-lg bg-violet-600 px-3 text-xs font-semibold text-white disabled:opacity-40">{props.loadingTemplate ? '载入中…' : '载入模板'}</button><button type="button" onClick={props.onBlank} className="min-h-10 rounded-lg border border-slate-200 px-3 text-xs">新建空白</button></div><p className="mt-2 text-xs text-slate-500">{props.templates.find(template => template.id === props.selectedTemplate)?.description}</p></details></>}
    <section aria-label="构建计算步骤" className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <CalculationSteps compact={props.builderOnly} steps={nodes.map(node => ({ id: node.id, label: node.label || props.schemas.find(item => (item.id || item.type) === node.type)?.label || node.id, kind: kind(node.type) }))} selectedId={active?.id || null} onSelect={id => { setSelectedId(id); props.onSelectNode?.(id) }}>
        ...
        </article> : <p className="p-4 text-sm text-slate-500">添加资源后，在这里编辑变量、常量和计算输入。</p>}
      </CalculationSteps>
    </section>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-violet-50 p-4"><p className="text-xs leading-5 text-violet-800">定义完成后选择实时或事后模式及截至日，试算查看状态色带与区间明细。</p><button type="button" disabled={!props.canRun} onClick={props.onRun} className="min-h-11 rounded-lg bg-violet-600 px-4 text-sm font-semibold text-white disabled:opacity-40">运行当前方案</button></div></>}
  </div>
}
