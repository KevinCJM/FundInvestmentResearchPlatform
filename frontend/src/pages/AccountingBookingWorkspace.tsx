...
export default function AccountingBookingWorkspace() {
  ...
  return (
    <div className="space-y-5" data-testid="accounting-booking-workspace">
      <StaticDemoBanner />
      <header className="rounded-xl bg-gradient-to-r from-accent-950 via-accent-900 to-accent-800 p-6 text-white shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent-100/80">Source event → journal voucher → ledger → statements</p>
        <h2 className="mt-2 text-2xl font-bold">组合 Booking 与复式记账</h2>
        <p className="mt-2 max-w-5xl text-sm leading-6 text-accent-50/85">本页承接已经完成来源账户、真实组合和基金经理拆分的业务事实。Booking 不等于会计凭证，系统按基金与管理人公司两个独立核算主体分别生成借贷分录。</p>
      </header>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        ].map(([label, value, hint]) => <article key={label} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-sm text-slate-600">{label}</p><p className="mt-2 text-2xl font-bold text-slate-950">{value}</p><p className="mt-1 text-xs text-slate-600">{hint}</p></article>)}
      </section>
      ...
      {activeTab === '业务录入' ? (
        <div className="grid gap-5 xl:grid-cols-2">
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-sm text-slate-600">这里只登记可追溯的经济业务事实；会计科目、借贷方向和金额由已生效规则生成并由会计复核。</p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <label className="text-sm text-slate-600">真实组合（来自组合中心）<select aria-label="Booking 真实组合" value={selectedPortfolioId} onChange={(event) => selectBookingPortfolio(event.target.value)} disabled={draft.eventScope === '管理人单独事项'} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 disabled:bg-slate-100">{portfolios.map((portfolio) => <option key={portfolio.portfolioId} value={portfolio.portfolioId}>{portfolio.name} · {portfolio.portfolioId}</option>)}</select></label>
              <label className="text-sm text-slate-600">来源账户<select aria-label="Booking 来源账户" value={draft.sourceAccountId} onChange={(event) => updateDraft('sourceAccountId', event.target.value)} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2">{availableAccountRelationships.map((relationship) => { const account = getExternalAccount(relationship.accountId); return account ? <option key={relationship.relationshipId} value={account.accountId}>{account.kind} · {account.accountId}</option> : null })}</select></label>
              <label className="text-sm text-slate-600">基金经理任职关系<select aria-label="Booking 基金经理任职关系" value={selectedAssignment?.assignmentId ?? '不适用'} onChange={(event) => { const assignment = availableAssignments.find((item) => item.assignmentId === event.target.value); setDraft((current) => ({ ...current, portfolioManagerAssignmentId: assignment?.assignmentId ?? '不适用', sleeveId: assignment?.sleeveId ?? '全组合' })) }} disabled={draft.eventScope === '管理人单独事项'} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 disabled:bg-slate-100">{availableAssignments.map((assignment) => <option key={assignment.assignmentId} value={assignment.assignmentId}>{assignment.portfolioManagerName} · {assignment.role}</option>)}</select></label>
              ...
              <label className="text-sm text-slate-600">事项范围<select aria-label="事项范围" value={draft.eventScope} onChange={(event) => updateDraft('eventScope', event.target.value)} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2">{eventScopeOptions.map((option) => <option key={option}>{option}</option>)}</select></label>
              <label className="text-sm text-slate-600">业务类型<select aria-label="业务类型" value={draft.eventType} onChange={(event) => selectEventType(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2"><option>ETF 买入</option><option>ETF 卖出</option><option>底层基金申购</option><option>底层基金赎回</option><option>客户资金投入</option><option>客户资金转出</option><option>管理费计提</option><option>管理人自有资金跟投</option><option>衍生工具交易</option><option>管理人公司费用</option></select></label>
            ...
            <p className="mt-3 text-xs leading-5 text-slate-600">只有来源账户、订单/成交、分配编号、真实组合和核算主体关系完整的记录才能制证；分类仍是规则预判而非最终会计结论。</p>
            <button type="button" onClick={addBooking} disabled={!draft.instrument || !draft.amount} className="mt-4 min-h-11 rounded-lg bg-accent-800 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300">登记业务事实（演示）</button>
          </section>

          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="font-semibold text-slate-900">Excel / CSV 导入</h3>
            <p className="mt-1 text-sm text-slate-600">本期只展示字段契约与文件选择反馈，不读取、不上传文件，也不模拟凭证过账。</p>
            <label className="mt-4 block rounded-xl border-2 border-dashed border-accent-200 bg-accent-50 p-6 text-center text-sm text-accent-900">
              选择 Booking 文件
            ...
            <p className="mt-3 min-h-6 text-sm font-medium text-accent-800" aria-live="polite">{selectedFile ? `已选择：${selectedFile}（未解析、未上传）` : '尚未选择文件'}</p>
            <h4 className="mt-5 text-sm font-semibold text-slate-800">业务事实字段</h4>
            <div className="mt-2 flex flex-wrap gap-2">{sourceColumns.map((column) => <code key={column} className="rounded-lg bg-slate-100 px-2 py-1 text-xs text-slate-700">{column}</code>)}</div>
            <h4 className="mt-5 text-sm font-semibold text-slate-800">存量凭证明细字段</h4>
            <div className="mt-2 flex flex-wrap gap-2">{journalColumns.map((column) => <code key={column} className="rounded-lg bg-amber-50 px-2 py-1 text-xs text-amber-900">{column}</code>)}</div>
          </section>
        </div>
      ) : null}
      ...
      {activeTab === '会计规则' ? (
        <section className="border-y border-slate-200 bg-slate-50/60 px-4 py-5 sm:px-5" aria-label="会计规则说明（非交互）">
          ...
          <p className="mt-1 text-sm text-slate-600">以下内容用于解释当前静态演示采用的记账边界，不是可点击的规则配置项。</p>
          ...
          <div className="mt-6">
            ...
            <p className="mt-1 text-sm text-slate-600">借、贷是记账方向，不直接等同于增加或减少；必须结合账户性质判断。</p>
      ...
      {activeTab === '复式凭证' ? (
        <section className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold text-slate-900">凭证与分录预览</h3><p className="mt-1 text-sm leading-6 text-slate-600">以下为规则生成的静态凭证示例。每张凭证至少两行、单行不能同时有借贷金额、逐张借贷相等；当前均未复核、未过账。</p></div>
          {visibleVouchers.map((voucher) => {
            const totals = voucherTotalsById.get(voucher.id)
            return (
              <article key={voucher.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                  <div><div className="flex flex-wrap items-center gap-2"><h4 className="font-bold text-slate-950">{voucher.id}</h4><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${voucher.entity === '组合/基金账' ? 'bg-accent-100 text-accent-900' : 'bg-accent-100 text-accent-900'}`}>{voucher.entity}</span><span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-900">{voucher.status}</span></div><p className="mt-2 text-sm text-slate-600">来源 {voucher.sourceId} · {voucher.entityId} · {voucher.eventPhase}</p></div>
                  <div className="text-sm text-slate-600"><p>会计日期：{voucher.accountingDate}</p><p>政策版本：{voucher.policyVersion}</p></div>
                </div>
                <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="bg-slate-50 text-left text-xs tracking-wide text-slate-600"><tr><th scope="col" className="px-4 py-3">科目编码</th><th scope="col" className="px-4 py-3">会计科目</th><th scope="col" className="px-4 py-3">账户类别</th><th scope="col" className="px-4 py-3 text-right">借方</th><th scope="col" className="px-4 py-3 text-right">贷方</th></tr></thead><tbody>{voucher.lines.map((line, index) => <tr key={`${voucher.id}-${index}`} className="border-t border-slate-100"><td className="px-4 py-3 font-mono text-slate-600">{line.accountCode}</td><td className="px-4 py-3 font-medium text-slate-900">{line.accountName}</td><td className="px-4 py-3 text-slate-600">{line.accountClass}</td><td className="px-4 py-3 text-right tabular-nums">{line.debit ? currency(line.debit) : '—'}</td><td className="px-4 py-3 text-right tabular-nums">{line.credit ? currency(line.credit) : '—'}</td></tr>)}</tbody><tfoot className="border-t-2 border-slate-200 bg-slate-50 font-semibold text-slate-900"><tr><td className="px-4 py-3" colSpan={3}>凭证合计</td><td className="px-4 py-3 text-right tabular-nums">借方合计 {totals ? currency(totals.debit) : '—'}</td><td className="px-4 py-3 text-right tabular-nums">贷方合计 {totals ? currency(totals.credit) : '—'}</td></tr></tfoot></table></div>
          ...
          {visibleVouchers.length === 0 ? <p className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-600">当前真实组合没有凭证示例。</p> : null}
        </section>
      ) : null}

      {activeTab === '试算平衡' ? (
        <section className="space-y-5">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold text-slate-900">逐主体试算与资产负债等式</h3><p className="mt-1 text-sm leading-6 text-slate-600">借贷平衡是凭证和总账的机械校验，资产负债等式是期末余额校验；两者都通过仍不代表确认、计量和估值一定正确。</p></div>
          <div className="grid gap-4 lg:grid-cols-2">{entityTotals.map((item) => <article key={item.entity} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-sm font-semibold text-slate-600">{item.entity}｜本期凭证发生额</p><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs text-slate-600">借方发生额</p><p className="mt-1 text-lg font-bold tabular-nums text-slate-950">{currency(item.debit)}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs text-slate-600">贷方发生额</p><p className="mt-1 text-lg font-bold tabular-nums text-slate-950">{currency(item.credit)}</p></div></div><p className="mt-3 text-sm font-semibold text-emerald-700">试算差额 {currency(item.difference)} · {item.voucher_count} 张凭证</p></article>)}</div>
          <div className="grid gap-4 lg:grid-cols-2">
            <article className="rounded-xl border border-accent-200 bg-accent-50 p-5"><p className="text-sm font-semibold text-accent-900">组合/基金账期末余额（静态截面，单位：万元）</p><p className="mt-4 text-xl font-bold text-accent-950">12,648 = 118 + 12,530</p><p className="mt-2 text-sm text-accent-900">资产 = 负债 + 净资产 · 等式差额 0.00</p></article>
            <article className="rounded-xl border border-accent-200 bg-accent-50 p-5"><p className="text-sm font-semibold text-accent-900">管理人公司账期末余额（静态截面，单位：万元）</p><p className="mt-4 text-xl font-bold text-accent-950">13,480 = 2,310 + 11,170</p><p className="mt-2 text-sm text-accent-900">资产 = 负债 + 所有者权益 · 等式差额 0.00</p></article>
          </div>
        </section>
      ) : null}
      ...
      {activeTab === '双账勾稽' ? (
        <section className="space-y-5">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold text-slate-900">跨主体业务勾稽</h3><p className="mt-1 text-sm leading-6 text-slate-600">双账勾稽只核对同一经济业务的来源、期间、对手方和金额；基金账的一条借方不能抵销管理人账的一条贷方。</p></div>
          <div className="grid gap-4 lg:grid-cols-[1fr_auto_1fr] lg:items-stretch">
            <article className="rounded-xl border border-accent-200 bg-accent-50 p-5"><p className="text-xs font-semibold tracking-wide text-accent-700">{selectedPortfolio.portfolioId} · JV-FUND-0003</p><h4 className="mt-2 font-bold text-accent-950">基金承担管理费</h4><p className="mt-4 text-sm text-accent-950">借：6403 管理人报酬　{currency(86420)}</p><p className="mt-2 text-sm text-accent-950">贷：2206 应付管理人报酬　{currency(86420)}</p><p className="mt-4 text-xs font-semibold text-emerald-700">本主体借贷平衡</p></article>
            <div className="flex items-center justify-center text-2xl font-bold text-slate-600">↔</div>
            <article className="rounded-xl border border-accent-200 bg-accent-50 p-5"><p className="text-xs font-semibold tracking-wide text-accent-700">{selectedPortfolio.managerId} · JV-MGR-0001</p><h4 className="mt-2 font-bold text-accent-950">管理人确认管理费收入</h4><p className="mt-4 text-sm text-accent-950">借：1122 应收管理费　{currency(86420)}</p><p className="mt-2 text-sm text-accent-950">贷：6001 管理费收入　{currency(86420)}</p><p className="mt-4 text-xs font-semibold text-emerald-700">本主体借贷平衡</p></article>
          </div>
      ...
      {activeTab === '校验异常' ? (
        <section className="space-y-5">
          <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-sm text-slate-600">修改借贷合计可验证硬阻断逻辑；本演示不生成、不复核、不提交凭证。</p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <aside className="border-l-2 border-amber-300 bg-amber-50/50 px-4 py-4" role="note" aria-label="阻断规则说明（非交互）">
            <p className="text-xs font-semibold tracking-wide text-amber-700">非交互说明</p>
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-5 py-4"><h3 className="font-semibold text-slate-900">组合 Booking 业务事实（示例）</h3><p className="mt-1 text-sm text-slate-600">以下记录已经完成来源账户和组合分配；它们用于制证、重建持仓与现金，但本身不是总账。</p></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[1840px] text-sm"><thead className="bg-slate-50 text-left text-xs font-semibold text-slate-600"><tr>{['来源号', '来源账户/分配', '交易/会计/交收日', '真实组合/管理人主体', '基金经理任职/投资单元', '事项范围', '业务类型', '产品或事项', '金额', '组合/基金账', '管理人账', '绩效性质', '会计状态', '操作'].map((heading) => <th scope="col" key={heading} className="px-4 py-3">{heading}</th>)}</tr></thead><tbody>{visibleRecords.map((record) => <tr key={record.id} className="border-t border-slate-100"><td className="px-4 py-3 font-medium text-slate-900">{record.id}</td><td className="px-4 py-3 font-mono text-xs text-slate-600">{record.sourceAccountId}<br /><span className="text-slate-600">{record.allocationId}</span></td><td className="px-4 py-3 text-slate-600"><span title="交易日期">{record.tradeDate}</span><br /><span className="text-xs text-slate-600" title="会计日期">{record.accountingDate ?? record.tradeDate}</span><br /><span className="text-xs text-slate-600" title="交收日期">{record.settlementDate}</span></td><td className="px-4 py-3 font-mono text-xs text-slate-600">{record.portfolio}<br /><span className="text-slate-600">{record.manager}</span></td><td className="px-4 py-3 font-mono text-xs text-slate-600">{record.portfolioManagerAssignmentId}<br /><span className="text-slate-600">{record.sleeveId}</span></td><td className="px-4 py-3">{record.eventScope}</td><td className="px-4 py-3">{record.eventType}</td><td className="px-4 py-3">{record.instrument}</td><td className="px-4 py-3 tabular-nums">{record.amount}</td><td className="px-4 py-3">{record.portfolioTreatment}</td><td className="px-4 py-3">{record.managerTreatment}</td><td className="px-4 py-3">{record.performanceFlow}</td><td className="px-4 py-3"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{record.status}</span></td><td className="px-4 py-3"><button type="button" onClick={() => removeBooking(record.id)} className="min-h-11 rounded-lg px-3 text-sm font-semibold text-rose-700 hover:bg-rose-50">移除演示记录</button></td></tr>)}</tbody></table></div>
        {visibleRecords.length === 0 ? <p className="border-t border-slate-100 px-5 py-8 text-center text-sm text-slate-600">当前真实组合没有 Booking 示例记录。</p> : null}
      </section>
