...
function VariablePicker({ title, variables, selected, onChange }: { title: string; variables: RiskVariable[]; selected: string[]; onChange: (keys: string[]) => void }) {
  ...
  return <fieldset className="min-w-0"><legend className="text-sm font-semibold text-slate-900">{title} <span className="font-normal text-slate-500">已选 {selected.length} 项</span></legend><input className={`${inputClass} mb-2`} aria-label={`搜索${title}`} placeholder="搜索名称或代码" value={query} onChange={event => setQuery(event.target.value)} /><div className="max-h-64 space-y-1 overflow-y-auto rounded-lg border border-slate-200 p-2">{filtered.map(variable => <label key={variable.id} className="flex cursor-pointer items-start gap-2 rounded-md px-2 py-2 text-sm hover:bg-slate-50"><input className="mt-1" type="checkbox" checked={selected.includes(variable.id)} disabled={(!variable.availability?.available && !selected.includes(variable.id)) || (selected.length >= 8 && !selected.includes(variable.id))} onChange={event => onChange(event.target.checked ? [...selected, variable.id] : selected.filter(key => key !== variable.id))} /><span className="min-w-0"><span className="font-medium text-slate-800">{variable.name}</span><span className="ml-2 text-xs text-slate-500">{variable.unit_label} · {frequencyLabels[variable.frequency]}</span>{!variable.availability?.available && <span className="mt-1 block text-xs text-amber-800">{variable.availability?.reason ?? '数据状态未知，请刷新。'}</span>}</span></label>)}{!filtered.length && <p className="p-3 text-sm text-slate-500">没有匹配变量。缺少自有数据时，可到「数据与因子」导入。</p>}</div></fieldset>
}
...
function PublishedProductLinks({ releaseId, targets }: { releaseId: string; targets: RiskRun['targets'] }) {
  ...
  return <nav aria-label="到产品查看这份成果" className="space-y-2 border-t border-slate-100 pt-3">
    <p className="text-xs text-slate-500">跳转后锁定这份发布版本，不会换成其他模型或重新训练。</p>
...
export default function ResearchWorkbench({ domain = 'product' }: { domain?: ResearchDomain }) {
  ...
  return <div className="min-w-0 space-y-5" data-testid={`${domain}-risk-workbench`}>
    ...
    {loading && !catalog ? <div role="status" className="rounded-xl bg-white p-6 text-sm text-slate-500">正在读取本地研究目录…</div> : !catalog ? <Empty title="研究目录暂不可用"><p>检查数据磁盘与后端连接后重试。</p><button type="button" className={`${buttonClass} mt-3`} onClick={() => setRefresh(old => old + 1)}>重新加载</button></Empty> : <>
      {pane === 'research' && <>
