...
export default function TaaResearchContext({ baseline, request, preview, preflight, checking, error, contextIssue, onChange, onEdit, onRetry }: {
  ...
  return <section className={`${sectionClass} space-y-3`} aria-label="本次研究条件与资格">
    <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-semibold">本次研究条件</h2><button type="button" className="min-h-9 text-sm font-medium text-teal-800 underline" onClick={onEdit}>修改日期与费用</button></div>
    <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-xs sm:grid-cols-3 lg:grid-cols-6">
      ...
      ].map(([label, value]) => <div key={label} className="min-w-0"><dt className="text-slate-500">{label}</dt><dd className="mt-1 break-words font-medium leading-5">{value}</dd></div>)}
    </dl>
    <div role="status" className={`rounded-lg px-3 py-2 text-xs leading-5 ${blocked || error ? 'bg-rose-50 text-rose-900' : preflight?.can_calculate ? 'bg-teal-50 text-teal-900' : 'bg-amber-50 text-amber-900'}`}>
    ...
    {preview && request.signal_mode === 'momentum' && !preview.data.training && <p className="text-xs text-slate-500">此结果未记录有效信号期数；重新计算会采用当前趋势口径。</p>}
    ...
    <details className="text-xs leading-5 text-slate-500"><summary className="cursor-pointer">日期含义、继承范围与证据限制</summary><div className="mt-2 space-y-1"><p>继承 SAA 权重、资产范围和约束。以下回测按本页日期、日频目标再平衡和费用重算，不直接沿用 SAA 页的历史曲线。</p><p>研究日限制本次可知信息；行情日期不是信息首次可得日期，当前修订数据的历史回放不等于正式 PIT 业绩。</p>{reasons.map(reason => <p key={reason}>{reason}</p>)}</div></details>
  </section>
}
