...
vi.mock('../app/ResearchContext', () => ({ useResearchDay: () => '2026-09-12' }))
...
function install(overrides: Record<string, (init?: RequestInit) => Promise<Response>> = {}) {
  const mock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url === `${root}/mandates`) return response({ ...mandateVersion, definition: JSON.parse(String(init?.body)) }, 201)
...
function renderPolicy() {
  return render(<MemoryRouter initialEntries={['/pre-investment/saa/policy?alloc=股债分类&mandate=mandate-1']}><Routes>
...
describe('真实自上而下操作顺序', () => {
  it('目标未完成时不允许保存；保存后直接携带目标版本进入政策研究', async () => {
    ...
    const save = screen.getByRole('button', { name: '保存新目标版本' })
    ...
    const call = fetch.mock.calls.find(([url]) => String(url) === `${root}/mandates`)!
    expect(JSON.parse(String(call[1]?.body)).target_return).toBe(.03)
  ...
  it('目标保存中修改输入，不把旧版本当成新目标', async () => {
    let resolve!: (value: Response) => void
    install({ [`${root}/mandates`]: () => new Promise(done => { resolve = done }) })
    writeAllocationDraft('strategic-mandate:editor', mandateVersion.definition)
