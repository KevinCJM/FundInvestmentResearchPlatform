...
function MatrixTable({ title, matrix }: { title: string; matrix?: { labels: string[]; values: number[][] } | null }) {
  if (!matrix?.labels.length) return <section className="rounded-xl border border-slate-200 bg-white p-4"><h2 className="font-semibold">{title}</h2><p className="mt-2 text-sm text-slate-600">该快照没有足够的共同历史样本，无法计算矩阵。</p></section>
...
export default function HoldingDiagnosis() {
  ...
  return <div className="mx-auto max-w-7xl space-y-5 p-4 sm:p-6" aria-busy={scenarioLoading || exporting !== null}>
    ...
    <div className="flex flex-wrap items-end gap-2"><label className="text-xs text-slate-600">单表 CSV<select aria-label="CSV 导出表" value={exportTable} onChange={(event) => setExportTable(event.target.value as PortfolioExportTable)} className="mt-1 block rounded-xl border border-slate-300 bg-white px-2 py-2 text-sm"><option value="summary">汇总</option><option value="components">成分</option><option value="daily-contributions">日收益贡献</option><option value="weight-path">权重路径</option><option value="covariance">协方差矩阵</option><option value="correlation">相关矩阵</option><option value="risk-contributions">风险贡献</option>{scenarioResult && <><option value="scenario-summary">情景汇总</option><option value="scenario-series">情景序列</option></>}</select></label><button type="button" onClick={() => handleExport('csv')} disabled={exporting !== null} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm">{exporting === 'csv' ? '导出中…' : '导出 CSV'}</button><button type="button" onClick={() => handleExport('zip')} disabled={exporting !== null} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm">{exporting === 'zip' ? '导出中…' : '导出 ZIP'}</button><span className="text-xs text-slate-500">ZIP 包含全部表{scenarioResult ? '及当前情景结果' : ''}</span><Link to="/pre-investment/product-allocation-timing/construction" className="rounded-lg bg-emerald-700 px-3 py-2 text-sm text-white">新建组合研究</Link></div>
    <section className="grid grid-cols-2 gap-3 md:grid-cols-4">{metrics.map((metric) => <div key={metric.name} className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-xs text-slate-600">{metric.name}</p><p className="mt-1 text-lg font-semibold"><MetricValue value={metric.value} presentation={metricPresentation(metric)} /></p></div>)}</section>
