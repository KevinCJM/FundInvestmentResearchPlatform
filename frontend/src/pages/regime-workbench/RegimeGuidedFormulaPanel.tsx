...
export default function RegimeGuidedFormulaPanel({ definition, schemas, outputId, onBuild, onResources }: {
  ...
  return <section aria-label="情景公式构建向导" className="mt-4 space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
    <div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold text-slate-600">当前结果：{outputLabel}</p><span className="text-xs text-slate-400">{root ? '已生成' : '待构建'}</span></div>
    {root ? <>
      <p className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs leading-5 text-slate-600">选择计算步骤可修改参数和输入；完整结构与画布、高级公式保持同步。</p>
      ...
      <div aria-label="当前输出计算逻辑" className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-4">{steps.map((step, i) => <button type="button" key={step.id} onClick={() => onBuild(step.id)} className="min-h-10 rounded-lg border border-violet-100 bg-violet-50 px-3 py-2 text-sm text-violet-900">{i + 1}. {label(step.id)}</button>)}</div>
    </> : <div className="rounded-lg border border-dashed border-violet-200 bg-white p-4 text-center"><p className="font-semibold text-slate-800">从变量、数学算子或已有指标开始</p><p className="mt-1 text-xs leading-5 text-slate-500">组合输入与计算步骤，再将结果连接到输出通道。</p></div>}
    <div className="flex flex-wrap gap-2"><button type="button" onClick={onResources} className="min-h-11 flex-1 rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-700">浏览公式构建资源</button>{definition.graph.nodes.length > 0 && <button type="button" onClick={() => onBuild(root?.node_id)} className="min-h-11 rounded-lg border border-violet-200 bg-white px-4 text-sm font-semibold text-violet-700">修改计算逻辑</button>}</div>
  </section>
}
