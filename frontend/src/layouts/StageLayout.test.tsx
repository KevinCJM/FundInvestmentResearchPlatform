...
it('shows the actual selected research and preserves the allocation in a return link', () => {
  ...
  expect(flow.getByRole('link', { name: '4. 长期配置 SAA' })).toHaveAttribute('href', '/pre-investment/saa/policy?baseline=b1&alloc=60%2F40&universe=u1')
  expect(flow.getByRole('link', { name: '5. 战术研究 TAA' })).toHaveAttribute('aria-current', 'step')
...
it('does not mark downstream steps complete when no product range has been locked', () => {
  ...
  const flow = within(screen.getByRole('navigation', { name: '配置研究流程' }))
  expect(flow.queryByRole('link', { name: '4. 长期配置 SAA' })).not.toBeInTheDocument()
  expect(flow.queryByRole('link', { name: '5. 战术研究 TAA' })).not.toBeInTheDocument()
...
it('allows independent strategic research without pretending products or mappings exist', () => {
  ...
  const flow = within(screen.getByRole('navigation', { name: '配置研究流程' }))
  expect(flow.getByRole('link', { name: '4. 长期配置 SAA' })).toHaveAttribute('href', '/pre-investment/saa/policy?mandate=m1&strategic_universe=s1')
...
it('highlights saved strategic policy and only links TAA after an explicit mapping reference', () => {
  ...
  const flow = within(screen.getByRole('navigation', { name: '配置研究流程' }))
  expect(flow.getByRole('link', { name: '4. 长期配置 SAA' })).toHaveAttribute('aria-current', 'step')
  expect(flow.getByRole('link', { name: '5. 战术研究 TAA' })).toHaveAttribute('href', '/pre-investment/taa?baseline=b1')
  expect(flow.queryByRole('link', { name: '6. 产品配置' })).not.toBeInTheDocument()
})
