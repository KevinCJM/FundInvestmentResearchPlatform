...
export default function RegimeNodePreviewPanel({ definition, schemas, initialNodeId, initialMode, initialAsOf, onChange }: {
  ...
  const jobRef = useRef('')
  const currentSignature = JSON.stringify([definitionForRequest(definition), nodeId, outputPort, mode, asOf])
  ...
  const preview = async (draft = definition) => {
    if (!node || !outputPort) return
    controllerRef.current?.abort()
    if (jobRef.current) void cancelRegimePreviewRun(jobRef.current).catch(() => undefined)
    const controller = new AbortController(); controllerRef.current = controller
    const snapshot = cloneRegimeGraphDefinition(draft)
    const target = { node_id: nodeId, port: outputPort }
    setBusy(true); setError(''); setDiagnostics([]); setPage(null); setCollapsed(false)
    setSignature(JSON.stringify([definitionForRequest(snapshot), nodeId, outputPort, mode, asOf]))
    setRun({ id: '', status: 'preparing', message: '正在检查所选节点及其上游。' })
    try {
      const prepared = await prepareRegimeGraph(snapshot, controller.signal, target)
      if (controller.signal.aborted) return
      let next = await startRegimePreviewRun(snapshot, { compileToken: prepared.compile_token, mode, asOf: asOf || undefined, previewTarget: target }, controller.signal)
  ...
  return <section className="space-y-4" aria-label="独立节点预览">
    <p className="text-sm leading-6 text-slate-600">只计算所选节点和它依赖的上游。无需写完公式或连接最终市场状态。</p>
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="text-xs font-semibold text-slate-600">预览节点<select aria-label="待预览节点" value={nodeId} onChange={event => { setNodeId(event.target.value); setPort('') }} className="mt-1 min-h-10 w-full rounded-lg border border-slate-300 bg-white px-2">{definition.graph.nodes.map(item => <option key={item.id} value={item.id}>{item.label || schemas.find(s => [s.id, s.type, s.type_id].includes(item.type))?.label || item.id}</option>)}</select></label>
      <label className="text-xs font-semibold text-slate-600">输出数据<select aria-label="预览输出端口" value={outputPort} onChange={event => setPort(event.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-slate-300 bg-white px-2">{schema?.outputs.map(item => <option key={item.id} value={item.id}>{regimePortLabel(item)}</option>)}</select></label>
      <label className="text-xs font-semibold text-slate-600">分析方式<select aria-label="节点预览分析方式" value={mode} onChange={event => setMode(event.target.value as RegimeMode)} className="mt-1 min-h-10 w-full rounded-lg border border-slate-300 bg-white px-2"><option value="realtime">实时分析</option><option value="retrospective">事后研究</option></select></label>
      <label className="text-xs font-semibold text-slate-600">截至日<input aria-label="节点预览截至日" type="date" value={asOf} onChange={event => setAsOf(event.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-slate-300 bg-white px-2" /></label>
    </div>
    <div className="flex items-center gap-3"><button type="button" disabled={!node || !outputPort || busy} onClick={() => void preview()} className="min-h-10 rounded-lg bg-violet-600 px-4 text-sm font-semibold text-white disabled:opacity-40">{busy ? '正在预览…' : '预览节点数据'}</button>{busy && <button type="button" onClick={() => { controllerRef.current?.abort(); if (jobRef.current) void cancelRegimePreviewRun(jobRef.current).catch(() => undefined); jobRef.current = ''; setBusy(false); setRun(previous => previous ? { ...previous, status: 'cancelled' } : previous) }} className="min-h-10 px-2 text-sm text-slate-600">取消预览</button>}</div>
    ...
    {!stale && !busy && onChange && alignmentSchema && alignmentNodes.length > 0 && <div className="space-y-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-slate-700">
      ...
      <button type="button" onClick={alignAndPreview} className="min-h-10 rounded-lg bg-violet-600 px-3 font-semibold text-white">按共同日期对齐并预览</button>
    </div>}
    ...
    <RegimeResultDock nodeOnly run={run} page={page} nodes={[]} schemas={schemas} previewNodeId={nodeId} loadingSeries={busy} height={height} collapsed={collapsed} onPreviewNode={() => undefined} onLoadSeries={() => undefined} onHeight={setHeight} onCollapsed={setCollapsed} />
  </section>
}
