...
export default function RegimeResultView({ runId, runKind = 'preview', stale = false }: RegimeResultViewProps) {
  ...
  return <section aria-label="完整历史情景结果" className="min-w-0 space-y-4 p-3 sm:p-5" data-run-id={overview.run_id}>
    <header className="flex flex-wrap items-start justify-between gap-3">
      <div><h2 className="text-lg font-bold text-slate-950">历史情景结果</h2><p className="mt-1 text-xs text-slate-500">{overview.date_range.start ?? '无样本'} 至 {overview.date_range.end ?? '无样本'} · 共 {overview.summary.total} 个观测日{overview.as_of ? ` · 截至 ${overview.as_of}` : ''} · {overview.created_at ? '运行于 ' + overview.created_at : '运行 ' + overview.run_id}</p></div>
      <span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-800">{overview.mode === 'retrospective' ? '事后解释' : '当时可知模式'} · 观测日色带</span>
    </header>
    ...
    <p className="rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">{overview.capabilities.effective.reason || '当前仅展示观测日区间，未提供完整生效时间轴。'}{overview.mode === 'retrospective' ? ' 事后色带不能直接当作当时可交易的信号。' : ''}</p>
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4" aria-label="全样本情景摘要">
      <Summary label="有效分类覆盖" value={percent(overview.summary.classified, overview.summary.total)} detail={overview.summary.classified + ' / ' + overview.summary.total + ' 个观测日'} />
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-2 text-xs text-slate-600">{overview.states.map(state => <span key={state.id}>{state.label}：{overview.summary.state_counts[state.id] ?? 0} 日（{percent(overview.summary.state_counts[state.id] ?? 0, overview.summary.total)}）</span>)}<span>占比分母：全部 {overview.summary.total} 个观测日；图表缩放不改变统计。</span></div>
    ...
    <div role="tabpanel" id={'regime-result-panel-' + tab} aria-labelledby={'regime-result-tab-' + tab}>
      {tab === 'intervals' ? <div className="max-h-96 overflow-auto"><table className="min-w-full text-left text-xs" aria-label="完整情景区间明细"><thead className="sticky top-0 bg-slate-50 text-slate-600"><tr>{['状态', '起始日期', '结束日期', '观测日数', '首点确认', '首点生效', '依据'].map(label => <th key={label} className="whitespace-nowrap px-3 py-2">{label}</th>)}</tr></thead><tbody>{result.intervals.map(item => <tr key={item.id} className={'border-t border-slate-100 ' + (interval?.id === item.id ? 'bg-indigo-50' : '')}><td className="whitespace-nowrap px-3 py-2"><span className="mr-2 inline-block h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: intervalColor(result, item) }} />{item.label}</td><td className="whitespace-nowrap px-3 py-2">{item.start_date}</td><td className="whitespace-nowrap px-3 py-2">{item.end_date}</td><td className="px-3 py-2">{item.observations}</td><td className="whitespace-nowrap px-3 py-2">{item.confirmed_at ?? '未提供'}</td><td className="whitespace-nowrap px-3 py-2">{item.effective_start ?? '未提供'}</td><td className="px-3 py-2"><button type="button" onClick={() => selectInterval(item)} className="whitespace-nowrap text-indigo-700 underline" aria-label={'查看 ' + item.label + ' ' + item.start_date + ' 的依据'}>查看依据</button></td></tr>)}</tbody></table></div> : null}
      ...
      {tab === 'details' ? <dl className="grid gap-3 break-all rounded-xl bg-slate-50 p-4 text-xs sm:grid-cols-2">{[['运行身份', overview.run_kind + ' / ' + overview.run_id], ['识别模式', overview.mode === 'retrospective' ? '事后解释' : '当时可知'], ['定义快照', overview.definition_hash], ['计算图快照', overview.graph_hash], ['频率 / 日历', (overview.frequency ?? '未提供') + ' / ' + (overview.calendar ?? '未提供')], ['时间口径', '真实观测日双闭区间'], ['数据快照', JSON.stringify(overview.data_snapshots)]].map(([label, value]) => <div key={label}><dt className="text-slate-500">{label}</dt><dd className="mt-1 text-slate-800">{value}</dd></div>)}</dl> : null}
    </div>
  </section>
}
