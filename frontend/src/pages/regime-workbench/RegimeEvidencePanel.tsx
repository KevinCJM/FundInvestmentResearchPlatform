...
export default function RegimeEvidencePanel({ result, interval }: { result: RegimeResultData; interval: RegimeResultInterval | null }) {
  ...
  return <section aria-label="区间判断依据" className="space-y-4 rounded-xl border border-slate-200 p-4">
    <div><h3 className="font-bold text-slate-900">{interval.label} · {interval.start_date} 至 {interval.end_date}</h3><p className="mt-1 text-xs text-slate-600">本段 {interval.observations} 个真实观测日；以下时点对应区间首个观测日，不代表整段均已可交易。</p></div>
    <dl className="grid gap-3 text-sm sm:grid-cols-3">
      <div><dt className="text-slate-600">首个观测日</dt><dd className="mt-1 font-medium">{interval.start_date}</dd></div>
      <div><dt className="text-slate-600">首点识别 / 确认日</dt><dd className="mt-1 font-medium">{confirmed ?? '未提供'}</dd></div>
      <div><dt className="text-slate-600">首点生效日</dt><dd className="mt-1 font-medium">{effective ?? '尚未提供或未生效'}</dd></div>
    </dl>
    {peakStart != null && peakStart >= 0 && peakEnd != null && peakEnd >= 0 ? <div className="space-y-2 rounded-lg bg-accent-50 p-3 text-sm" aria-label="峰谷定界依据">
    ...
    {hasTrend ? <div className="space-y-2 rounded-lg bg-amber-50 p-3 text-sm" aria-label="趋势判断证据">
      ...
      <dl className="grid gap-2 sm:grid-cols-3">
        {([['distance', '标准化价格偏离'], ['slope', '标准化趋势斜率'], ['efficiency', '方向效率']] as const).map(([key, label]) => <div key={key}><dt className="text-slate-600">{label}</dt><dd>{regimeFeatureNumber(first, key)?.toFixed(4) ?? '未知'}</dd></div>)}
      </dl>
    ...
    <div className="rounded-lg bg-slate-50 p-3 text-sm text-slate-700">
      ...
      {first?.raw.reason_code != null ? <p className="mt-2 text-xs text-slate-600">服务端原因码：{String(first.raw.reason_code)}</p> : null}
    </div>
    <p className="text-xs text-slate-600">{result.overview.mode === 'retrospective' ? '本次为事后解释；分类可能使用之后才可得的信息，不能直接用作当时的交易信号。' : '本次采用当时可知的识别模式；当前图仍按观测日展示，交易时点需以服务端可得性与发布门禁为准。'}</p>
  </section>
}
