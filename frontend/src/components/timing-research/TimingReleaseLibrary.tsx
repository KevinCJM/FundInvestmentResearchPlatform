...
export default function TimingReleaseLibrary({ onView, disabled }: { onView: (runId: string) => void; disabled?: boolean }) {
  ...
  return <section aria-label="已有研究版本" className="space-y-4">
    <div><h2 className="text-base font-semibold text-slate-900">选择已经检验的研究版本</h2><p className="mt-2 text-sm leading-6 text-slate-600">先查看适用产品和研究结果，再将固定版本引用到投前。引用不会重新计算或重复发布。</p></div>
    ...
    {!!releases?.length && <><div role="radiogroup" aria-label="选择已有研究版本" className="grid gap-3 md:grid-cols-2">{releases.map(release => <label key={release.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 ${selected?.id === release.id ? 'border-accent-400 bg-accent-50/40' : 'border-slate-200 bg-white'}`}><input type="radio" name="timing-release" className="mt-1 h-4 w-4 shrink-0" checked={selected?.id === release.id} disabled={binding || disabled} onChange={() => { setSelectedId(release.id); setNotice('') }} /><span className="min-w-0"><strong className="block break-words text-sm text-slate-900">{release.name || '择时研究版本'}</strong><span className="mt-1 block text-xs text-slate-600">{release.created_at?.replace('T', ' ').slice(0, 19) || '已保存'} · 研究用途</span>{release.products?.length ? <span className="mt-2 block break-words text-xs text-slate-600">适用产品：{release.products.map(product => product.product_id).join('、')}</span> : null}{release.note && <span className="mt-2 block text-xs leading-5 text-slate-600">{release.note}</span>}</span></label>)}</div>
      ...
      <div className="flex flex-wrap gap-3"><button type="button" className="min-h-10 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm text-slate-700 disabled:opacity-40" disabled={!selected || binding || disabled} onClick={() => selected && onView(selected.run_id)}>查看此版本结果</button><button type="button" className="min-h-10 rounded-lg bg-accent-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40" disabled={!selected || binding || disabled} onClick={bind}>{binding ? '引用中…' : '引用此版本到投前'}</button></div>
      <p className="text-xs text-slate-600">不自动交易、不改变组合权重；大类配置调整仍需遵守 TAA 约束。</p>
    </>}
  </section>
}
