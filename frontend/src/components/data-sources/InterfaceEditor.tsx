...
function ValidationReport({ result, targets }: { result: MappingValidation; targets: DataModelCatalog }) {
  ...
  return <section className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-4" aria-label="映射验证结果">
    ...
    <p className="text-xs leading-5 text-slate-600">定义校验检查配置；样本预览检查字段转换。两者都不会下载完整历史或发布数据。</p>
    ...
    {isPreview ? (result as MappingPreview).tables.map((table, index) => {
      ...
      return <details key={`${table.table_id}-${index}`} className="min-w-0 rounded-xl border border-slate-200 bg-white p-3" open>
        ...
        <div className="mt-3 max-h-80 overflow-auto"><table className="text-left text-xs"><caption className="sr-only">{definition?.label ?? table.table_id} 映射预览</caption><thead><tr>{columns.map(name => <th key={name} scope="col" className="whitespace-nowrap border-b px-3 py-2">{definition?.fields.find(field => field.name === name)?.label ?? name}<code className="mt-1 block font-normal text-slate-600">{name}</code></th>)}</tr></thead><tbody>{table.rows.map((row, i) => <tr key={i}>{columns.map(name => <td key={name} className="whitespace-nowrap border-b px-3 py-2">{row[name] == null ? '空值' : typeof row[name] === 'object' ? JSON.stringify(row[name]) : String(row[name])}</td>)}</tr>)}</tbody></table></div>
      </details>
    }) : null}
...
export default function InterfaceEditor({ record, source, targets, editingEnabled, credentialConfigured = false, onSaved, onDirty, onBusy }: {
  ...
  return <form ref={form} noValidate className="min-w-0 space-y-4 rounded-xl border border-slate-200 bg-white p-5" onSubmit={event => {
    ...
    <div className="flex flex-wrap justify-between gap-2"><div><h2 className="text-lg font-bold text-slate-950">{record.revision ? config.name : '新建接口'}</h2><p className="mt-1 text-xs text-slate-600">{source.name} · {record.revision ? `修订 ${record.revision}` : '未保存'}</p></div><button type="submit" disabled={!editingEnabled || busy} className={primaryClass}>{busy ? '处理中…' : '保存接口配置'}</button></div>
    <p className="text-xs leading-5 text-slate-600">{dirty ? '有未保存修改：先保存，再使用真实接口采样。' : record.revision ? '当前显示已保存的配置。修改后保存并重新验证。' : '先配置接口与返回结构，再映射标准字段，最后验证。'}</p>
    <nav aria-label="接口配置步骤" className="grid gap-2 sm:grid-cols-3">{steps.map(([id, label]) => <button type="button" key={id} disabled={busy} aria-current={step === id ? 'step' : undefined} className={`${buttonClass} ${step === id ? 'border-accent-400 bg-accent-50 text-accent-800' : ''}`} onClick={() => revealStep(id)}>{label}</button>)}</nav>
    ...
    <fieldset disabled={busy} onChangeCapture={event => { if (!(event.target as HTMLElement).closest('[data-editor-ui]')) markDirty() }} className="min-w-0 space-y-4">
      <details id="endpoint-settings" data-editor-step hidden={step !== 'endpoint-settings'} tabIndex={-1} open className="scroll-mt-6 rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer text-sm font-bold">数据与接口</summary><fieldset disabled={!editingEnabled} className="mt-4 space-y-4"><p className="text-sm leading-6 text-slate-600">这里决定读取哪一类数据。已有接口通常只需核对名称和参数；不确定的地址、响应路径和权限设置不要随意改动。</p>
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {source.transport !== 'akshare' ? <TextField label="接口相对路径" value={config.path} placeholder="/v1/prices；根路径留空" onChange={path => patch({ ...config, path })} /> : <p className="text-xs leading-5 text-slate-600">SDK 接口名称填写函数名；请求方法由 SDK 执行，网络调用仍受共享限额与超时控制。</p>}
      ...
      <details id="limit-settings" data-editor-step hidden={step !== 'limit-settings'} open tabIndex={-1} className="scroll-mt-6 rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer text-sm font-bold">高级：下载限制与分页设置</summary><fieldset disabled={!editingEnabled} className="mt-4 space-y-4">
        ...
        <p className="text-xs leading-5 text-slate-600">Tushare 全量 / 增量仍由原下载器按代码和日期分片。本页的接口限频、重试、超时与支持的分页参数会实际生效。</p>
      </fieldset></details>
    ...
    <section id="mapping-validation" data-editor-step hidden={step !== 'mapping-validation'} tabIndex={-1} className="scroll-mt-6 space-y-3 rounded-xl border border-slate-200 p-4" aria-label="验证与预览">
      <h3 className="text-sm font-bold">验证与使用</h3><p className="text-xs leading-5 text-slate-600">离线预览不访问外网、不写入数据。真实采样最多请求一次，不表示已完成全量下载。</p>
      ...
      <div className="rounded-xl bg-accent-50 p-4"><h4 className="text-sm font-bold">下一步：到下载工作区选择范围</h4><p className="mt-2 text-sm leading-6 text-slate-600">保存只保存配置。下载后还需检查候选数据和发布状态，才能用于正式研究。</p>{record.revision > 0 && !dirty && !busy ? <Link className={`${primaryClass} mt-3 inline-flex`} to={`/settings/data-sources?source=${encodeURIComponent(source.id)}`}>进入下载工作区 →</Link> : <p className="mt-2 text-xs text-amber-900">请先保存接口配置。</p>}</div>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4"><p className="text-xs text-slate-600">{step === 'mapping-validation' ? '定义校验 → 样本验证 → 保存配置 → 下载；各步骤的结果独立。' : '切换步骤不会丢失输入；离开当前接口前请保存。'}</p><button type="button" className={primaryClass} disabled={busy} onClick={() => revealStep(step === 'endpoint-settings' || step === 'mapping-validation' ? 'mapping-settings' : 'mapping-validation')}>{step === 'mapping-validation' ? '上一步：检查字段对应' : step === 'endpoint-settings' ? '下一步：对应系统字段' : '下一步：验证与使用'}</button></div>
