...
| 原来每行都印 | 现在 |
| --- | --- |
| `内置 · N 日复权收盘价均线`（来源、通道名，多半重复指标名） | 移入行名的悬停提示 |
...
## 11. 请求分批与加载期间复制（2026-09-18）

### 问题与需求

PR #28 审核确认：列表不限数量，但第51条曲线仍一次提交，触发后端50条请求守卫并清空整批叠加结果；首批结果返回前复制，还会把缺少通道元数据时的临时子图位置固化。

- 继续允许添加多条曲线，每次请求最多50条，不放宽后端保护。
- 复制后默认与来源叠加，保留明确选择的右轴、独立/共用子图；慢响应和连续复制不能改变该行为。

### 实现边界

- `ProductTrendChart` 按50条顺序调用现有时序接口，全部成功后一次更新结果，继续按 `instance_key` 匹配，不依赖响应顺序。其他调用方的单实例接口和数值计算保持原契约。
- 数据请求键包含产品、类型、实例、锁定修订及参数。请求失效或组件卸载后，旧响应不能更新页面或启动剩余批次；中途失败显示错误，不把部分结果当作完成。
- 通道尚未返回时只继承明确位置，另记录默认子图宿主；有匹配原生轴时采用原生轴，否则经过现有口径校验共用宿主子图。连续复制沿用同一宿主，避免多级依赖；显式位置优先，宿主消失或口径不一致仍按原规则退回独立子图。
- 没有新增算子、数值内核或依赖；50条为现有接口合同的前端批宽，软件测试不替代大规模曲线渲染性能验收。

### 验收

定向测试覆盖50/51/101条、乱序响应、后续批次失败、卸载取消、慢响应下原生轴/默认子图/显式右轴/显式子图的连续复制与手动拆分。浏览器使用现有离线API夹具，覆盖320/768/1440布局、慢响应复制、51条分批、文字对比度及控制台异常。

本轮结果：前端147个测试文件、1170项通过（走势图22项）；新增浏览器6项通过，旧详情页断言按已取消的数量上限及观察值展示同步后3项通过。TypeScript、Vite构建、设计/i18n检查、AI Hermes validate/evolve通过。浏览器为离线合同夹具，未修改正式数据或后端数值算法。
