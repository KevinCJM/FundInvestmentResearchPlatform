...
export default function RegimeSeriesOutputs({ definition, schemas, onChange, activeId, onSelectOutput }: {
  ...
  return <section aria-label="时序输出配置" className="space-y-4">
    <div><h3 className="text-sm font-semibold text-slate-900">输出通道</h3><p className="mt-1 text-xs leading-5 text-slate-600">指定每个时点返回的值。市场状态使用枚举；图表与区间明细在结果页展示。</p></div>
    <div role="tablist" aria-label="时序输出通道" className="flex flex-wrap gap-2">{allSlots.filter(item => item[0] === 'state' || definition.graph.outputs[item[0]]).map(item => <button type="button" role="tab" aria-selected={active === item[0]} key={item[0]} onClick={() => setActive(item[0])} className={`min-h-10 rounded-lg border px-3 text-xs font-semibold ${active === item[0] ? 'border-accent-500 bg-accent-600 text-white' : 'border-slate-200 bg-white'}`}>{item[1]}</button>)}<select aria-label="添加输出通道" value="" onChange={event => setActive(event.target.value as Slot)} className="min-h-10 rounded-lg border border-dashed border-accent-300 bg-white px-2 text-xs text-accent-700"><option value="">添加输出通道</option>{slots.filter(item => item[0] !== 'state' && !definition.graph.outputs[item[0]]).map(item => <option key={item[0]} value={item[0]}>{item[1]}</option>)}</select><button type="button" disabled={!numericChoices.length || Object.keys(definition.graph.outputs).length >= 8} onClick={() => { let n = 1; while (definition.graph.outputs[`channel_${n}`]) n++; const id = `channel_${n}`; onChange({ ...definition, graph: { ...definition.graph, outputs: { ...definition.graph.outputs, [id]: numericChoices[0] }, channel_metadata: { ...definition.graph.channel_metadata, [id]: { label: `数值通道 ${n}` } } } }); setActive(id) }} className="min-h-10 rounded-lg border border-dashed border-accent-300 bg-white px-3 text-xs text-accent-700 disabled:opacity-40">新增数值通道</button></div>
