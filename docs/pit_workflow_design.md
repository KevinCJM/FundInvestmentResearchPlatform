...
上面的布局上线后暴露一个新问题：② 是「选哪一批数据」（下拉框），而「封版 + 版本列表」是页面**底部另一个 section**。同一件事被拆到页面两头，唯一的关联线索是我写的一句跨区指路文案：「封版不是用来选日期的——请用上面的 ① 研究日」。

**需要写跨区指路文案，本身就是布局错了的证据。** 版本管理不是 ② 的兄弟节点，是 ② 的下级。

同样的错还犯了第二次：「站在某日的可选产品域」面板也是独立 section，而且**自带一个日期框**——一个页面上两个研究日输入框，正是这一节要消除的混淆。

改成两个块，每一步自带它的支撑工具：

```
A. 研究口径（决策——你要设的东西）
   ① 站在哪一天看？   [日期] [数据最新一天] [不设研究日]
       该日可选基金 224 只，比今天的表少 1,568 只   [查看产品域明细 ▾]
         └ 产品域明细（域别切换 · 三格对比 · 示例 · 警告）—— 跟着 ① 的日期，无独立日期框
   ② 用哪一批数据？   [最新数据（未封版）▾]   [管理版本（已封 N 版）▾]
         └ 封版表单 + 版本列表（点一行即选为②的版本）
   ③ 严不严格？       (研究模式)(严格 PIT)
   将要生效：…                                    [应用到全平台]

B. 数据能力体检（诊断——你要读的东西）
   KPI 条 · 数据集 PIT 能力表 · 选中表的公告滞后分布
```

顺带删掉的东西：

- 跨区指路文案（布局说清了，不用再写）
- 「将要生效」行里重复的域预览（挪到 ① 下面——它是 ① 的后果，不是整张表单的后果）
- KPI 条里的「已封版本」（数字挪到 ② 的展开按钮上：`管理版本（已封 0 版）`），6 格变 5 格
- 产品域面板自带的日期框（一个页面只留一个研究日）

两个折叠区默认收起：常见路径是「选完就应用」，不是「封版」或「探索域」。

诊断块加了一行标题「数据能力体检 · 上面的口径能不能成立，取决于下面这些表能证明什么」——把五个松散 section 变成两个有名字的块。

## 14. 第五轮：口径没有到达产品研究

### 14.1 两个报告

1. 首页 PIT 下拉框被页面内容盖住，只露出下半截。
2. 选了数据版本「2015前数据版本。」之后，产品研究页的行情图、指标卡仍然读到 2026-09-03。

### 14.2 遮盖：层级没有分层

`Header` 是 `sticky top-0 z-30`。`position: sticky` 一定会创建层叠上下文，所以下拉框内部写多大的 `z-50` 都只是在 header 这一层内部排序；而 `StageLayout` 的阶段横幅同样是 `relative z-30`，DOM 在后，直接盖住。

页面里 `z-40`/`z-50` 同时被两类东西使用——页内下拉、粘性表头，以及本该盖住一切的全屏弹窗，两者混在同一段数值里，这才是根因。分成三层：

| 层 | 值 | 谁 |
| --- | --- | --- |
| 页内浮层 | ≤ `z-50` | 页内下拉、粘性表头、阶段导航 |
| 全站头部 | `z-[60]` | `Header` 及其下拉 |
| 全屏弹窗 | ≥ `z-[100]` | 抽屉、模态、指标面板 |

顺带修掉了粘性表头滚动时盖住全站导航的老问题。

### 14.3 没联动：路由层从来没问过口径

`产品研究` 用三个端点：`/api/instruments/products/{id}`（行情图）、`/api/instruments/products/{id}/analysis`（收益统计）、`/api/custom-indicators/evaluate` 与 `/evaluate-series`（指标卡与时序图）。前端的 `installPitOverrideFetch` 早已给每个 `/api/` 请求带上 `X-Pit-*`；后端这两个路由模块却没有一处 `resolve_request_context`——指标端点有 `as_of` 字段，但只由页面上的「截止日（可选）」填，空着就读到磁盘最后一行。

修在两个装载函数里，而不是每个面板里：一张图停在研究日、旁边的统计却读到今天，比完全没有 PIT 更糟。

* `instrument_routes._load_timeseries` / `_load_product_research_points` → 统一走 `_pit_cut`；截断后为空则报「研究日 X 及之前没有该产品的…数据」，而不是让下游抛一个看不懂的 422。
* `custom_indicator_routes.pit_as_of(stated)` → `stated or 平台研究日`。显式的截止日仍然赢：那个框是本次运行的覆盖，回测扫 `as_of` 也不能被夹住。

口径按 `indicator_service.market_data_dir` 解析，不是模块自己拼一个 `DATA_DIR`：数据从哪个目录来，口径就从哪个目录读，否则单测把 tmp 目录的数据配上开发机的 `pit_settings.json`，结果随机。

### 14.4 版本 ≠ 截断日

用户把意图写进了封版备注：「使用二零一五年一月一日之前的数据」。但 `available_through` 是这批文件能回答到哪一天（2026-09-01），不是要截在哪一天。版本回答「用哪一份历史」，研究日回答「站在哪一天」——§13.2 的两个旋钮。名字叫「2015前数据版本」不会让它变成 2014 的截断。

所以产品研究页的「截止日（可选）」下加一行：空着不等于「用全部数据」，它跟随平台研究日。不说出来，被截断的数字看起来像 bug。

## 15. 第六轮：把两个概念合成一个

### 15.1 用户的反驳

> 版本和研究日有什么区别？我觉得就应该只有一个 PIT 的逻辑吧，为什么你拆成了两个概念？我制定了一个 PIT 版本，在这个版本里面设置了我是站在哪一天去看数据的……以后我在其他的页面应用这个 PIT 版本，那么这个页面里面的其他的数据，就应该截止到这一天为止。

理论上两者确实是两个时钟：研究日按可得时间切行，版本决定"用哪一份历史来回答"（供应商回溯修订过的净值，同一个研究日在上周和本周会给出两个答案）。

但在本系统里，§12.4 已经承认：**封版不物理冻结文件，读的还是活文件**。所以两个旋钮里有一个没有可观测效果——用户看到的是两个设置，其中一个不干活。§13 把研究日独立出来解决了"没地方填日期"，却把这个不对称暴露成了 UX 债。

### 15.2 新模型

**一个版本 = 一整套口径**：站在哪一天（`as_of`）+ 用哪一批数据（表指纹）+ 有多严格（`run_mode`）。

* `data_releases.json` 的每个 release 新增 `as_of` 与 `run_mode`，封版时定义，`as_of` 不得晚于这批数据的可得截止日（单向约束，唯一的约束）。
* `pit_settings.json` 只存 `active_release_id`；`as_of`/`run_mode` 退化为**逃生通道**——没封任何版本时看某一天用，应用版本后写 `null`，由版本回答。
* `parse_view_override`：顶栏切换只带版本 id，日期和模式跟着版本走，不再由标签页重复携带（重复就会漂移）。
* 四个时钟在内部照旧。回测仍然显式传 `as_of` 扫日期，一个版本一个日期的担心不存在。

向后兼容：`as_of` 缺失的旧版本回落到 `available_through`，也就是它当时被应用的口径；`run_mode` 缺失的旧版本不会把严格系统口径悄悄放松（`parse_view_override` 的系统兜底仍在）。
...
研究口径（系统级 · 全平台生效）
  用哪个 PIT 版本？
  [不用 PIT · 使用全部磁盘数据 ▾ | 站在 2014 年末的投前基线 · 站在 2014-12-31 · 严格]
  [新建版本 / 管理（已有 N 个）▾]
      站在哪一天看？[日期][数据最新一天]     ← 这个版本的研究日
      有多严格？(研究模式)(严格 PIT)
      名称 [__] 备注 [__]  [创建版本]
      版本表：版本 / 站在哪一天 / 模式 / 创建时间 / 表数 / 行数 / 指纹
  站在 2014-12-31，可选基金 224 只，比今天的表少 1,568 只  [查看产品域明细 ▾]
  将要生效：…                                     [应用到全平台]
```

顶栏下拉里每个版本从"研究模式 / 严格 PIT"两个按钮收成一个：版本自带模式。全页只剩一个日期框，在新建版本的表单里。

### 15.4 快照指标与 PIT

快照指标（`instrument_metrics_snapshot.parquet`）是用**全部已下载数据**算出来的，表里没有 `as_of`，因此一旦研究日生效它就是错的答案。

两处快照入口本来就有 `if as_of is None` 的门槛——但在 §14 之前从来没有人把研究日传进去，所以门槛形同虚设。§14 把平台研究日接上之后，两条路径自动改走实时计算：

| 面 | 行为 |
| --- | --- |
| 单产品研究 `evaluate` | 有研究日 → 跳过快照，实时算 |
| 评价方案 `run_plan` | 有研究日 → 跳过快照，整个融合批次都实时算 |
| 概览 `/api/instruments/products` | 仍读快照（筛选面，不是研究结论），但响应新增 `pit.snapshot_is_hindsight`，页面上打黄条说明"未按研究日重算" |

回归锁：`test_a_research_day_never_answers_from_the_hindsight_snapshot` 把 `_evaluate_from_snapshot` 换成 `pytest.fail`，两条路径都跑一遍；`test_evaluate_inherits_the_platform_research_day` 锁住路由层确实问了口径，且显式截止日仍然赢。

## 16. 规模是算出来的，所以它也受 PIT 约束

### 16.1 原来错在哪

页头「当前规模」读的是 `instrument_metrics_snapshot.parquet` 的最后一行。快照表没有 `as_of`，所以任何历史研究日下它都报今天的规模。规模不是维表属性（§12.4 说的 `LATEST_ONLY` 不适用于它）——它是 `份额 × 单位净值`，和收益率一样是算出来的量。

### 16.2 数据在，但只有 6 个交易日

`etf_share_size_df.parquet`：1653 只 ETF，**2026-08-27 → 2026-09-03**，9894 行。字段 `total_share`（万份）、`nav`、`close`、`total_size`（数据源直接给出的份额×净值）。

下载器 `fetch_etf_share_size` 支持任意区间并在触顶时二分，所以历史是可回补的；当前文件只是一次增量刷新的结果。

口径澄清：规模用**单位净值**，不是收盘价。`322102.19 × 1.0541 = 339527.92` 正好等于数据源的 `total_size`；乘收盘价得 336596.8，对不上。份额×收盘价是这批份额的**市值**，和规模（净资产）是两回事。

### 16.3 加了什么

| 位置 | 内容 |
| --- | --- |
| `pit/catalog.py` | 声明 `etf_share_size`，事件日 `trade_date`，无公告日 → **B 级**（按事件日当天可得处理） |
| `series_provider.py` | 新增 `share` 数据集（仅 ETF）→ `etf_share_size_df.parquet` |
| `variable_registry.py` | 变量 `total_share`（总份额，`\mathbf{s}`）、`fund_size`（规模，`\mathbf{S}`），均 conditional，仅 ETF |
| `scale_indicator.py` | 内置指标 `builtin-fund-size-latest` 规模（期末）= `last(fund_size)`、`builtin-fund-share-latest` 份额（期末）= `last(total_share)` |
| `instrument_routes._size_as_of` | 有研究日时页头改读源文件并按事件日截断，无研究日时仍走已校验快照 |

`fund_size` 取数据源同排的 `total_size`，而不是自己把两个变量乘一遍：同一行里份额和净值已经对齐，跨数据集再对齐一次只多一个失败点。

实测（510900.SH，`ALL` 区间）：

```
as_of=None        规模=336564.58 (2026-09-03)   ← 页面原来显示的 33.66 亿
as_of=2026-09-01  规模=339527.92 (2026-09-01)   ← 33.95 亿
as_of=2026-08-28  规模=340687.49 (2026-08-28)
```

`1W` 区间在研究日下返回 `INSUFFICIENT_SAMPLE`——磁盘上只有 6 天，截断后不足一周。这是诚实答案，不是 bug；回补份额历史后自然消失。

### 16.4 PIT 审计改成后台执行

审计要读每个声明文件的两个时间列。冷缓存下 `fund_nav_df.parquet`（3373 万行 / 1.2GB，还挂在外置卡上）要几分钟，而它原来就坐在 `GET /api/pit/audit` 的请求线程里——设置页因此可以卡几分钟。

* `audit_all(data_dir, scan=False)` 只答已测量的部分，未测量的返回 `pending: True` 且 `grade: None`（给一个没打开过的文件判 C 级，看起来像是量过的结论）。
* `start_scan(data_dir)` 起一个 daemon 线程；重复调用汇入正在跑的那次，因为两个线程读同一批 1GB 文件比一个更慢。
* 路由先返回缓存内容，再在后台启动扫描，payload 带 `scan.state` 与 `summary.pending`；前端在 `running` 期间每 4 秒轮询，表头显示「正在后台扫描 N 个数据集…扫完自动刷新」。

实测：`scan=False` **0.01 秒**返回（原先分钟级），后台扫完后 `pending: 0`，A/B/C = 3/3/4。

## 17. 版本可编辑、可删除

### 17.1 一个版本有两半，只有一半该被冻住

| 半 | 内容 | 可改？ |
| --- | --- | --- |
| 口径 | 名字、备注、站在哪一天、多严格 | **可改**。研究日填错就得重新钉 1.5GB 指纹，这不是完整性，是摩擦 |
| 证据 | 表清单与各表指纹 | **不可改**。一个 vintage 能被改写的版本什么也证明不了 |

`immutable: True` 指的一直是下半；`update()` 只重写上半，不碰任何 fingerprint。改动会盖 `updated_at`，版本表上显示「口径已改于 …」——口径变了要看得见，不能悄悄变。

编辑时研究日的上限是**这个版本自己记录的** `available_through`，不是今天审计出来的：版本回答的是它封住的那批数据，那批数据不会因为磁盘长了而变长。

### 17.2 删除的唯一守卫

删掉正在被全平台使用的版本，会让每个页面静默退回无 PIT，而没人被告知。所以 `DELETE /api/pit/releases/{id}` 对当前生效的版本返回 **409**：

> 版本「X」正在被全平台使用；请先切到别的版本或「不用 PIT」并应用，然后再删除。

先切再删，是用户的决定，不是提示框里的唠叨。`parent_release_id` 指向已删除的 id 不做级联——它是溯源痕迹，不是外键。

### 17.3 前端

版本表每行加「编辑 / 删除」（两个按钮 `stopPropagation`，否则点它们会顺带把这一版选为生效版本）。编辑把该行读回**同一个表单**——另开一个编辑对话框就是第二处要跟同样四个字段保持一致的地方；按钮随之变成「保存修改」，旁边多一个「取消」，被编辑的行标黄。删除前 `window.confirm` 说明后果。

## 18. 第七轮：产品池绕开了口径

### 18.1 同一个方案，两条入口，两个口径

在「评价方案」页点运行，前端不传截止日，路由走 `pit_as_of(None)` → 继承平台研究日。把同一个方案挂到「产品池」上，走的却是 `product_pools/service.py` → `IndicatorEvaluationGateway.run_plan`：表单里的「评价截止日」原样透传，空着就是 `None`，一路读到磁盘最后一行。站在 2015 的研究日筛池子，排名用的是 2026 的数据。

根因不是产品池忘了传参，而是**网关本身是第二条入口**，而 `pit_as_of` 当时是 `custom_indicator_routes` 的模块私有函数，只有那一条路由用得上。所以修在网关里，`pit_as_of` 升为共享函数，而不是在产品池服务里再写一遍日期兜底——那样第三条入口出现时还会再错一次。

显式的日期照旧赢，所以「回放到某一天」（`replay_version` 传 `target`）不受影响。

### 18.2 顺带修好的那条警告

`version_data_as_of()` 取的是各 binding 的 `as_of` 最大值，也就是这一版成分表的「知识日」；`_with_pit()` 拿它和研究日比，才能说出「这份成分表是未来知识」。截止日空着时这个字段也是空的，比较无从发生——警告不是错了，是从来没有数据可比。网关补上口径之后，binding 记下的就是真实用到的那一天，`_with_pit` 的前视检查才第一次有输入。

### 18.3 审核剩下的两处，明知未做

* **严格模式与版本不落到计算**：`pit_as_of` 只取 `as_of`。`run_mode` 只在 `require_usable` 的数据集闸门里生效，`data_release_id` 在生产代码里没有读取点（`pit/frame.read_pit` 目前只有测试在用）。这是 §15.2 「一个版本 = 一整套口径」承认过的既有缺口，不在本轮范围。
* **投资域快照的 `research_date` 必填**：由用户直接给，不与平台研究日比对，也不告警。填一个晚于平台研究日的日期，只有 `_with_pit` 会在版本详情上提示，创建时不会。

## 19. 第八轮：构建大类的三处漏口

审计对象是手动与自动两条构建大类路径的**全部输入**，不只净值。

### 19.1 净值是对的，另外三类输入不是

| 输入 | 读取路径 | 审计结果 |
|---|---|---|
| 净值 | `load_adj_nav_pit` | ✅ 两条路径都对；`fit.py` 里除 NAV 外没有任何裸读 |
| 合同分类信息 | `_load_instrument_metadata` → 裸 `read_parquet` | ❌ 最新态 |
| 指标画像 | `_load_metric_features` → 裸 `read_parquet` | ❌ 未声明、无任何时间列 |
| 可投资域快照 | `InvestableUniverseMembership.validate` | ❌ 校验了成员资格，没比对 `research_date` |

### 19.2 合同分类：`require_usable` 查的是资格，不是内容

`require_usable` 只看数据集的**等级**，不负责重放。而 `etf_info` 一旦有了历史快照就从 C 升到 B，
于是严格模式放行——读的却仍是最新态整表。**严格 PIT 的承诺在这条路径上是空的。**

这不是标签问题：`rule` 算法**完全**在这张表上分类，`blockBy` 的分层结构也从它来。
某只基金 2024 年从债券型改成混合型，2018 年的 run 会把它归进错的块，簇本身就变了。

改成经 `read_pit` 读（有快照就回放，没有就报 `LATEST_ONLY`），并给 `fund_info_df.parquet`
补上 catalog 声明与 `save_dataframe(..., history=True)`——它一直是 `etf_info` 的孪生表，
却既没有历史快照也不在声明里，整个场外基金域的过去无从还原。

### 19.3 指标画像：没有时间轴的数据不能配研究日

`instrument_metrics_snapshot.parquet` 是一产品一行、就地重算，没有可得时间列也没有快照日志。
把它和研究日配在一起不是「降级的 PIT 读」，是**前视**：2026 年的三年最大回撤在给 2021 年分类。
严格模式直接拒；研究模式点名，且只在 `features` 真的读它时才提——相关性 run 不该被它连坐。

### 19.4 可投资域：守卫早就写好了，只是没人接线

`pit/guard.py` 的 `check_universe` 正是为「用 2026 数据筛出的池子替 2018 做决策」写的，
`product_pool_routes` 和 `strategy_routes` 都调了，两条构建大类路径都没调。
而 `auto_class_preview` 手里什么都有：`universe_reference["research_date"]` 与 `context`
前后差十几行。手动侧则是后端接了 `universe_snapshot_id`、前端从来没传——
页面上明明显示着「已锁定可投资域 · 研究日期 XXXX」，落盘的血缘却是 `None`。

新增 `product_pools.membership.universe_pit_lineage()` 作两条路径共用的判定点，
接在**决策点**上：自动预览、手动拟合、手动保存。滚动相关性与风险预算是同一批大类的派生诊断，
不再重复同一句话。

### 19.5 展示

`PitProvenance` 增加渲染 `universe.findings`——产品域的时点问题不在任何一条公式里，
只在候选集合里，所以印在口径旁边，两个页面同时受益。手动构建大类此前**完全没有**口径展示，
`FitResponse` 也没有 `pit` 字段；顺带删掉 `app.py` 里那份未注册的 `fit_classes` 影子副本，
它正是两个口径能各自漂移的原因。

### 19.6 §18.3 两处遗留的现状

* `pit/frame.read_pit` 不再只有测试在用：合同分类信息现在经它读。`data_release_id` 仍无读取点。
* 投资域快照的 `research_date` 现在会在**构建大类的决策点**与研究日比对（严格拒、研究告警）；
  创建快照时仍不比对。

## 20. 第九轮：SAA 的三处漏口

构建大类修完之后，往下就是 SAA——有效前沿、组合构建、回测。
这一轮审计的结论跟上一轮相反：**净值读取那一层是真的做对了，问题在它前后。**

### 20.1 做对的那一层

`backtest_engine.load_allocation_nav` 是这条链路的唯一读入口，五个端点共用：
按研究日挑 `as_of` 最新的那版序列、按 `available_at` 砍掉当时尚未公告的行、
全历史序列标成 `hindsight_series`。`backtest_portfolio` 在每个调仓日走
`visible_at(full_nav, d0, available_at)` 重新拟合，并行排程的 worker 也带着可得时间过去。
回测是整条链上覆盖最好的一步。

### 20.2 有效前沿一条产品域判定都没有

同一份配置、同一个严格 PIT 口径，两个端点两个答案：

```
STRICT 回测      : HTTP 400 配置「demo」的大类净值只有最新态，无法还原研究日当时的可选集合…
STRICT 有效前沿  : 200 OK
```

`assert_no_universe_lookahead` 就在隔壁文件，前沿没调。而前沿才是**真正选权重的那一步**：
`max_sharpe` 那个点用户点一下就变成策略了。

### 20.3 锁定的产品池，保存之后再没人看过

`asset_alloc_info.parquet` 里存着 `universe_snapshot_id`，
全仓库只有 `/api/allocation-lineage` 读它出来显示。
所有守卫判的 `established_at` 都是 **`series_as_of`——大类净值被"算出来"的那天**，
而不是**产品被"筛出来"的那天**。两者可以差好几年：

```
STRICT 回测通过。established_at = 2023-12-01（净值的研究日），clean = True
实际锁定的池子研究日 = 2026-01-01
```

讽刺的是下游 TAA 自己把这个字段读回来做了四条检查（`tactical_allocation/data.py`，
含「可投资域的研究日在基线研究日之后，含事后产品筛选」）——SAA 自己反而没有。

**修法在共用读入口，不在端点。** `load_allocation_nav` 现在回读
`universe_snapshot_id`，并调 `allocation_universe_lineage()` 把**两个时间声明**
（净值口径日 + 池子研究日）合成**一个裁决**。五个端点因此同时拿到同一个答案；
回测在此之上再用调仓日扫一遍，因为它的决策日比研究日更早，所以更严。

两个声明必须合成一个裁决而不是两个血缘块：分成两块，读的人看到干净的那块就不往下看了。
为此把 `universe_pit_lineage` 里"查快照"的部分拆成 `universe_reference()`——只查不判。

### 20.4 组合构建的权重不带口径

`/compute-weights` 与 `/compute-schedule-weights` 只返回 `{weights, execution}`。
净值是按 PIT 读的，但权重拿出来之后看不出是哪天、哪个产品域算的。
用户读的是权重，采用的是权重，很可能根本不跑回测——而回测是唯一印这件事的地方。
两处都补上 `pit`。

### 20.5 回测算出来的产品域结论，前端扔了

后端返回了 `pit.universe.findings` 与 `pit.decision_clock`；
前端 `PitAllocationLineage` 类型里这两个字段都不存在，`PitDecisionNotice` 只渲染 `warnings`。
研究模式下的 `UNIVERSE_LOOKAHEAD`——算了、传了、没显示。

现在 `PitDecisionNotice` 渲染 findings，并多印一个徽标：
有池子就印「产品域研究日 XXXX」，没有就印「未记录产品域」——
**"不知道"必须跟"干净"长得不一样**。有效前沿此前一行口径都没渲染，现在跟回测用同一个组件。

### 20.6 `app.py` 里 522 行 SAA 影子代码

`post_efficient_frontier`、`api_compute_weights`、`api_backtest`、
`api_compute_schedule_weights`、`api_default_start`、`rolling_corr`
全都标着注释「Registered by services.X」，但既无装饰器也无人 import，
而且**每一个都在裸读 `asset_nv.parquet`、零 PIT**。
跟 §19.5 删掉的 `fit_classes` 是同一个病：两份口径能各自漂移，就是因为有第二份。
连同随之失效的导入一起删，`app.py` 从 1450 行降到 901 行。
