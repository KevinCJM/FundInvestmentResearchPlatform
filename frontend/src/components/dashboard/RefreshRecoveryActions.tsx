...
export default function RefreshRecoveryActions({ status, locked, rebuilding, recoverableCandidate, canResume, onResume, onRebuild }: {
  ...
  if (status?.job.analytics_snapshot?.status === 'failed') {
    return <section aria-label="恢复数据处理" className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-950">
  ...
  return <section aria-label="恢复上次下载" className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-950">
