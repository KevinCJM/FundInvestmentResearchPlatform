...
function SeriesTable({ page }: { page: RegimeSeriesPage }) {
  ...
  return <section aria-label="节点预览数据表" className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="space-y-2 p-3 md:hidden" data-testid="regime-result-mobile-list">{page.items.map((row) => <article key={`${row.date}-${String(row.state_id ?? '')}`} className="rounded-lg bg-slate-50 p-3"><p className="text-xs font-bold text-slate-800">{row.date}</p><dl className="mt-2 grid grid-cols-2 gap-2">{columns.slice(1).map((column) => <div key={column} className="min-w-0"><dt className="truncate text-xs text-slate-600">{COLUMN_LABELS[column] || column}</dt><dd className="truncate text-xs font-semibold text-slate-800">{valueText(row[column])}</dd></div>)}</dl></article>)}</div><div className="hidden max-h-64 overflow-auto md:block"><table className="min-w-full text-left text-xs" aria-label="节点预览抽样数据"><thead className="sticky top-0 bg-slate-50 text-slate-600"><tr>{columns.map((column) => <th scope="col" key={column} className="whitespace-nowrap px-3 py-2 font-semibold">{COLUMN_LABELS[column] || column}</th>)}</tr></thead><tbody>{page.items.map((row, index) => <tr key={`${row.date}-${index}`} className="border-t border-slate-100">{columns.map((column) => <td key={column} className="max-w-64 truncate whitespace-nowrap px-3 py-2 text-slate-700">{valueText(row[column])}</td>)}</tr>)}</tbody></table></div></section>
}
