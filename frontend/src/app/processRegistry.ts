...
const productResearch = withAccent({
  id: 'product-research',
  order: 1,
  label: '产品研究',
  ...
  nodes: [
    ...
    { id: 'product-backtest', label: '产品择时研究', description: '用可编辑规则研究 ETF 入场与退出，比较样本外、年度和月度表现，并保存研究版本。', path: '/product-research/timing', status: 'partial' },
...
const preInvestment = withAccent({
  id: 'pre-investment',
  order: 2,
  label: '投前决策',
  ...
  tools: [
    ...
    { label: '产品择时版本', description: '查看与引用已冻结的择时研究版本；不自动交易或改变组合权重。', path: '/pre-investment/product-allocation-timing/timing' },
  ],
})

const investmentExecution = withAccent({
...
const portfolioCenter = withAccent({
  id: 'portfolio-center',
  label: '组合中心',
...
const fundAccounting = withAccent({
  id: 'fund-accounting',
  label: '基金会计',
...
const postInvestment = withAccent({
  id: 'post-investment',
  order: 4,
  label: '投后管理',
...
const portfolioSolutions = withAccent({
...
const settings = withAccent({
  ...
  nodes: [
    ...
    { id: 'scenario-algorithms', label: '情景算法中心', description: '历史情景识别与模拟分开；从事件、宏观变量或市场冲击构建已发布情景，复用风险模型进行产品和组合压测。', path: '/settings/scenario-algorithms', status: 'available' },
