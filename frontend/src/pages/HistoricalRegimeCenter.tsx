        ...
        return (
          <article key={(audit.source_kind || 'calculation') + auditIndex} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-950 p-4 text-white lg:flex-row lg:items-start lg:justify-between">
              <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-accent-300">{sourceLabel}</p><h4 className="mt-1 text-base font-bold">{nodes.length ? 'typed AST → DAG → NJIT 固定签名' : '固定签名 NJIT 内核链'}</h4><p className="mt-1 break-all font-mono text-xs text-slate-600">{String(plan.compiled_plan_id || '未返回 plan id')}</p></div>
