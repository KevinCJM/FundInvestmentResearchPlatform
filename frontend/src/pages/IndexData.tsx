...
export default function DataQuality() {
  ...
  const attentionTone: QualityTone = criticalIssueCount > 0 || deepQuality?.status === 'blocked' ? 'danger' : issues.length > 0 ? 'warning' : loading ? 'planned' : 'good';
  ...
  return (
    <div className="mx-auto min-w-0 max-w-7xl space-y-6">
      ...
      <section aria-label="数据质量概览" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <QualityMetric label="受影响产品" value={summary ? integerFormatter.format(summary.affected_products) : '--'} detail={summary ? `占已检查 ${integerFormatter.format(summary.total_products)} 个产品的 ${formatPercent(summary.affected_rate)}` : '等待产品级检查结果'} tone={loading ? 'planned' : summary?.affected_products ? 'warning' : 'good'} statusLabel={loading ? '检查中' : undefined} />
        <QualityMetric label="净值突变" value={summary ? integerFormatter.format(summary.nav_anomaly_products) : '--'} detail={summary ? `共 ${integerFormatter.format(summary.nav_anomaly_events)} 个异常点，使用参考净值交叉确认` : '等待净值连续性结果'} tone={loading ? 'planned' : summary?.nav_anomaly_products ? 'warning' : 'good'} statusLabel={loading ? '检查中' : undefined} />
