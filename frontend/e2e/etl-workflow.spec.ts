...
test('user composes cross-source steps, controls order, validates and saves a workflow', async ({ page }, info) => {
  ...
  const upstream = await last.getByLabel('添加等待完成', { exact: true }).locator('option').evaluateAll(options => options.find(option => (option as HTMLOptionElement).text.includes('取值'))?.getAttribute('value'))
  await last.getByLabel('添加等待完成', { exact: true }).selectOption(upstream!)
