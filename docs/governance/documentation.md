...
影响归属同时读取候选和比较基线的模块/文档目录，删除源码、测试、配置或撤销模块登记不能消除原契约的审核义务。工作区与暂存检查以 HEAD 为基线，PR 检查以 merge-base 为基线；首次提交前没有基线时只使用当前目录。审核指纹绑定基线提交，基线变化后须重新核对。计划表按同一规则去除行首尾空白，合法的 1–3 空格缩进不能绕过状态和证据检查；代码块中的示例仍不作为活动计划。
