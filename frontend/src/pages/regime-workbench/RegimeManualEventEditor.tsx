...
export default function RegimeManualEventEditor({ value, onChange }: { value: unknown; onChange: (events: ManualHistoricalEvent[]) => void }) {
  ...
  return <section aria-label="人工历史事件区间" className="space-y-3 rounded-xl border border-accent-200 bg-accent-50/40 p-3">
    {selectionError && <p role="alert" className="text-xs text-rose-700">{selectionError}</p>}
    <button type="button" onClick={() => setSelecting(true)} className="min-h-10 rounded-lg border border-accent-300 bg-white px-3 text-sm font-semibold text-accent-700">从事件库选择</button>
    <div className="flex flex-wrap items-start justify-between gap-2">
      ...
      <button type="button" disabled={events.length >= 100} onClick={() => onChange([...events, newEvent(events.length)])} className="min-h-10 rounded-lg bg-accent-600 px-3 text-xs font-bold text-white disabled:opacity-40">添加事件</button>
    </div>
    {!events.length ? <div className="rounded-lg border border-dashed border-accent-200 bg-white p-4 text-center text-xs text-slate-600">尚未定义事件。点击“添加事件”，填写事件名称和日期区间。</div> : null}
    <div className="space-y-3">{events.map((event, index) => {
      ...
      return <article key={event.id || index} className="rounded-xl border border-slate-200 bg-white p-3" aria-label={`历史事件 ${index + 1}`}>
        ...
        {event.library_reference && <div className="my-2 rounded-lg bg-accent-50 p-2 text-xs text-accent-800">事件库 v{event.library_reference.revision} · 已锁定窗口；库更新不改变本项。<button type="button" onClick={() => { const { library_reference: _ref, ...local } = event; onChange(events.map((e, i) => i === index ? { ...local, id: newEvent(index).id } : e)) }} className="block min-h-9 underline">转为人工副本后编辑</button></div>}
        <div className="mt-2 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs font-semibold text-slate-600">颜色<span className="mt-1 flex min-h-10 items-center gap-2 rounded-lg border border-slate-300 px-2"><input aria-label={`事件${index + 1}颜色`} type="color" disabled={Boolean(event.library_reference)} value={/^#[0-9a-f]{6}$/i.test(event.color) ? event.color : '#7c3aed'} onChange={e => patch(index, { color: e.target.value })} className="h-7 w-10 cursor-pointer border-0 bg-transparent p-0" /><span className="font-mono font-normal text-slate-600">{event.color}</span></span></label>
    ...
    <p className="text-xs leading-5 text-slate-600">日期采用闭区间；边界可以不是交易日，运行时按观察序列中实际落入区间的观测点统计。多个事件重叠时不会互相覆盖或自动合并。</p>
  </section>
}
