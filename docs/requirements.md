...
## 1. 背景与目标
- 构建覆盖「ETF/公募基金宇宙 → 产品研究 → 资产大类构建 → 策略回测」的一站式投研工具，缩短资产配置策略的验证周期。
...
| 角色 | 诉求 | 典型操作 |
| --- | --- | --- |
| 量化研究员 | 查看 ETF/公募基金宇宙、定义研究指标、构建资产大类、验证再平衡策略 | 编写受限 LaTeX 指标、验证 DAG、生成调仓表、执行回测并导出结果 |
| 产品经理 | 分析 ETF 与公募基金、了解费用与发行、筛选潜在产品 | 在仪表盘查看趋势、在产品库按产品类别筛选过滤 |
...
| 公募基金产品检索 | 场外公募基金多维过滤、排序、分页、关键字搜索 | `ProductResearch` 页面 / `/api/instruments/products?kind=fund` | 已上线 |
| 公募基金详情与对比 | 展示元信息、费用和真实复权净值，可进行多产品比较 | `ProductDetail`、`ProductCompare` / `/api/instruments/products/{id}?kind=fund` | 已上线 |
...
| 统一产品搜索 | 供资产配置时选择 ETF 或场外公募基金 | `ManualConstruction` 页面 / `/api/instruments/search?kind=all` | 已上线 |
| 模块化 Tushare 刷新 | 基础信息、ETF、公募基金模块；增量/全量模式 | `Dashboard` 页面 / `/api/data/refresh` | 已上线 |
| 自定义研究指标 | 受限 LaTeX、DAG 校验、真实产品预览、版本化工作区共享 | `IndicatorStudio` / `/api/custom-indicators/*` | 已上线 |
| 产品指标消费 | 在产品研究、详情和对比中复用已保存指标 | `ProductResearch`、`ProductDetail`、`ProductCompare` | 已上线 |
| 真实评价方案 | 锁定指标版本、严格缺失策略、方向归一化、加权排名 | `EvaluationPlan` / `/api/evaluation-plans/*` | 已上线 |
...
- **数据文件**：
  ...
  - 公募基金：`data/fund_info_df.parquet`（产品宇宙）、`data/fund_nav_df.parquet`（场外复权净值）。
  - 基金公司：`data/fund_company_df.parquet`。
  ...
  - 研究配置：`data/custom_indicators.json`、`data/evaluation_plans.json` 为实例内共享运行时数据，使用 revision、锁文件和原子替换保证并发安全。
- **数据更新流程**：
  - 主链路仅使用 Tushare-only 的 `T01_get_data.py` 更新 parquet，避免在代码层面硬编码路径或凭证。
  - 常用命令：
...
- **外部集成**：
  - Tushare Token 由用户在主界面“数据管理”中配置，后端仅保存到本机忽略文件 `data/.tushare_token`；接口不得回传或记录明文。
  - 未来若对接第三方行情源，需提供兼容的 parquet schema，并在 README 中补充说明。

## 5. 非功能需求
- **性能**：常用 API（`/api/etf/analytics`、`/api/strategy/compute-weights`、`/api/strategy/backtest`）在常规数据量（近 10 年日度数据、几十只资产）下响应时间应控制在 1~2 秒内；自定义指标按需计算最多支持 500 个指标-产品组合，并使用数据指纹 TTL 缓存；回测允许更长，但需反馈进度状态。
...
- **安全性**：禁止在仓库提交密钥；CORS 在生产环境需收紧；日志中避免输出敏感字段；指标公式只允许受控 AST、变量和算子，不允许 `eval`、任意 Python、文件或网络访问。
...
5. **工作区权限**：项目尚无登录体系，自定义指标和评价方案为实例级共享资源；个人/团队隔离需在引入可信身份后建设。
...
- 自定义指标的计算、持久化与交互契约见 [`custom_indicator_design.md`](custom_indicator_design.md)。
