...
export default function RegimeNodeSeriesChart({ page, onSelectComparisons }: { page: RegimeSeriesPage; onSelectComparisons?: () => void }) {
  ...
  const overlays = page.overlay_outputs || page.upstream_outputs || []
  const [selected, setSelected] = useState<Record<string, Placement>>(() => Object.fromEntries((page.comparison_targets || []).filter(ref => overlays.some(item => outputKey(item) === outputKey(ref) && item.plottable)).map(ref => [outputKey(ref), 'subplot'])))
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      ...
      setPlotHeight(Math.round(height))
      // Updating grid options alone does not resize ECharts' canvas. Bind the
      // renderer to its actual container, including fullscreen viewport changes.
      const instance = chartInstance.current
      if (instance && !instance.isDisposed()) instance.resize({ width, height, silent: true })
    })
  ...
  useEffect(() => {
    ...
    for (const key of keys) {
      ...
      const target = (page.overlay_outputs || page.upstream_outputs)?.find(item => outputKey(item) === key)
      ...
      void getAllRegimePreviewSeries(page.run_id, target.node_id, target.port, controller.signal).then(next => {
        ...
        if (next.value_type !== target.value_type) throw new Error('对比数据类型与本次预览不一致，请重新预览。')
        cache.current[key] = next
        setLoads(previous => ({ ...previous, [key]: { page: next } }))
      }).catch(reason => {
        if (!controller.signal.aborted) setLoads(previous => ({ ...previous, [key]: { error: reason instanceof Error ? reason.message : '读取对比数据失败。' } }))
      })
    }
  ...
  const layers = useMemo<Layer[]>(() => [
    ...
    ...(page.overlay_outputs || page.upstream_outputs || []).filter(item => selected[outputKey(item)]).map((item, index) => ({
  ...
  const option = useMemo<EChartsOption | null>(() => {
    ...
    const panelHeight = (plotHeight - 60) / panels.length
    ...
    return {
      animation: false,
      title: panels.map((layer, index) => ({ text: `${index ? '' : '主图 · '}${layer.label}${layer.rebased ? '（首日＝1）' : ''}`, top: index * panelHeight, left: 58, textStyle: { color: layer.color, fontSize: 11, width: 210, overflow: 'truncate' } })),
      ...
      grid: panels.map((_, index) => ({ left: 58, right, top: 26 + index * panelHeight, height: panelHeight - 34 })),
  ...
  }, [main, curves, subplots, dates, startIndex, endIndex, plotHeight])

  if (!option) return null
  const mainCurve = curves[0]
  const mainResult = mainCurve.response?.result
  const mainEnd = mainCurve.indices.get(range.end)
  const options = overlays.filter(item => `${item.node_label} ${item.port_label} ${item.node_id}`.toLowerCase().includes(search.toLowerCase()))
  ...
  const onFullscreenKey = (event: KeyboardEvent<HTMLDialogElement>) => {
    if (event.key === 'Escape') {
      event.preventDefault(); event.stopPropagation(); setFullscreen(false)
    } else if (event.key === 'Tab') {
      event.stopPropagation()
      const controls = [...event.currentTarget.querySelectorAll<HTMLElement>('button:not([disabled]), input:not([disabled]), select:not([disabled]), summary, [tabindex="0"]')].filter(element => element.getClientRects().length > 0)
  ...
  const chart = <section aria-label="节点走势图" className="flex min-w-0 flex-1 flex-col gap-3">
    ...
    <div ref={plotRef} className="relative flex-1" style={{ minHeight: (fullscreen ? 240 : 320) + subplots.length * 200 }} data-testid="regime-node-plot">
      <div className="absolute inset-0"><ReactECharts option={option} onEvents={onEvents} onChartReady={(instance: ECharts) => { chartInstance.current = instance; instance.resize({ silent: true }) }} style={{ height: '100%' }} notMerge lazyUpdate aria-label="历史情景节点结果图" /></div>
    </div>
  ...
  return fullscreen ? createPortal(<dialog ref={dialogRef} aria-label="节点图表全屏" onCancel={event => { event.preventDefault(); setFullscreen(false) }} onKeyDown={onFullscreenKey} className="fixed inset-0 m-0 flex h-[100dvh] max-h-none w-screen max-w-none flex-col border-0 bg-white p-0 text-slate-900 backdrop:bg-slate-950/30">
    <header className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
      <h2 className="min-w-0 truncate text-sm font-bold">{page.node_label || page.node_id || '节点'} · 走势图</h2>
      <button autoFocus type="button" onClick={() => setFullscreen(false)} className="min-h-10 shrink-0 rounded-lg border border-slate-300 px-3 text-sm font-semibold">退出全屏</button>
    </header>
    <div className="flex min-h-0 flex-1 flex-col overflow-auto p-3 sm:p-4">{chart}</div>
  </dialog>, document.body) : chart
}
