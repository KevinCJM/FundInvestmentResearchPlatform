...
export function MetricResultCard({
  ...
  return <article className="rounded-xl border border-slate-200 bg-white p-4">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs text-slate-500">{presentation.category_label} · {presentation.source === 'built_in' ? '内置' : '工作区'} v{presentation.revision}</p><h3 className="mt-1 font-semibold text-slate-900">{presentation.name}</h3></div><div className="flex items-center gap-2">{result && <MetricStatus status={result.status} warnings={result.warnings} />}{onRemove && <button type="button" onClick={onRemove} aria-label={`移除指标 ${presentation.name}`} title="仅从当前页面移除，不会删除指标定义" className="inline-flex min-h-9 items-center rounded-lg border border-slate-200 px-2.5 text-xs font-medium text-slate-500 hover:border-rose-300 hover:bg-rose-50 hover:text-rose-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500">移除</button>}</div></div>
