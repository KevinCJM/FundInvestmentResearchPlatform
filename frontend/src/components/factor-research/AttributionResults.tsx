...
function RegressionOverview({ run }: { run: AttributionRun }) {
  ...
  return <div className="overflow-x-auto">
    <table className="w-full min-w-[760px] text-left text-sm" aria-label="暴露与拟合摘要">
      <thead className="text-xs text-slate-500"><tr><th className="p-2">产品</th><th>风格 / 因子暴露</th><th>{rolling ? '末次窗口 R²' : '样本内 R²'}</th><th>{rolling ? '前推样本外 R²' : '样本外 R²'}</th><th>年化截距</th><th>样本外残差波动</th><th>拟合 / 样本外日数</th></tr></thead>
      <tbody>{run.results.map(row => <tr key={row.code} className="border-t border-slate-100">
        <th className="p-2 font-medium">{row.name}<span className="block text-xs text-slate-500">{row.code}{row.reason && ' · ' + row.reason}</span></th>
    ...
    {rolling && <p className="mt-2 text-xs leading-6 text-slate-500">此摘要展示末次计划拟合的系数；下方逐日贡献使用各日实际适用的系数，而非用末次系数回填历史。</p>}
  </div>
}
...
export default function AttributionResults({ run }: { run: AttributionRun }) {
  ...
  return <div className="min-w-0 space-y-5" aria-label="收益归因结果">
    <Card title={run.name + ' · 风格与收益贡献'}>
      <p className="mb-3 break-words text-xs leading-6 text-slate-500">已保存运行 · {run.request.model} · {run.request.start_date}—{run.request.end_date} · 样本外从 {run.request.oos_date} 开始。来源：{run.request.dataset_id || '本地指数代理'}。</p>
      <RegressionOverview run={run} />
      <p className="mt-3 text-xs leading-6 text-slate-500">{run.warnings.join(' ')}</p>
    ...
    {analysis && product && summary && <>
      <Card title="查看归因对象与区间">
        ...
        <p className="mt-2 text-xs leading-6 text-slate-500">{analysis.dependent_return === 'excess' ? '拟合使用超额收益；贡献对账时单列 RF，恢复至基金实际总收益。' : '拟合使用总收益，不单列 RF；模型截距不是风险调整 Alpha。'} 系数不等同真实持仓比例。</p>
      </Card>
      <Card title="累计贡献与实际收益对账">
        ...
        ].map(([label, value]) => <div key={label} className="rounded-lg bg-slate-50 p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-2 break-words text-lg font-bold tabular-nums text-slate-900">{value}</p></div>)}</div>
        <p className="mt-3 text-xs leading-6 text-slate-500">对账闭合不代表模型有效；残差也参与对账。样本外解释度与残差波动 {percentText(summary.residual_volatility)} 需要一起看。</p>
        <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[340px] text-left text-sm" aria-label="累计因子贡献"><thead><tr><th className="p-2">组成</th><th>累计收益贡献</th></tr></thead><tbody>{analysis.components.map((item, i) => <tr key={item.id} className="border-t border-slate-100"><th className="p-2 font-medium">{item.label}</th><td>{contributionText(summary.contributions[i])}</td></tr>)}</tbody></table></div>
      </Card>
      <Card title="风格暴露随时间变化">
        <p className="mb-3 text-xs leading-6 text-slate-500">纵轴是回归系数，不是收益或实际持仓比例。估计窗口与原始因子收益可在逐日明细和 CSV 中核对；无法估计的日期断开。</p>
      ...
      <Card title="各组成的累计收益贡献">
        ...
        <p className="text-xs leading-6 text-slate-500">按每日基金期初财富串联贡献，而非直接求和或分别复利各因子。负贡献如实保留。</p>
      </Card>
      ...
      <Card title="导出与研究边界">
        ...
        <p className="mt-3 text-xs leading-6 text-slate-500">CSV 包含所选产品全区间的暴露、因子收益、贡献和状态；数值为原始小数，0.01代表1个百分点，缺失留空。</p>
        {error && <p role="alert" className="mt-3 text-sm text-rose-700">{error}</p>}
        {analysis.notes.map(note => <p key={note} className="mt-2 text-xs leading-6 text-slate-500">{note}</p>)}
      </Card>
    </>}
  </div>
}
