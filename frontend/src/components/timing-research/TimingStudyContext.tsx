...
export function TimingBasketEditor({ value, onChange, definition }: { value: BasketDraft; onChange: (value: BasketDraft) => void; definition: TimingDefinition }) {
  ...
  return <details open={required || undefined} className="border-t border-slate-100 pt-3"><summary className="cursor-pointer py-2 text-sm font-medium text-slate-700">参考 ETF 篮子{required ? ' · 当前算法需要' : ' · 可选'}</summary><p className="mt-2 text-xs leading-5 text-slate-500">只用于广度和横截面条件，不代表全市场。不会自动使用研究对象充当篮子，不合成投资组合。</p><div className="mt-3 space-y-3">{(['market', 'category'] as const).map(group => <label key={group} className="block text-xs text-slate-600">{group === 'market' ? '市场参考篮子代码' : '同类参考篮子代码'}<textarea aria-label={group === 'market' ? '市场参考篮子代码' : '同类参考篮子代码'} className={timingField} rows={3} spellCheck={false} placeholder="逗号或换行分隔，如 510300.SH" value={value[group]} onChange={event => onChange({ ...value, [group]: event.target.value })} /><span className="mt-1 block text-xs text-slate-500">已填 {basketCodes(value[group]).length} 个 · 每篮子 2–12 个 ETF</span></label>)}</div><p className="mt-2 text-xs leading-5 text-slate-500">不自动取日期交集，不补零；缺失保留未知。结果中可按 ETF 成员预览篮子步骤。不同篮子先各自汇总，不能直接逐列混算。</p></details>
}
