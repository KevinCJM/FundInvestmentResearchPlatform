...
export default function RegimeFormulaEditor({ definition, mode, schemas, outputId, onApply, onPending, onBusy }: Props) {
  ...
  return <section aria-label="情景高级公式编辑器" className="mx-auto max-w-6xl space-y-4 p-4">
    <div><h2 className="text-lg font-bold text-slate-900">高级公式</h2><p className="mt-1 text-xs leading-5 text-slate-600">每行定义一个计算步骤，连接写为“节点.端口”。支持指标通用函数与数学表达式；检查并应用后同步到画布和向导。</p><p className="mt-2 rounded-lg bg-accent-50 p-3 text-xs text-accent-900">例如：<code>smooth = rolling_mean(market.value, 20, 20)</code>。再用分类算子把计算结果映射为市场状态。</p></div>
    ...
    <div className="flex flex-wrap items-center gap-3"><button type="button" disabled={!loaded || busy || stale} onClick={() => void apply()} className="rounded-lg bg-accent-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-40">{busy ? '正在检查…' : '检查并应用公式'}</button>
      ...
      <span className="text-xs text-slate-600">{dirty ? '存在未应用公式，保存和运行已暂停。' : '公式与当前方案同步。'}</span></div>
