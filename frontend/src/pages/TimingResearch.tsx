...
const button = 'min-h-10 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40'
const primary = 'min-h-10 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-40'
...
export default function TimingResearch({ mode = 'research' }: { mode?: Mode }) {
  ...
  return <main className="mx-auto w-full min-w-0 max-w-[1600px] space-y-5 p-3 sm:p-5 lg:p-6">
    <header className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><h1 className="text-2xl font-semibold tracking-tight text-slate-950">{title}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">选择 ETF，组合计算与买卖规则，用真实历史数据检验效果。</p>{mode === 'application' && <p className="mt-1 text-xs leading-5 text-slate-500">研究版本引用不自动交易、不改变组合权重；改变大类配置比例时进入 TAA 决策。</p>}</div><span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700">日频 ETF · 研究用途</span></header>
    ...
    {!catalog && !error && <p role="status" className="p-8 text-sm text-slate-500">正在读取算法目录…</p>}
    {catalog && tab !== 'releases' && <button type="button" aria-expanded={settingsOpen} aria-controls="timing-study-settings" className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left lg:hidden" onClick={() => setSettingsOpen(value => !value)}><span className="min-w-0"><strong className="block text-sm text-slate-800">研究设置 · {targets.length} 个 ETF</strong><span className="text-xs text-slate-500">{startDate} 至 {endDate}</span></span><span className="shrink-0 text-xs text-indigo-700">{settingsOpen ? '收起' : '展开修改'}</span></button>}
    {catalog && <div className={`grid min-w-0 items-start gap-5 ${tab === 'releases' ? '' : 'lg:grid-cols-[280px_minmax(0,1fr)]'}`}>
      {tab !== 'releases' && <aside id="timing-study-settings" className={`${settingsOpen ? 'block' : 'hidden'} min-w-0 space-y-5 rounded-2xl border border-slate-200 bg-white p-4 lg:block`}>
        <section className="space-y-3"><h2 className="text-sm font-semibold text-slate-900">1. 选择研究对象</h2><div className="flex flex-wrap gap-2">{targets.map(target => <span key={target.product_id} className="inline-flex max-w-full items-center gap-1 rounded-lg bg-slate-100 py-1 pl-2 text-xs text-slate-700"><span className="truncate" title={target.name}>{target.name}</span><button type="button" aria-label={`移除 ${target.product_id}`} className="min-h-8 min-w-8 rounded-lg hover:bg-slate-200" onClick={() => { version.current += 1; setTargets(targets.filter(item => item.product_id !== target.product_id)) }}>×</button></span>)}</div>
          ...
          {!!products.length && <ul aria-label="ETF 搜索结果" className="max-h-56 overflow-auto rounded-lg border border-slate-200">{products.map(item => { const code = item.code || item.ts_code || ''; return <li key={code}><button type="button" className="min-h-11 w-full px-3 py-2 text-left text-xs hover:bg-indigo-50 disabled:opacity-40" disabled={!code || targets.some(target => target.product_id === code)} onClick={() => addTarget(code, item.name || code)}>{item.name || code}<span className="ml-2 text-slate-400">{code}</span></button></li> })}</ul>}
          {searchError && <p className="text-xs text-amber-700">{searchError}</p>}
          {/^[0-9]{6}\.(SH|SZ)$/i.test(search.trim()) && !targets.some(target => target.product_id === search.trim().toUpperCase()) && <button type="button" className="min-h-9 text-xs text-indigo-700" onClick={() => addTarget(search.trim().toUpperCase())}>添加代码 {search.trim().toUpperCase()}</button>}
          <p className="text-xs text-slate-400">最多 12 个 ETF，分别检验；不合成为投资组合。</p>
          <label className="block text-xs text-slate-600">研究开始日<input type="date" className={timingField} value={startDate} onChange={event => { version.current += 1; setStartDate(event.target.value) }} /></label><label className="block text-xs text-slate-600">研究结束日<input type="date" className={timingField} value={endDate} onChange={event => { version.current += 1; setEndDate(event.target.value) }} /></label><label className="block text-xs text-slate-600">样本外开始日<input type="date" className={timingField} value={holdout} onChange={event => { version.current += 1; setHoldout(event.target.value) }} /></label><p className="text-xs leading-5 text-slate-500">在前段构建规则，查看后段表现。反复查看后调整参数，会消耗这段样本的独立性。</p>
        </section>
        <section className="space-y-3 border-t border-slate-100 pt-4"><h2 className="text-sm font-semibold text-slate-900">2. 选择起点</h2><label className="block text-xs text-slate-600">内置模板<select className={timingField} value={templateId} onChange={event => applyTemplate(event.target.value)}><option value="">自建算法</option>{catalog.templates.map(template => <option key={template.id} value={template.id}>{template.label}</option>)}</select></label>{templateId && <p className="text-xs leading-5 text-slate-500">{catalog.templates.find(template => template.id === templateId)?.description}</p>}
          <label className="block text-xs text-slate-600">已保存算法<select className={timingField} value={saved?.id || ''} onChange={event => { const item = definitions.find(value => value.id === event.target.value); if (item) { change(currentDefinition(item)); setSaved(item); setTemplateId(''); setTab('editor') } }}><option value="">选择已保存算法</option>{definitions.map(item => <option key={item.id} value={item.id}>{item.name} · v{item.revision}</option>)}</select></label><button type="button" className="min-h-9 text-xs font-medium text-indigo-700" onClick={() => { change(emptyDefinition()); setSaved(undefined); setTemplateId(''); setTab('editor') }}>从空白开始</button>
        </section>
        <TimingBasketEditor value={basketDraft} definition={definition} onChange={next => { version.current += 1; setBasketDraft(next) }} />
        <details className="border-t border-slate-100 pt-3"><summary className="cursor-pointer py-1 text-sm font-medium text-slate-700">交易与价格设置</summary><p className="mt-2 text-xs leading-5 text-slate-500">下一交易日开盘执行；买入当天不卖出。同日同时触及止盈止损时，按止损处理。</p><div className="mt-3 space-y-3"><label className="block text-xs text-slate-600">价格口径<select className={timingField} value={priceBasis} onChange={event => { version.current += 1; setPriceBasis(event.target.value as TimingRequest['price_basis']) }}><option value="hfq">后复权（研究收益）</option><option value="qfq">前复权（截止日锚定）</option><option value="raw">原始交易价格</option></select></label>{([
      ...
      <div className="min-w-0 space-y-4">
        <div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          <div id="timing-workspace-panel" role="tabpanel" className="mt-5 min-w-0">
            ...
            {tab === 'editor' && <div className="space-y-4"><TimingAdaptationNote adaptation={definition.adaptation} /><details><summary className="cursor-pointer text-xs text-slate-500">算法说明</summary><textarea className={timingField} rows={2} maxLength={2000} value={definition.description} onChange={event => change({ ...definition, description: event.target.value })} placeholder="记录你的研究假设和适用范围" /></details><TimingTrainingEditor definition={definition} catalog={catalog} onChange={change} /><TimingRuleEditor definition={definition} catalog={catalog} onChange={change} /></div>}
            {tab === 'results' && (run ? <>{stale && <p role="status" className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">配置已修改。下方为上次运行结果，请重新运行后再保存研究版本。</p>}<TimingResults key={run.id} run={run} /><div className="mt-5 space-y-3 border-t border-slate-200 pt-4"><label className="block text-xs text-slate-600">研究引用说明<input className={timingField} value={note} onChange={event => setNote(event.target.value)} placeholder="例如：仅用于宽基 ETF 的研究对照" /></label><div className="flex flex-wrap gap-2"><button type="button" className={button} disabled={!!busy || stale} onClick={() => publish()}>保存研究版本</button><button type="button" className={button} disabled={!!busy || stale} onClick={() => publish(true)}>引用到投前研究</button></div><p className="text-xs text-slate-500">研究引用锁定本次结果和适用范围，不自动生成交易。</p></div></> : <div className="rounded-xl border border-dashed border-slate-300 p-10 text-center"><h2 className="text-sm font-medium text-slate-700">还没有研究结果</h2><p className="mt-2 text-sm text-slate-500">检查左侧产品与日期，调整算法后点击“运行研究”。</p></div>)}
            {tab === 'history' && <div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-slate-600">选择历史结果查看，或勾选 2–4 次研究比较。</p><button type="button" className={button} disabled={!!busy || comparisonIds.length < 2} onClick={compare}>比较选中研究</button></div>{!history.length && <p className="rounded-xl bg-slate-50 p-6 text-sm text-slate-500">运行研究后，历史记录会保存在这里。</p>}<div className="space-y-2">{history.map(item => <div key={item.id} className="flex items-center gap-3 rounded-xl border border-slate-200 p-3"><input type="checkbox" aria-label={`对比 ${item.name} ${item.id}`} className="h-4 w-4 shrink-0" checked={comparisonIds.includes(item.id)} disabled={!comparisonIds.includes(item.id) && comparisonIds.length >= 4} onChange={event => setComparisonIds(previous => event.target.checked ? [...previous, item.id] : previous.filter(id => id !== item.id))} /><button type="button" className="min-w-0 flex-1 text-left" disabled={!!busy} onClick={() => loadRun(item.id)}><strong className="block truncate text-sm text-slate-800">{item.name}</strong><span className="text-xs text-slate-400">{item.created_at?.replace('T', ' ').slice(0, 19)}</span></button><button type="button" className="min-h-9 shrink-0 px-2 text-xs text-indigo-700" disabled={!!busy} onClick={() => loadRun(item.id)}>查看</button></div>)}</div>{comparison && <div className="overflow-auto rounded-lg border border-slate-200"><table aria-label="研究对比" className="w-full text-left text-xs [&_td]:whitespace-nowrap [&_td]:border-t [&_td]:p-3 [&_th]:whitespace-nowrap [&_th]:bg-slate-50 [&_th]:p-3"><thead><tr><th>研究</th><th>产品</th><th>样本外收益</th><th>最大回撤</th><th>已平仓交易</th></tr></thead><tbody>{comparison.items.flatMap(item => item.products.map(product => <tr key={`${item.run_id}:${product.product_id}`}><td>{item.name}</td><td>{product.product_id}</td><td>{timingPercent(product.summary?.out_of_sample.total_return)}</td><td>{timingPercent(product.summary?.out_of_sample.max_drawdown)}</td><td>{timingNumber(product.summary?.out_of_sample.trade_count, 0)}</td></tr>))}</tbody></table></div>}</div>}
          </div>
        </div>
      </div>
    </div>}
