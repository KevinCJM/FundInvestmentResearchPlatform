...
export default function TimingTrainingEditor({ definition, catalog, onChange }: { definition: TimingDefinition; catalog: TimingCatalog; onChange: (next: TimingDefinition) => void }) {
  ...
  return <section aria-label="训练与方案选择" className="rounded-xl border border-slate-200 bg-white p-4">
    ...
    <p className="mt-1 text-xs leading-5 text-slate-600">{training ? `${trainingModeLabels[training.mode]}；${training.actions.length} 个动作。` : '关闭时直接执行下方买入条件，不做自动选择。'} 只使用样本外开始前已完成的信号；之后冻结，不滚动重训。</p>
    {training && <details className="mt-3"><summary className="cursor-pointer py-2 text-sm font-medium text-accent-700">编辑训练规则与候选动作</summary><div className="mt-3 space-y-4">
      ...
      <details><summary className="cursor-pointer py-2 text-xs font-medium text-slate-600">参数搜索空间 · {training.search_space.length ? `${training.search_space.length} 个维度` : '不搜索'}</summary><p className="mt-2 text-xs leading-5 text-slate-600">同一方案的参数一起替换，不同维度组合。仅在训练期比较；非空仓动作 × 参数组合最多 108 个。</p><div className="mt-3 space-y-3">{training.search_space.map((dimension, dimensionIndex) => <div key={dimensionIndex} className="space-y-3 rounded-lg border border-slate-200 p-3">
