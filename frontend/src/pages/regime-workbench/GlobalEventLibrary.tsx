...
export default function GlobalEventLibrary({ onSelect, onCancel }: { onSelect?: (events: ManualHistoricalEvent[]) => void; onCancel?: () => void }) {
  ...
  return <section aria-label={onSelect ? '选择库中历史事件' : '全球历史事件库'} className="min-w-0 space-y-4 rounded-xl border border-slate-200 bg-white p-3 sm:p-5">
    ...
    <div className="grid min-w-0 gap-4 lg:grid-cols-2">
      <div className="min-w-0"><div className="mb-2 text-xs text-slate-600">{loading ? '正在读取…' : `共 ${page.total} 个事件`} · 列表按主要研究窗口倒序</div>
        {loading ? <p role="status" className="p-5 text-sm">正在读取事件库…</p> : !page.items.length ? <p className="rounded-lg border border-dashed p-5 text-sm text-slate-600">没有匹配事件。可新建事件，或从已保存的人工情景导入。</p> : <div className="max-h-[65vh] space-y-2 overflow-y-auto" aria-label="事件列表">{page.items.map(event => <article key={event.id} className={'rounded-xl border p-3 ' + (detail?.id === event.id ? 'border-accent-400 bg-accent-50/40' : 'border-slate-200')}><button type="button" onClick={() => show(event)} className="min-h-10 w-full break-words text-left text-sm font-semibold">{event.name} <span className="text-xs font-normal text-slate-600">v{event.revision}</span></button><p className="text-xs text-slate-600">{event.windows[0].start_date} — {event.windows[0].end_date}</p><div className="mt-2 flex flex-wrap gap-1 text-xs"><span className={'rounded-full px-2 py-1 ' + (event.verification === 'verified' ? 'bg-emerald-50 text-emerald-800' : 'bg-amber-50 text-amber-800')}>{event.verification === 'verified' ? '已核验' : '待核验'}</span>{event.archived && <span className="px-2 py-1">已归档</span>}{event.categories.map(c => <span key={c} className="rounded-full bg-slate-100 px-2 py-1">{eventCategories[c] || c}</span>)}</div><button type="button" disabled={event.archived || busy} aria-pressed={chosen(event, event.windows[0].id)} onClick={() => toggle(event, event.windows[0].id)} className="mt-2 min-h-9 text-xs font-semibold text-accent-700 disabled:opacity-40">{chosen(event, event.windows[0].id) ? '已选择 · 移除主要窗口' : '选择主要窗口'}</button></article>)}</div>}
      ...
      <div className="min-w-0">{editing ? <EventLibraryEditor key={editing === 'new' ? 'new' : editing.id + ':' + editing.revision} current={editing === 'new' ? undefined : editing} onCancel={() => setEditing(null)} onSaved={event => { setEditing(null); show(event); setRefresh(n => n + 1); setNotice('事件已保存为新修订，既有情景不变。') }} /> : detail ? <article aria-label="事件详情" className="space-y-4 rounded-xl border bg-slate-50 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="break-words font-semibold">{detail.name}</h3><button type="button" disabled={busy} onClick={() => setEditing(detail)} className="min-h-10 text-xs font-semibold text-accent-700">编辑这个版本</button></div>
        ...
        <p className="whitespace-pre-wrap break-words text-xs leading-6">{detail.description || '暂无背景说明。'}</p><dl className="grid gap-3 text-xs sm:grid-cols-2"><div><dt className="text-slate-600">事实日期</dt><dd>{detail.fact_start || '尚未核实'} — {detail.fact_end || '结束未知'}</dd></div><div><dt className="text-slate-600">信息公开时间</dt><dd className="break-words">{detail.known_at || '未知（不能据此构造实时信号）'}</dd></div><div><dt className="text-slate-600">地区</dt><dd>{detail.regions.join('、') || '未分类'}</dd></div><div><dt className="text-slate-600">来源状态</dt><dd>{detail.verification === 'verified' ? '已人工核验' : '待核验，不是官方标准区间'}</dd></div></dl>
        <section><h4 className="text-sm font-semibold">研究窗口</h4><div className="mt-2 space-y-2">{detail.windows.map(w => <div key={w.id} className="rounded-lg bg-white p-3"><p className="text-xs font-semibold">{w.label}</p><p className="mt-1 text-xs">{w.start_date} — {w.end_date}</p><p className="mt-2 break-words text-xs leading-5 text-slate-600">{w.rationale}</p><button type="button" disabled={busy || detail.archived} aria-pressed={chosen(detail, w.id)} onClick={() => toggle(detail, w.id)} className="mt-2 min-h-9 text-xs font-semibold text-accent-700">{chosen(detail, w.id) ? '已选择 · 移除此窗口' : '选择此窗口'}</button></div>)}</div></section>
        <section><h4 className="text-sm font-semibold">参考来源</h4>{detail.sources.length ? detail.sources.map((s, i) => <a key={i} href={/^https?:\/\//.test(s.url) ? s.url : undefined} target="_blank" rel="noopener noreferrer" className="mt-2 block break-words text-xs text-accent-700 underline">{s.title}</a>) : <p className="mt-2 text-xs text-amber-800">尚无核实的来源链接。原人工说明仅作为待核验线索。</p>}</section>
