...
export default function EtlRunOptionsEditor({ definition, value, onChange, disabled }: {
  ...
  const overrides = definition.steps.filter(step => step.kind === 'download' && step.mode !== 'inherit')
  return <fieldset disabled={disabled} className="space-y-3 rounded-xl border border-indigo-200 bg-indigo-50 p-4" aria-label="本次运行设置">
    ...
    <label className="block text-sm font-semibold">本次运行模式<select aria-label="本次运行模式" className={inputClass} value={value.mode} onChange={event => onChange({ ...value, mode: event.target.value as EtlRunOptions['mode'] })}>
      <option value="incremental">增量更新：从成功断点补充，无断点则初始化</option>
      <option value="full">全量重取：重新下载指定范围，不删除已有数据</option>
    </select></label>
    <p className="text-xs leading-5 text-slate-600">同一个流程可用两种模式运行。映射、取值和快照步骤保持不变；快照按完整输入重新计算，不是假装只算新增几天。</p>
    {overrides.length ? <p className="text-xs leading-5 text-slate-600">固定策略步骤：{overrides.map(step => `${step.name}（${step.mode === 'full' ? '每次全量刷新' : '固定增量'}）`).join('、')}。</p> : null}
    <div className="grid gap-3 sm:grid-cols-2">{(definition.parameters ?? []).map(parameter => <label key={parameter.id} className="text-xs font-semibold">{parameter.label}{parameter.required ? '（必填）' : ''}
