...
点击草稿保存后，先由服务端持久化确认预览，再用原生确认框展示冻结名称、完整公式、产品/周期/研究日和校验状态。同名冲突须在公式前明确提示“新增而非覆盖”；用户单独确认后才发送业务保存请求，取消不写入。已经明确确认且结果不确定的重试继续沿用原确认与请求 ID，不重新预览；确认失效后须重新展示影响并由用户确认。
