...
2026-09-11 存储接入增补：数据区不再绑定 worktree 绝对路径。旧 `{id, project}` 标记只校验数据 ID、不改写原标记；新标记 `schema_version=2` 只允许 `state=ready` 接入。页面“使用已有数据目录（不复制）”及 `python scripts/manage_data_storage.py attach --path <已有目录> --confirm <数据ID>` 保存并离线应用本项目连接，不复制或重新下载共享数据。本项目原数据另行保留，不合并、不自动清理。多实例在数据区共用 `.fund-storage-use.lock` 使用锁、原 `.tushare_refresh.lock` 下载锁和 `data_sources.sqlite3` 配额；未知格式、掉盘或身份变化均失败关闭。未升级进程不具备新锁协议，启用共享前须统一升级；旧任务执行指纹检查继续生效，不因接入而允许原地续跑。此次不改供应商接口、字段、限频、超时、重试、Parquet schema、检查点或发布合同，无真实供应商请求；仅离线存储与原有下载链路回归，不能称为新增数据已下载。详见 `docs/data_storage_design.md`。
