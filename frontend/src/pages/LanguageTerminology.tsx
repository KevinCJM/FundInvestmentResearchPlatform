...
const input = 'min-h-11 w-full min-w-0 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400'
const button = 'min-h-10 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40'
...
export default function LanguageTerminology() {
  ...
  return <div className="min-w-0 space-y-5" data-testid="language-terminology">
    <header className="rounded-2xl border border-slate-200 bg-white p-5"><h1 className="text-2xl font-bold text-slate-900">{s('i18n.title')}</h1><p className="mt-2 text-sm leading-6 text-slate-600">{s('i18n.description')}</p>
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <label className="text-sm font-semibold">{s('i18n.language')}<select aria-label={s('i18n.language')} className={`${input} mt-1`} disabled={cellEditing || busy || languageManager} value={storedLocale() ? resolveActiveLocale(storedLocale(), state?.default_locale ?? DEFAULT_LOCALE) : ''} onChange={event => void chooseLocale(isLocale(event.target.value) ? event.target.value : null, state?.default_locale ?? DEFAULT_LOCALE)}><option value="">{s('i18n.followWorkspace')}</option>{languages.filter(item => item.enabled).map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select><span className="mt-2 block text-xs font-normal leading-5 text-slate-500">{s('i18n.browserHint')}</span></label>
        <details className="rounded-xl border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-semibold">{s('i18n.defaultLanguage')}</summary><p className="my-2 text-xs leading-5 text-slate-500">{s('i18n.defaultHint')}</p><div className="flex gap-2"><select aria-label={s('i18n.defaultLanguage')} className={input} value={defaultLocale} disabled={busy || !state || cellEditing || languageManager} onChange={event => setDefaultLocale(event.target.value as Locale)}>{languages.filter(item => item.enabled).map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select><button type="button" className={button} disabled={busy || !state || cellEditing || languageManager} onClick={() => void saveDefault()}>{s('common.save')}</button></div></details>
      </div><p className="mt-3 text-xs leading-5 text-slate-500">{s('i18n.scopeWarning')}</p>
    </header>
    ...
    <div className="flex flex-wrap gap-2" role="tablist" aria-label={s('i18n.tabs')}>{(['system', 'business', 'history'] as const).map(value => <button key={value} id={`i18n-tab-${value}`} aria-controls="i18n-panel" type="button" role="tab" aria-selected={tab === value} disabled={cellEditing || busy || languageManager} className={`${button} ${tab === value ? '!border-violet-500 !bg-violet-50 !text-violet-800' : ''}`} onClick={() => switchTab(value)}>{s(`i18n.${value}`)}</button>)}</div>
    <section id="i18n-panel" role="tabpanel" aria-labelledby={`i18n-tab-${tab}`} className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
