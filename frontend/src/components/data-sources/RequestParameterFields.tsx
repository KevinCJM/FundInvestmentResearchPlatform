...
export default function RequestParameterFields({ fields, values, bindings = {}, onChange }: {
  ...
  return <div className="grid gap-3 sm:grid-cols-2">{fields.map(field => {
    ...
    return <label key={field.name} className="text-xs font-semibold">{field.label}
      {bindings[field.name] ? <span className="mt-2 block rounded-lg bg-accent-50 p-3 text-accent-800">运行时填写：{bindings[field.name]}</span>
        : field.data_type === 'choice' ? <>
          <select aria-label={field.label} className={inputClass} value={value}
          ...
          {(field.choices ?? []).find(choice => choice.value === value)?.description
            ? <span className="mt-1 block font-normal leading-5 text-slate-600">{(field.choices ?? []).find(choice => choice.value === value)!.description}</span>
            : null}
        </> : <input aria-label={field.label} className={inputClass} type={field.data_type === 'text' ? 'text' : field.data_type}
