...
const field = 'mt-1 min-h-11 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400'
...
export default function ScalarOutputEditor({ draft, validation, disabled, onPatch }: {
  ...
  return <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-label={s('primitiveAuthoring.result', {}, '结果设置')} data-testid="indicator-output-settings">
    <h2 className="font-semibold text-slate-900">{s('primitiveAuthoring.result', {}, '结果设置')}</h2>
    <p className="mt-1 text-xs leading-5 text-slate-500">{s('primitiveAuthoring.singleHint', {}, '一个指标对应一个结果。分别选择多个指标时，系统自动复用相同的计算步骤。')}</p>
