...
前端不能把点击保存视为对尚未返回的影响预览的批准。`commit-preview` 持久化冻结结果后，原生确认框展示该结果的名称、全部公式、计算口径和校验状态；同名冲突优先展示，并说明将新增指标、不覆盖已有指标。用户在该确认框再次明确确认后才生成已确认的幂等请求并调用 `commit`；取消不调用业务写入。旧保存回执的恢复与已确认请求的幂等重试继续保留。
