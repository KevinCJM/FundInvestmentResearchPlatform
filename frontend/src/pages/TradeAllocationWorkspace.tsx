...
export default function TradeAllocationWorkspace() {
  ...
  return <div className="space-y-5" data-testid="trade-allocation-workspace">
    <StaticDemoBanner />
    <header className="rounded-2xl bg-gradient-to-r from-amber-900 via-orange-800 to-slate-900 p-6 text-white shadow-sm">
      ...
      <p className="mt-2 max-w-5xl text-sm leading-6 text-amber-50/90">多个组合及基金经理的独立指令可通过同一执行通道汇总成交；成交后必须按预分配规则拆回组合、投资单元和专用结算账户。</p>
    </header>

    <section className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">共享执行通道</p>
        <h3 className="mt-2 font-semibold text-slate-950">{executionAccount.name}</h3>
        <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
          <div><dt className="text-slate-500">账户编号</dt><dd className="mt-1 font-mono font-semibold text-slate-900">{executionAccount.accountId}</dd></div>
          <div><dt className="text-slate-500">法定归属</dt><dd className="mt-1 font-mono font-semibold text-slate-900">{executionAccount.legalOwnerEntityId}</dd></div>
          <div><dt className="text-slate-500">共享规则</dt><dd className="mt-1 font-semibold text-indigo-700">{executionAccount.sharingPolicy}</dd></div>
          <div><dt className="text-slate-500">业务边界</dt><dd className="mt-1 text-slate-700">只汇总执行，不承载共享基金资产</dd></div>
        </dl>
      </article>
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">成交汇总</p>
        <h3 className="mt-2 font-semibold text-slate-950">ORD-DEMO-20260901-001 · 510300.SH 买入</h3>
        <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
          <div><dt className="text-slate-500">成交数量</dt><dd className="mt-1 text-xl font-bold tabular-nums text-slate-950">{filledQuantity.toLocaleString('zh-CN')}</dd></div>
          <div><dt className="text-slate-500">成交均价</dt><dd className="mt-1 text-xl font-bold tabular-nums text-slate-950">¥{averagePrice.toFixed(4)}</dd></div>
          <div><dt className="text-slate-500">成交金额</dt><dd className="mt-1 font-semibold tabular-nums text-slate-900">{allocationResult ? `¥${allocationResult.source_amount.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}` : allocationError || 'NJIT 计算中…'}</dd></div>
          <div><dt className="text-slate-500">分配规则版本</dt><dd className="mt-1 font-mono text-xs font-semibold text-slate-900">FAIR-ALLOC-v3</dd></div>
        </dl>
      </article>
    </section>

    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-5 py-4"><h3 className="font-semibold text-slate-900">成交分配明细</h3><p className="mt-1 text-sm text-slate-500">示例数量可修改；刷新页面后恢复，不生成订单、成交或真实 Booking。</p></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[1180px] text-sm"><thead className="bg-slate-50 text-left text-xs text-slate-500"><tr>{['分配编号', '真实组合', '基金经理任职', '内部投资单元', '事前指令数量', '成交分配数量', '分配金额', '专用资金结算账户'].map((heading) => <th key={heading} className="px-4 py-3">{heading}</th>)}</tr></thead><tbody>{initialAllocations.map((allocation, index) => {
