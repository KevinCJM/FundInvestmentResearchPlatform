...
export default function RegimeResultView({ runId, runKind = 'preview', stale = false }: RegimeResultViewProps) {
  ...
  return <section aria-label="完整历史情景结果" className="min-w-0 space-y-4 p-3 sm:p-5" data-run-id={overview.run_id}>
    ...
    <header className="flex flex-wrap items-start justify-between gap-3">
      <div><h2 className="text-lg font-bold text-slate-950">历史情景结果</h2><p className="mt-1 text-xs text-slate-500">{overview.date_range.start ?? '无样本'} 至 {overview.date_range.end ?? '无样本'} · 共 {overview.summary.total} 个{observationLabel}{overview.as_of ? ` · 截至 ${overview.as_of}` : ''} · {overview.created_at ? '运行于 ' + overview.created_at : '运行 ' + overview.run_id}</p></div>
      <span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-800">{overview.mode === 'retrospective' ? '事后解释' : '当时可知模式'} · {frequencyLabel(overview.frequency)}色带</span>
    </header>
