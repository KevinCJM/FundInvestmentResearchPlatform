...
const layerStyles: Record<DataModelTable['layer'], string> = {
  ...
  master: 'bg-accent-100 text-accent-800',
  canonical: 'bg-emerald-100 text-emerald-800',
  mart: 'bg-accent-100 text-accent-800',
}
...
function SummaryCard({ label, value, description }: { label: string; value: number; description: string }) {
  return (
    <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1 px-3 py-2" title={description}>
...
function TableBadge({ table }: { table: DataModelTable }) {
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${table.delivery_phase === 'core' ? 'bg-accent-100 text-accent-800' : 'bg-amber-100 text-amber-900'}`}>
        {phaseLabels[table.delivery_phase]}
      </span>
      {table.pit_supported ? (
        <span className="rounded-full bg-accent-100 px-2.5 py-1 text-xs font-semibold text-accent-800">支持 PIT</span>
      ) : null}
      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${table.source_mappable ? 'bg-accent-100 text-accent-800' : 'bg-slate-100 text-slate-600'}`}>
  ...
  return (
    <tr data-field-kind={field.source_mappable ? 'source' : 'system'} className={`border-t border-slate-200 align-top ${field.source_mappable ? 'bg-white' : 'bg-slate-100 text-slate-600'}`}>
      <td className="px-4 py-3">
        <p className="font-semibold text-slate-900">{field.label}{!field.source_mappable ? <span className="ml-2 inline-block rounded-lg border border-slate-300 bg-slate-200 px-2 py-0.5 text-xs font-medium text-slate-700">系统维护</span> : null}</p>
        <code className={`mt-1 block break-all text-xs ${field.source_mappable ? 'text-accent-700' : 'text-slate-600'}`}>{field.name}</code>
      </td>
      <td className="px-4 py-3">
        <code className="break-all text-xs text-slate-700">{field.data_type}</code>
        {field.unit ? <p className="mt-1 text-xs text-slate-600">单位：{field.unit}</p> : null}
      </td>
      ...
      <td className="px-4 py-3 text-sm leading-6 text-slate-600">
        ...
        {field.reference ? <p className="mt-1 break-all text-xs text-slate-600">引用：{field.reference}</p> : null}
        {field.enum_values.length ? <p className="mt-1 break-all text-xs text-slate-600">枚举：{field.enum_values.join(' / ')}</p> : null}
      </td>
    </tr>
  )
}
  ...
  return (
    <article className="min-w-0 rounded-xl border border-slate-200 bg-white shadow-sm">
      <header className="border-b border-slate-100 p-5 sm:p-6">
        ...
        <code className="mt-1 block break-all text-sm font-semibold text-accent-700">{table.table_id}</code>
      ...
      <section className="p-5 sm:p-6" aria-labelledby="field-list-title">
        ...
        {table.source_mappable && systemFields.length > 0 ? (
          <button
            ...
            className="mt-3 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-accent-500"
          >
            {showSystemFields ? '收起系统维护字段' : '高级：查看系统维护字段'}
          </button>
        ) : null}
        <p className="mt-3 flex flex-wrap gap-4 text-xs text-slate-600"><span><span aria-hidden="true" className="mr-1 inline-block h-3 w-3 rounded-xl border border-slate-300 bg-white" />白色：来源业务字段</span><span><span aria-hidden="true" className="mr-1 inline-block h-3 w-3 rounded-lg border border-slate-300 bg-slate-200" />灰蓝色：系统维护字段</span></p>
        <div id="data-model-fields" className="mt-4 overflow-x-auto rounded-xl border border-slate-200">
          <table className="min-w-[760px] w-full border-collapse text-left">
            <thead className="bg-slate-50 text-xs font-semibold text-slate-600">
              <tr>
                <th scope="col" className="px-4 py-3">字段</th>
                <th scope="col" className="px-4 py-3">类型与单位</th>
                <th scope="col" className="px-4 py-3">规则</th>
                <th scope="col" className="px-4 py-3">说明</th>
              </tr>
            </thead>
            <tbody>
      ...
      <header className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3" aria-label="数据字典范围">
          <button
            ...
            className={`min-h-11 rounded-xl px-4 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-accent-500 ${scope === 'external' ? 'bg-accent-700 text-white' : 'bg-slate-100 text-slate-700'}`}
          >
            外部导入表
          </button>
          <button
            ...
            className={`min-h-11 rounded-xl border px-4 py-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-500 ${scope === 'internal' ? 'border-slate-700 bg-slate-800 text-white' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}
          >
...
function CatalogContent({ scope }: { scope: DataModelScope }) {
  ...
  return (
    <div className="space-y-5">
      <header className="rounded-xl border border-accent-200 bg-gradient-to-br from-accent-950 via-slate-900 to-slate-950 p-4 text-white shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 className="text-lg font-bold">{scope === 'external' ? '外部数据导入标准' : '系统内部结构'}</h2>
            <details className="mt-2 text-sm leading-6 text-slate-200"><summary className="cursor-pointer">查看数据范围说明</summary><p className="mt-1 max-w-3xl">{scope === 'external' ? catalog.description : '这里仅供查看代码对照、配置、任务和计算结果等内部结构，不需要用户单独导入。'}</p></details>
            <p className="mt-2 text-xs leading-5 text-accent-200">这里展示标准结构，不表示已接入数据。数据源与字段映射可在设置中的“数据源与接口映射”配置；定时调度尚未开放。</p>
          </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm" aria-label="数据表筛选">
        <div className={`grid gap-3 ${scope === 'internal' ? 'lg:grid-cols-[minmax(0,1fr)_190px]' : ''}`}>
          <label className="text-sm font-medium text-slate-700">
            ...
            <input
              ...
              className="mt-1 block min-h-11 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:border-accent-500 focus:outline-none focus:ring-2 focus:ring-accent-500"
            />
            <span className="mt-1 block text-xs font-normal leading-5 text-slate-600">支持中文表名、英文 table_id、字段中文名和字段名。</span>
          </label>
          {scope === 'internal' ? <label className="text-sm font-medium text-slate-700">
            ...
            <select
              ...
              className="mt-1 block min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm focus:border-accent-500 focus:outline-none focus:ring-2 focus:ring-accent-500"
            >
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4" aria-label="数据模型分类">
          <button
            ...
            className={`rounded-xl border p-3 text-left transition focus:outline-none focus:ring-2 focus:ring-accent-500 ${categoryId === 'all' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-slate-50 text-slate-700 hover:border-slate-300'}`}
          >
            <span className="block text-sm font-bold">全部业务数据</span>
            <span className={`mt-1 block text-xs ${categoryId === 'all' ? 'text-slate-300' : 'text-slate-600'}`}>当前可见 {eligibleTables.length} 张表</span>
          </button>
          {catalog.categories.map((category) => {
            ...
            return <button
              ...
              className={`rounded-xl border p-3 text-left transition focus:outline-none focus:ring-2 focus:ring-accent-500 disabled:cursor-not-allowed disabled:opacity-40 ${selected ? 'border-accent-600 bg-accent-700 text-white' : 'border-accent-100 bg-accent-50 text-accent-950 hover:border-accent-200'}`}
            >
              <span className="flex items-start justify-between gap-2"><span className="text-sm font-bold">{category.label}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-bold ${selected ? 'bg-white/15 text-white' : 'bg-white text-accent-700'}`}>{count}</span></span>
              <span className={`mt-1 line-clamp-2 block text-xs leading-5 ${selected ? 'text-accent-100' : 'text-slate-600'}`}>{category.description}</span>
            </button>
          })}
        </div>
      </section>

      <section className="flex flex-wrap gap-x-3 rounded-lg bg-white" aria-label="数据模型摘要">
            ...
            {catalog.type_conventions.map((item) => (
              <div key={item.logical_type} className="rounded-xl border border-slate-100 p-3">
                <div className="flex flex-wrap items-center gap-2">
                  <code className="text-xs font-semibold text-accent-700">{item.logical_type}</code>
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="min-w-0 rounded-xl border border-slate-200 bg-white p-3 shadow-sm xl:max-h-[calc(100vh-170px)] xl:overflow-y-auto" aria-label={scope === 'external' ? '外部导入表列表' : '系统内部表列表'}>
          <div className="px-2 py-2">
            <p className="text-sm font-bold text-slate-900">{scope === 'external' ? '外部导入表' : '系统内部表'}</p>
            <p className="mt-1 text-xs text-slate-600">当前筛选 {filteredTables.length} 张；按业务分类展示。</p>
          </div>
          <div className="mt-2 space-y-4">
            {groupedTables.map(({ category, tables }) => <section key={category.category_id} aria-label={`${category.label}表`}>
              <div className="sticky top-0 z-10 flex items-center justify-between gap-2 rounded-lg bg-slate-100 px-3 py-2">
                ...
                <span className="text-xs font-semibold text-slate-600">{tables.length} 张</span>
              </div>
              <div className="mt-2 grid gap-2">{tables.map((table) => {
                ...
                return <button
                  ...
                  className={`rounded-xl border p-3 text-left transition focus:outline-none focus:ring-2 focus:ring-accent-500 ${effectiveSelectedTableId === table.table_id ? 'border-accent-300 bg-accent-50' : 'border-slate-100 hover:border-slate-300 hover:bg-slate-50'}`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className="font-semibold text-slate-900">{table.label}</span>
                    <span className={`shrink-0 rounded-lg px-1.5 py-0.5 text-xs font-semibold ${layerStyles[table.layer]}`}>{layerLabels[table.layer]}</span>
                  </div>
                  <code className="mt-1 block break-all text-xs text-slate-600">{table.table_id}</code>
                  <p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-600">{table.description}</p>
                  {fieldMatches.length ? <div className="mt-2 flex flex-wrap gap-1" aria-label="搜索命中字段">{fieldMatches.map((field) => <span key={field.name} className="rounded-lg bg-amber-50 px-1.5 py-1 text-xs font-semibold text-amber-800">{field.label} · {field.name}</span>)}</div> : null}
                </button>
              })}</div>
            </section>)}
          </div>
          {!filteredTables.length ? <p className="px-3 py-8 text-center text-sm text-slate-600">没有符合条件的表或字段。可以清空搜索或切回“全部业务数据”。</p> : null}
        </aside>

        {selectedTable ? <TableDetail key={selectedTable.table_id} table={selectedTable} /> : (
          <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-600">请选择一张数据表。</div>
        )}
      </div>
    </div>
  )
