  ...
  return <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6">
    <header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button type="button" disabled={saving} onClick={() => void savePlan()} className="min-h-11 rounded-lg bg-accent-600 px-4 text-sm font-semibold text-white disabled:bg-accent-300">{saving ? '保存中…' : '保存方案'}</button>
        <button type="button" disabled={saving || running} onClick={() => void runPlan()} className="min-h-11 rounded-lg bg-emerald-700 px-4 text-sm font-semibold text-white disabled:bg-emerald-300">{running ? '运行中…' : '保存并运行'}</button>
      </div>
    </header>
    ...
    {loading ? <div className="rounded-xl bg-white p-10 text-center text-slate-600 shadow-sm">正在加载{kindLabel}方案和指标…</div> : <>
...
function IndicatorConfiguration({ productKind, indicators, selectableIndicators, disabledReasons, entries, runtimePeriods, selectedMetricIds, weightControl, weightControlError, onSelectedMetricsChange, onEntryChange, onEntriesChange }: {
  ...
  const kindLabel = productKind === 'etf' ? 'ETF' : '场外公募基金'
  return <section className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h2 className="text-xl font-semibold text-slate-900">2. 配置{kindLabel}研究指标</h2><p className="mt-1 text-sm text-slate-600">指标目录只展示适用于当前品类的定义；输入权重运行时归一化为 100%，保存时锁定版本。</p></div><div className="flex items-center gap-3"><span className="rounded-full bg-accent-50 px-3 py-1 text-sm font-semibold text-accent-700">输入合计 {weightControl ? weightControl.total.toFixed(2) : weightControlError || '计算中…'}</span><MetricSelector indicators={selectableIndicators} selectedIds={selectedMetricIds} onChange={onSelectedMetricsChange} maxSelected={10} disabledReasons={disabledReasons} /></div></div>
    <div className="mt-4 overflow-auto"><table className="min-w-[980px] w-full text-sm"><caption className="sr-only">{kindLabel}评价指标配置</caption><thead className="bg-slate-50 text-left text-slate-600"><tr><th scope="col" className="px-3 py-3">指标定义</th><th scope="col" className="px-3 py-3">方向</th><th scope="col" className="px-3 py-3">周期</th><th scope="col" className="px-3 py-3">输入权重</th><th scope="col" className="px-3 py-3">有效占比</th><th scope="col" className="px-3 py-3">操作</th></tr></thead><tbody>{entries.map((entry, entryIndex) => {
      ...
      return <tr key={entry.id} className="border-t border-slate-100"><td className="px-3 py-3"><p className="font-medium text-slate-800">{definition ? indicatorOptionLabel(definition) : entry.indicatorId}</p><p className="mt-1 text-xs text-slate-600">{definition?.presentation?.category_label ?? definition?.category_label ?? '未分类'} · {definition?.unit || '无单位'} · 最少 {definition?.minimum_observations ?? 1} 个观察值</p>{disabledReasons[entry.indicatorId] && <p className="mt-1 text-xs text-rose-600">{disabledReasons[entry.indicatorId]}</p>}</td><td className="px-3 py-3"><select value={entry.direction} onChange={(event) => onEntryChange(entry.id, { direction: event.target.value as IndicatorDirection })} className="min-h-11 rounded-lg border border-slate-200 px-2"><option value="neutral" disabled>{systemText('scalarResult.chooseDirection', {}, '请选择评分方向')}</option><option value="higher_better">数值高优先</option><option value="lower_better">数值低优先</option></select>{overridden && <p className="mt-1 text-xs text-amber-700">已覆盖指标默认方向</p>}</td><td className="px-3 py-3"><select value={entry.period} onChange={(event) => onEntryChange(entry.id, { period: event.target.value })} className="min-h-11 rounded-lg border border-slate-200 px-2">{runtimePeriods.map((period) => <option key={period} value={period}>{indicatorPeriodOptionLabel(period)}</option>)}</select></td><td className="px-3 py-3"><input aria-label={`${definition?.name ?? '指标'}权重`} type="number" min="0" value={entry.weight} onChange={(event) => onEntryChange(entry.id, { weight: Number(event.target.value) || 0 })} className="min-h-11 w-24 rounded-lg border border-slate-200 px-2 text-right" /></td><td className="px-3 py-3 font-medium text-accent-700">{weightControl?.positive && weightControl.normalized_shares[entryIndex] !== undefined ? `${(weightControl.normalized_shares[entryIndex] * 100).toFixed(1)}%` : '—'}</td><td className="px-3 py-3"><div className="flex gap-2"><button type="button" onClick={() => onEntriesChange((current) => [...current, { ...entry, id: crypto.randomUUID(), period: runtimePeriods.find((period) => !current.some((item) => item.indicatorId === entry.indicatorId && item.period === period)) ?? entry.period }])} className="text-accent-700 hover:underline">复制周期</button><button type="button" onClick={() => onEntriesChange((current) => current.filter((item) => item.id !== entry.id))} className="text-rose-600 hover:underline">删除</button></div></td></tr>
    })}</tbody></table></div>
  </section>
}
...
function RunResultSection({ runResult }: { runResult: EvaluationPlanRunResponse | null }) {
  return <section className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
    <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between"><div><h2 className="text-xl font-semibold text-slate-900">3. 运行结果</h2><p className="mt-1 text-sm text-slate-600">总分由 0–100 归一化得分乘有效权重后求和；缺少任一值的产品不参与排名。</p></div>{runResult && <p className="text-xs text-slate-600">方案 v{runResult.plan_revision} · {new Date(runResult.run_at).toLocaleString('zh-CN')} · 截止日 {runResult.as_of ?? '最新数据'}</p>}</div>
    {runResult ? <div className="mt-4 overflow-auto"><table className="min-w-[900px] w-full text-sm"><caption className="sr-only">评价方案排名结果</caption><thead className="bg-slate-50 text-left text-slate-600"><tr><th scope="col" className="px-3 py-3">排名</th><th scope="col" className="px-3 py-3">产品</th><th scope="col" className="px-3 py-3">状态</th><th scope="col" className="px-3 py-3">总分</th><th scope="col" className="px-3 py-3">贡献明细</th></tr></thead><tbody>{runResult.rows.map((row) => <tr key={`${row.target.kind}:${row.target.product_id}`} className="border-t border-slate-100 align-top"><td className="px-3 py-3">{row.rank ?? '—'}</td><td className="px-3 py-3 font-medium text-slate-800">{row.target.name}<span className="ml-2 text-xs font-normal text-slate-600">{row.target.product_id}</span></td><td className="px-3 py-3">{row.status === 'ranked' ? <span className="text-emerald-700">已排名</span> : <span className="text-amber-700">不可排名</span>}</td><td className="px-3 py-3 font-semibold text-accent-700">{row.score?.toFixed(2) ?? '—'}</td><td className="px-3 py-3"><details><summary className="cursor-pointer font-medium text-accent-700">{row.status === 'ranked' ? '查看原始值、得分与贡献' : '查看不可排名原因'}</summary><div className="mt-3 space-y-3">{row.values.map((value) => <div key={`${value.indicator_id}:${value.period}`} className="rounded-lg bg-slate-50 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-medium text-slate-800">{value.indicator_name} · {value.period}</span><MetricStatus status={value.status} warnings={value.warnings} showReason={value.value === null && !value.input_requirements} /></div><div className="mt-2 grid gap-2 text-xs text-slate-600 sm:grid-cols-4"><span>原始值：<MetricValue value={value.value} presentation={value.presentation} /></span><span>归一化：{value.normalized_score?.toFixed(2) ?? '—'}</span><span>有效权重：{value.effective_weight === null ? '—' : `${(value.effective_weight * 100).toFixed(1)}%`}</span><span>加权贡献：{value.weighted_contribution?.toFixed(2) ?? '—'}</span></div><p className="mt-2 text-xs text-slate-600">实际窗口 {value.window?.start_date ?? '—'} 至 {value.window?.end_date ?? '—'} · {value.window?.observation_count ?? 0} 个观察值</p><MetricUnavailableReason result={value} /></div>)}</div></details></td></tr>)}</tbody></table></div> : <div className="mt-4 rounded-xl bg-slate-50 p-6 text-sm text-slate-600">保存并运行方案后，在这里查看排名、有效权重和贡献拆解。</div>}
