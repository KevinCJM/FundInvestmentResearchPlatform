...
test('研究发布、三入口情景与产品应用在各屏幕保持完整闭环', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText(/Horizon 证据：月频 × 1 期/)).toBeVisible()
  await expect(page.getByText(/窗口可能截断了情景传播或恢复过程/)).toBeVisible()
  await expect(page.getByText(/经济持续期尚未由历史事件或衰减模型独立验证/)).toBeVisible()
