...
test('滚动夏普：真实接口下构建往返、十五日编辑、样本标准差和公式预览', async ({ page }, testInfo) => {
  ...
  expect(original).toContain(String.raw`\operatorname{rolling_std}\left(\mathbf{r},5,1\right)`)
  ...
  for (let pass = 0; pass < 2; pass++) {
    ...
    await expect(drawer.getByLabel('自由度修正（ddof） 有限常量')).toHaveValue('1')
  ...
  await windows.nth(0).fill('15')
  await windows.nth(1).fill('15')
  await drawer.getByRole('button', { name: '应用逻辑修改' }).click()
  await expect(drawer).toBeHidden()
  expect(await source.inputValue()).toContain(String.raw`\operatorname{rolling_mean}\left(\mathbf{r},15\right)`)
  expect(await source.inputValue()).toContain(String.raw`\operatorname{rolling_std}\left(\mathbf{r},15,1\right)`)
  expect(await source.inputValue()).toContain(String.raw`\frac{`)
  const checked = await validate()
  expect(checked.output_inferences.value.display_latex).toContain('s_{t,15}')
  expect(checked.output_inferences.value.python_expression).toContain('rolling_std(returns, 15, 1)')
  ...
  expect(manual.output_inferences.value.python_expression).toContain('rolling_std(returns, 10, 1)')
...
test('标量夏普：载入和构建使用 LaTeX，滚动派生重新校验编辑源码', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: '高级公式模式' }).click()
  expect(await source.inputValue()).toContain(String.raw`\operatorname{rolling_std}\left(\mathbf{r},5,1\right)`)
  await expect(page.getByTestId('formula-preview').locator('.katex')).toBeVisible()
})
