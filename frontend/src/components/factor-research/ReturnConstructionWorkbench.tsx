    ...
    <form ref={form} onSubmit={event => { event.preventDefault(); save(false) }}>
      <fieldset disabled={busy || !catalog} className="min-w-0 space-y-5">
        <Card title="1 · 算法与产物">
          ...
          <p className="mt-4 rounded-lg bg-indigo-50 p-3 text-sm leading-6 text-indigo-900">{draft.method === 'characteristic_spread' ? '产物：一条特征收益差额序列。使用当时已计算的特征分组，下一交易日收盘进入，按每日真实净值变化计算持有收益，不拼接重叠的远期标签。' : '产物：MKT_RF、SMB、HML、RF。使用上一财年账面权益、上一年12月市值及6月市值独立双排序，形成六组；这是自定义市场与参考池的 FF3 风格构造。'}</p>
        </Card>
        {draft.method === 'characteristic_spread' ? <Card title="2 · 冻结特征数据与分组">
          <div className="space-y-4">
            ...
            {!runs.length && <p className="text-sm text-slate-500">还没有特征研究运行。请先在“产品特征因子 → 研究工作台”完成一次检验。</p>}
            ...
            {parent && <p className="break-all text-xs leading-6 text-slate-500">来源：{parent.study_snapshot.start_date} — {parent.as_of}；{parent.latest_scores.length} 个产品；信号频率：{({ daily: '日频', weekly: '周频', monthly: '月频' })[parent.study_snapshot.signal_frequency || 'monthly']}。<br />冻结输入：{parent.input_checksum}</p>}
            <p className="text-xs leading-6 text-slate-500">多头名义 1、空头名义 1。场外基金可能不可做空，未计借券、融资和容量；这是一条研究因子，不是自动交易策略。</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            {panel && <p className="text-xs leading-6 text-slate-500">{panel.observations} 条股票收益记录；{panel.market}/{panel.currency}。输入哈希：<span className="break-all">{panel.checksum}</span></p>}
            <details className="rounded-lg border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-semibold text-slate-700">查看输入字段与下载空模板</summary><div className="mt-3 space-y-3">{catalog && Object.entries(catalog.source_fields).map(([field, description]) => <p className="break-words text-xs leading-6 text-slate-600" key={field}><strong>{field}</strong>：{description}</p>)}<p className="text-xs leading-6 text-slate-500">B/M = 上一财年账面权益 ÷ 上年12月市值。形成日是6月最后交易日，财报须提前公告。RF 使用实际日收益，小数 0.01 表示 1%，不可直接填年化利率。收益应包含分红与退市损失。空模板不含模拟行情。</p><button className={secondaryClass} type="button" onClick={() => downloadJson('ff3-source-template.json', catalog?.source_template)}>下载股票面板空模板</button></div></details>
          </div>
        </Card>}
        <div className="flex flex-wrap items-center gap-3"><button type="submit" className={secondaryClass}>保存构建方案</button><button type="button" className={buttonClass} disabled={!catalog?.ready} onClick={() => save(true)}>保存并生成收益率</button>{current && <span className="text-xs text-slate-500">当前 v{current.revision}；再次保存会生成新修订。</span>}</div>
      </fieldset>
    </form>
    {result ? <ReturnDatasetView dataset={result} onAttribute={onAttribute} busy={busy} /> : <p className="rounded-xl border border-dashed border-slate-300 p-6 text-sm leading-6 text-slate-500">尚未生成本方案结果。生成后可查看逐日收益、统计、分组证据，并直接用于基金或 ETF 收益归因。</p>}
