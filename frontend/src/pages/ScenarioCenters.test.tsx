...
describe('ScenarioCenters', () => {
  it('人工事件深链接归属全球事件库，切换中心不携带其他工作区的版本', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '时序算法工作台内容' })).toBeVisible()
    ...
    expect(await screen.findByRole('button', { name: '草稿修改 1' })).toBeVisible()
    await user.click(screen.getByRole('button', { name: '事件库' }))
    expect(await screen.findByRole('heading', { name: '事件库管理内容' })).toBeVisible()
  ...
  it('切换模拟中心再返回时保留情景草稿', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '情景模拟与压测内容' })).toBeVisible()
    expect(screen.queryByRole('button', { name: '草稿修改 1' })).not.toBeInTheDocument()
    await user.click(screen.getByRole('tab', { name: /历史情景识别/ }))
    expect(await screen.findByRole('button', { name: '草稿修改 1' })).toBeInTheDocument()
  })
