...
export function HistoricalRegimeBacktestSelector({
  ...
  return <section className="rounded-xl border border-indigo-200 bg-indigo-50/60 p-3" aria-label="历史情景条件化">
    ...
    <select
      ...
      className="mt-2 w-full rounded-lg border border-indigo-200 bg-white px-3 py-2 text-sm text-slate-800 disabled:bg-slate-100"
    >
    ...
    {selected ? <dl className="mt-2 grid gap-1 rounded-lg bg-white p-2 text-xs text-slate-600 sm:grid-cols-2">
      <div><dt className="inline text-slate-500">运行：</dt><dd className="inline font-medium text-slate-800">{selected.run.id}</dd></div>
      <div><dt className="inline text-slate-500">发布：</dt><dd className="inline font-medium text-slate-800">{selected.publication.id}</dd></div>
    </dl> : null}
  </section>
}
...
export function RegimeConditioningPanel({ result }: { result?: RegimeConditioningResult | null }) {
  ...
  return <section className="rounded-xl border border-indigo-200 bg-indigo-50/40 p-4" aria-label="历史情景条件表现">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <span className="rounded-full bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-800">正式回测锁定版本</span>
    </div>
    <dl className="mt-3 grid gap-2 rounded-lg bg-white p-3 text-xs sm:grid-cols-2 lg:grid-cols-4">
      <div><dt className="text-slate-500">运行 ID</dt><dd className="mt-1 break-all font-semibold text-slate-800">{result.binding.run_id}</dd></div>
      <div><dt className="text-slate-500">发布 ID</dt><dd className="mt-1 break-all font-semibold text-slate-800">{result.binding.publication_id}</dd></div>
      <div><dt className="text-slate-500">定义版本</dt><dd className="mt-1 font-semibold text-slate-800">{result.binding.definition_id ?? '—'} · R{result.binding.definition_revision ?? '—'}</dd></div>
      <div><dt className="text-slate-500">分类覆盖</dt><dd className="mt-1 font-semibold text-slate-800">{percent(result.coverage.classified_ratio)} · {result.coverage.classified_periods}/{result.coverage.periods} 期</dd></div>
    </dl>
    <div className="mt-4 space-y-3">
      {performance.map(([strategyName, rows], index) => <details key={strategyName} open={index === 0} className="rounded-lg border border-slate-200 bg-white">
        ...
        <div className="overflow-x-auto border-t border-slate-200">
          <table className="min-w-[860px] w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-600"><tr><th className="px-3 py-2">状态</th><th className="px-3 py-2">区间数</th><th className="px-3 py-2">收益样本</th><th className="px-3 py-2">年化收益</th><th className="px-3 py-2">波动率</th><th className="px-3 py-2">最大回撤</th><th className="px-3 py-2">夏普</th><th className="px-3 py-2">正收益率</th></tr></thead>
      ...
      {!performance.length ? <p className="rounded-lg bg-white p-3 text-sm text-slate-500">本次运行没有可展示的分状态统计。</p> : null}
    </div>
