...
export default function SeriesOutputFields<T extends OutputFieldsValue>({ output, index, onChange, enumItems, onEnumChange, identityReadOnly = false, onIdentifierCommit, children }: {
  ...
  const field = 'mt-1 block min-h-10 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-sm disabled:bg-slate-100'
  ...
  return <div className="space-y-3">
    ...
    {enumItems && <section aria-label="枚举输出设置" className="rounded-xl border border-slate-200 bg-white p-3">
      ...
      <p className="mt-1 text-xs leading-5 text-slate-500">编号表示类别；名称和颜色用于展示。分类规则在计算步骤中配置。</p>
      <div className="mt-3 space-y-3">{enumItems.map((item, i) => <div key={i} className="grid grid-cols-[2.5rem_minmax(0,1fr)_minmax(0,1fr)_auto] items-end gap-2">
        <label className="text-xs text-slate-500">颜色<input type="color" aria-label={`状态${i + 1}颜色`} value={item.color || '#64748b'} onChange={event => onEnumChange?.(enumItems.map((value, j) => i === j ? { ...value, color: event.target.value } : value))} className="mt-1 h-10 w-10 rounded border border-slate-200" /></label>
        <label className="text-xs text-slate-500">稳定编号<input aria-label={`状态${i + 1}编号`} value={item.id} onChange={event => onEnumChange?.(enumItems.map((value, j) => i === j ? { ...value, id: event.target.value } : value))} className={field} /></label>
        <label className="text-xs text-slate-500">显示名称<input aria-label={`状态${i + 1}名称`} value={item.label} onChange={event => onEnumChange?.(enumItems.map((value, j) => i === j ? { ...value, label: event.target.value } : value))} className={field} /></label>
