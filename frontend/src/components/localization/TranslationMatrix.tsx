...
const button = 'min-h-9 rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm font-semibold text-slate-700 disabled:opacity-40'
  ...
  return <>
    ...
    <div className="max-h-[65vh] max-w-full overflow-auto rounded-xl border border-slate-300 [--i18n-key-width:96px] sm:[--i18n-key-width:220px]" data-testid="translation-matrix-scroll">
      <table ref={tableRef} role="grid" aria-label={s(`i18n.${matrix.scope}`)} aria-readonly={matrix.scope === 'system'} aria-rowcount={matrix.items.length + 1} aria-colcount={matrix.locales.length + 2} className="w-full table-fixed border-separate border-spacing-0 text-left text-sm" style={{ minWidth: `calc(var(--i18n-key-width) + ${matrix.locales.length * 220 + 150}px)` }}
        ...
        <thead><tr role="row"><th role="columnheader" aria-sort={ariaSort('code')} className="sticky left-0 top-0 z-30 break-words border-b border-r border-slate-300 bg-slate-100 p-2 sm:p-3"><button type="button" className="flex min-h-9 w-full items-center justify-between gap-2 text-left font-semibold text-slate-800 disabled:cursor-not-allowed" disabled={disabled || Boolean(editing)} aria-label={sortLabel('code', s('i18n.matrix.code'))} onClick={() => onSort('code')}><span>{s('i18n.matrix.code')}</span><span aria-hidden="true" className="text-slate-400">{sortMark('code')}</span></button></th>
          {matrix.locales.map(item => <th key={item.id} role="columnheader" aria-sort={ariaSort(item.id)} lang={item.id} className="sticky top-0 z-20 min-w-[220px] border-b border-r border-slate-300 bg-slate-50 p-3"><button type="button" className="flex min-h-9 w-full items-start justify-between gap-2 text-left disabled:cursor-not-allowed" disabled={disabled || Boolean(editing)} aria-label={sortLabel(item.id, item.label)} onClick={() => onSort(item.id)}><span><span className="block font-semibold text-slate-800">{item.label}</span><code className="text-xs font-normal text-slate-500">{item.id}</code></span><span aria-hidden="true" className="pt-0.5 text-slate-400">{sortMark(item.id)}</span></button>{!item.enabled && <span className="ml-2 text-xs text-amber-700">{s('i18n.matrix.disabled')}</span>}{!item.system_pack && <p className="mt-1 text-xs font-normal text-amber-700">{s('i18n.matrix.noSystemPack')}</p>}</th>)}
          <th className="sticky top-0 z-20 min-w-[150px] border-b border-slate-300 bg-white p-3"><button type="button" className={button} disabled={disabled} onClick={() => { if (commit()) onAddLanguage() }}>{s('i18n.matrix.addLanguage')}</button></th>
        </tr></thead>
        <tbody>{matrix.items.map((entry, r) => <tr key={entry.key} role="row" data-entry-key={entry.key}>
          <th role="rowheader" aria-readonly="true" aria-selected={selected(r, 0)} title={entry.key} tabIndex={active.row === r && active.column === 0 ? 0 : -1} data-matrix-row={r} data-matrix-column={0} onClick={event => focus({ row: r, column: 0 }, event.shiftKey)} onKeyDown={event => navigate(event, { row: r, column: 0 })} className={`sticky left-0 z-10 border-b border-r border-slate-300 p-2 sm:p-3 align-top font-normal outline-none focus:ring-2 focus:ring-inset focus:ring-violet-500 ${selected(r, 0) ? 'bg-violet-50' : 'bg-slate-50'}`}><code className="block break-all text-xs font-semibold text-slate-800">{module === entry.module ? entry.code : entry.key}</code></th>
          {matrix.locales.map((item, index) => {
            ...
            return <td key={item.id} role="gridcell" aria-label={`${entry.key} ${item.id}`} aria-readonly={!entry.customizable} aria-selected={selected(r, c)} lang={item.id} dir="auto" tabIndex={active.row === r && active.column === c ? 0 : -1} data-matrix-row={r} data-matrix-column={c} data-cell-state={changed ? 'modified' : source} onClick={event => { if (!isEditing) focus({ row: r, column: c }, event.shiftKey) }} onDoubleClick={() => beginEdit({ row: r, column: c })} onKeyDown={event => { if (!isEditing) navigate(event, { row: r, column: c }) }} className={`min-w-[220px] max-w-[340px] border-b border-r border-slate-200 p-3 align-top outline-none focus:ring-2 focus:ring-inset focus:ring-violet-500 ${selected(r, c) ? 'bg-violet-50' : changed ? 'bg-amber-50' : 'bg-white'}`}>
              {isEditing ? <textarea ref={editRef} aria-label={`${s('i18n.matrix.edit')} ${entry.key} ${item.id}`} value={editing.value} rows={entry.key.endsWith('.description') ? 4 : 2} className="min-h-12 w-full resize-y rounded border border-violet-400 bg-white p-2 text-sm outline-none" onChange={event => setEditing({ ...editing, value: event.target.value })} onBlur={() => commit()} onKeyDown={event => {
              ...
              }} /> : <span className={`block whitespace-pre-wrap break-words ${value === null ? 'text-slate-400' : 'text-slate-800'}`}>{value ?? s('i18n.matrix.empty')}</span>}
              <span className={`mt-2 block text-[11px] ${changed ? 'text-amber-800' : 'text-slate-400'}`}>{changed ? s('i18n.matrix.modified') : source === 'custom' ? s('common.customized') : source === 'builtin' ? s('common.builtIn') : ''}</span>
              {value === null && own.fallback_value !== null && <p className="mt-1 text-[11px] leading-5 text-slate-400">{s('i18n.matrix.fallback', { locale: own.fallback_locale ?? '', value: own.fallback_value })}</p>}
            </td>
          })}<td className="border-b border-slate-200" />
        </tr>)}</tbody>
      </table>
    </div>
    {row && <div role="region" className="mt-3 min-w-0 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs leading-6" aria-label={s('i18n.matrix.selection')}><p><strong>{s('i18n.matrix.selection')}：</strong><code className="break-all">{row.key}{language ? ` · ${language.id}` : ''}</code></p>{cell && <p className="whitespace-pre-wrap break-words">{s('i18n.default')}：{cell.default_value ?? s('i18n.matrix.empty')}</p>}{row.placeholders.length > 0 && <p>{s('i18n.placeholderHint', { names: row.placeholders.join(', ') })}</p>}{row.customizable && language && <div className="mt-2 flex flex-wrap gap-2"><button type="button" className={button} disabled={disabled} onClick={() => beginEdit()}>{s('i18n.matrix.edit')}</button><button type="button" className={button} disabled={disabled || Boolean(editing) || !(pending[`${language.id}:${row.key}`]?.value ?? cell?.override_value)} onClick={() => onChanges([{ key: row.key, locale: language.id, value: null }])}>{s('common.reset')}</button><span className="self-center text-slate-500">{s('i18n.matrix.editHint')}</span></div>}</div>}
  </>
}
