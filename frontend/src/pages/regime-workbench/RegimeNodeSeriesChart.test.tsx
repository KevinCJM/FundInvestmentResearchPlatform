...
vi.mock('echarts-for-react', () => ({ default: ({ option, onEvents }: any) => <div><div data-testid="chart-option">{JSON.stringify(option)}</div><button onClick={() => onEvents.datazoom({}, { getOption: () => ({ dataZoom: [{ startValue: 1, endValue: 2 }] }) })}>缩放后两日</button></div> }))
...
const selectUpstream = async () => {
  await userEvent.click(screen.getByText('叠加上游数据'))
...
it('上游按同一日期分别归一化，缺失基准不借用次日，原始对比表不变', async () => {
  ...
  expect(screen.getByRole('table', { name: '上游节点对比数据' })).toHaveTextContent('180')
