...
export default function DataDownloadWorkspace() {
  ...
  return <form ref={form} noValidate onSubmit={e => e.preventDefault()} className="min-w-0 space-y-5">
    ...
    {view === 'quick' ? <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
      ...
      <p className="text-xs leading-6 text-slate-600">只显示 {source?.config.name} 已配置的接口。{source?.credential_required ? source?.credential_configured ? '凭据已保存。' : '尚未配置有效凭据。' : '此来源无需认证。'}<Link className="ml-1 text-accent-700 underline" to={`/settings/source-center?source=${source?.config.id}`}>管理来源、凭据与接口</Link></p>
      ...
      {!canSnapshot ? <p className="text-xs text-slate-600">指标快照需要产品信息和基金净值。跨来源组合或不同计算位置，请使用 ETL 编排。</p> : null}
