...
export default function PublishedScenarioCenter() {
  ...
  useEffect(() => {
    ...
    Promise.all([riskCatalog('transmission', controller.signal), riskReleases('transmission', {}, controller.signal), scenarioReleases(undefined, controller.signal)])
      .then(([nextCatalog, nextModels, nextReleases]) => { if (!controller.signal.aborted) { setCatalog(nextCatalog); setModels(nextModels); setReleases(nextReleases); setError('') } })
  ...
  async function publish() {
    ...
    try { const release = await publishScenario(draft, preview.preview_hash, validDays, note, preview.publication_request_id); if (alive.current && token === operation.current) { setPublishedId(release.id); setNotice('情景已确认发布并写入磁盘。产品和组合可以直接选择，不会重新运行宏观模型。'); setRefresh(old => old + 1) } }
...
function ScenarioHorizonDetails({ preview }: { preview: ScenarioPreview }) {
  const evidence = preview.horizon_evidence
  if (!evidence) return <p className="mt-1 text-slate-600">此版本未提供 Horizon 证据。</p>
  if (evidence.method !== 'explicit_path_horizon_evidence_v2') return <div className="mt-1 text-slate-600"><p>此版本只记录末期增量，未检验剩余传导及累计水平恢复。</p><p>窗口可能截断了情景传播或恢复过程，不能据此判断已回到基线。</p></div>
  return <div className="mt-2 space-y-1 text-slate-600">
    {(evidence.terminal_status !== 'increments_zero' || evidence.remaining_response_status !== 'zero' || evidence.cumulative_level_status !== 'recovered') && <p>窗口可能截断了情景传播或恢复过程；以下分别披露期末增量、剩余传导与累计水平。</p>}
    <p>末期增量：{evidence.terminal_status === 'increments_zero' ? '归零' : '仍有变动'}。</p>
    <p>剩余模型响应：{evidence.remaining_response_status === 'zero' ? '归零' : '仍有滞后传导'}（假设之后没有新增冲击）。</p>
    <p>累计市场水平：{evidence.cumulative_level_status === 'recovered' ? '恢复至起点' : '尚未恢复至起点'}；产品财富须另行计算。</p>
  </div>
}
