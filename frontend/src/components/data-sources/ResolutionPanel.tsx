...
function PriorityEditor({ value, catalog, onChange, label }: { value: string[]; catalog: SourceCatalog; onChange: (value: string[]) => void; label: string }) {
  ...
  return <section aria-label={label} className="space-y-2">
    ...
    {value.map((id, index) => <div key={id} className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-white p-2">
...
function RunResult({ result, catalog }: { result: ResolutionRun; catalog: SourceCatalog }) {
  ...
  return <section className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4" aria-label="多源取值结果">
    ...
    {result.decisions?.slice(0, 30).map((decision, index) => <details key={index} className="rounded-lg border border-slate-200 bg-white p-3"><summary className="cursor-pointer break-words text-sm">{labels[decision.status] ?? decision.status} · {sourceName(decision.selected_source)}</summary>
      <p className="mt-2 break-all text-xs">业务键：{JSON.stringify(decision.key)}</p>
      {decision.selected_batch ? <p className="mt-1 break-all text-xs text-slate-500">采用批次：{decision.selected_batch}</p> : null}
    ...
    <p className="text-xs text-slate-500">只展示前 30 条决策。实际合并的完整决策随不可变候选保存；原始来源数据不被覆盖。</p>
  </section>
}
...
export default function ResolutionPanel({ catalog, onDirty }: { catalog: SourceCatalog; onDirty: (dirty: boolean) => void }) {
  ...
  return <form ref={form} noValidate className="space-y-5" onSubmit={event => { event.preventDefault(); if (catalog.editing_enabled) void perform(async () => { const value = await saveResolutionPolicy(config, saved.revision); setSaved(value); setConfig(value.config); onDirty(false); setMessage('取值规则已保存；已有研究数据不会被覆盖。') }) }}>
    <header className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="text-xl font-bold">多源优先级与异常处理</h2><p className="mt-2 text-sm leading-6 text-slate-600">先对齐产品、日期、币种与复权口径，再按规则选取整条记录。不混拼开高低收，不用单位净值替代复权净值，也不对冲突值擅自求平均。</p><p className="mt-2 text-xs text-slate-500">规则修订 {saved.revision} · 先保存，再应用到已下载候选。当前正式研究仍使用原活跃数据。</p></header>
    ...
    <fieldset disabled={!catalog.editing_enabled || busy} className="space-y-5">
      ...
      <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-5">
        ...
        <p className="text-xs leading-6 text-slate-500">0.01 表示 1%。超过绝对与相对容差中的较大值才算冲突。跳变只代表可疑，分红、拆分也可能造成真实变化。</p>
        <details className="rounded-lg border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-semibold">必需字段、对比字段与值域</summary><p className="mt-2 text-xs leading-6 text-slate-500">要求复权净值时应勾选 adjusted_nav 为必需字段；缺少该值的备用来源不能替代。留空对比字段表示检查全部业务维度/数值字段。</p>
    ...
    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-5"><h3 className="font-bold">检查并合并已下载候选</h3><p className="text-xs leading-6 text-slate-500">不访问外部接口。按已保存规则合并所选表，生成独立候选及审计记录；冲突记录不进入选中结果。</p>
