...
export default function RiskScaleWorkspace() {
  ...
  const editSegmentation = (next: RiskScaleDefinition) => {
    ...
    segmentationTimer.current = window.setTimeout(() => {
      ...
      void task.run(signal => riskScales.preview({ definition: normalized }, signal), result => { setPreview(result); setFresh(true) })
    }, 180)
  }
  ...
  const save = () => { retryAction.current = conflict => { if (conflict && draft) loadFrom(draft.id); else save() }; const body = { name: value.name, scheme_id: value.scheme_id, editable_definition: { ...editor } }; void task.run(signal => draft ? riskScales.updateDraft(draft.id, { ...body, expected_revision: draft.revision }, signal) : riskScales.saveDraft(body, signal), result => { setDraft(result); setSaved(true) }) }
  const compute = () => { retryAction.current = compute; setFresh(false); setConfirmed(false); setAcknowledged([]); void task.run(signal => riskScales.preview({ definition: { ...value, research_as_of: researchDay } }, signal), result => { setPreview(result); setFresh(true); move(3) }) }
  ...
  const compareMethods = () => { retryAction.current = compareMethods; void task.run(async signal => { const results: PreviewResponse[] = []; for (const method of registeredAlgorithms(capabilities!).filter(item => item.available !== false && item.id !== 'manual_volatility_bands_v1')) results.push(await riskScales.preview({ definition: { ...value, segmentation: { algorithm_id: method.id } } }, signal)); return results }, setMethodComparisons) }
  ...
  return <div className="min-w-0 space-y-3 text-slate-900">
    ...
    <ErrorNotice error={bootstrap.error} retry={load} />{!loaded ? <Loading /> : <>
      ...
      <ErrorNotice error={task.error} retry={() => retryAction.current?.((task.error as { status?: number } | null)?.status === 409)} retryLabel={(task.error as { code?: string } | null)?.code === 'REVISION_CONFLICT' ? t('reloadDraft') : undefined} />
