...
export default function RequestParameterFields({ fields, values, bindings = {}, onChange }: {
  ...
  return <div className="grid gap-3 sm:grid-cols-2">{fields.map(field => {
    ...
    return <label key={field.name} className="text-xs font-semibold">{field.label}
      {bindings[field.name] ? <span className="mt-2 block rounded-lg bg-indigo-50 p-3 text-indigo-800">运行时填写：{bindings[field.name]}</span> : <input aria-label={field.label} className={inputClass} type={field.data_type === 'text' ? 'text' : field.data_type}
      ...
      {field.description ? <span className="mt-1 block font-normal leading-5 text-slate-500">{field.description}</span> : null}
    </label>
  })}</div>
}
