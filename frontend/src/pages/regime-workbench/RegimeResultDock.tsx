...
export default function RegimeResultDock({ nodeOnly = false, run, page, nodes, schemas, previewNodeId, loadingSeries, height, collapsed, onPreviewNode, onLoadSeries, onHeight, onCollapsed }: RegimeResultDockProps) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg" aria-label="可伸缩试算结果区">
      ...
      <header className="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-bold text-slate-950">{nodeOnly ? '节点预览结果' : '试算结果'} <RegimeHelpTip label="试算结果说明" text="试算用于随时查看当前草稿的节点时序和最终状态。修改图谱后旧结果会标记过期，必须重新试算。" /></h3><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${run?.status === 'completed' ? 'bg-emerald-100 text-emerald-800' : run?.status === 'failed' ? 'bg-rose-100 text-rose-800' : run?.status === 'cancelled' ? 'bg-slate-100 text-slate-600' : 'bg-indigo-100 text-indigo-800'}`}>{run ? ({ queued: '排队中', preparing: '准备中', running: '计算中', completed: '已完成', failed: '失败', cancelled: '已取消' } as const)[run.status] : '尚未运行'}</span>{run?.stage ? <span className="text-[11px] text-slate-500">{run.stage} · {Math.round((run.progress ?? 0) * 100)}%</span> : null}</div><p className="mt-1 text-[11px] text-slate-500">{run?.message || '运行完成后可选择任意节点读取真实序列；不会在浏览器生成模拟结果。'}</p></div><div className="flex flex-wrap gap-1"><button type="button" onClick={() => onHeight(240)} title="将结果区高度调整为紧凑模式" className="min-h-8 rounded-lg px-2 text-[11px] font-bold text-slate-600 hover:bg-slate-100">紧凑</button><button type="button" onClick={() => onHeight(420)} title="将结果区高度调整为标准模式" className="min-h-8 rounded-lg px-2 text-[11px] font-bold text-slate-600 hover:bg-slate-100">标准</button><button type="button" onClick={() => onHeight(620)} title="展开结果区以查看更多图表和数据" className="min-h-8 rounded-lg px-2 text-[11px] font-bold text-slate-600 hover:bg-slate-100">展开</button><button type="button" onClick={() => onCollapsed(!collapsed)} className="min-h-8 rounded-lg border border-slate-300 px-2 text-[11px] font-bold text-slate-700">{collapsed ? '显示结果区' : '收起结果区'}</button></div></header>
      {!collapsed ? <div className="overflow-auto p-4" style={{ height }}>
        {run?.status === 'completed' ? <div className="space-y-4"><RegimeEvaluationResults results={run.result?.evaluation_results} mode="preview" />{!nodeOnly && <div className="flex flex-col gap-2 sm:flex-row sm:items-end"><label className="min-w-0 flex-1 text-xs font-bold text-slate-600">预览节点<RegimeHelpTip label="预览节点说明" text="选择任意中间节点查看它的真实时序，便于判断滤波、特征、模型或稳定化处理是否符合预期。" /><select aria-label="预览节点" value={previewNodeId} onChange={(event) => onPreviewNode(event.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-slate-300 bg-white px-2 font-normal"><option value="">最终输出</option>{nodeOptions.map((node) => <option key={node.id} value={node.id}>{node.label}</option>)}</select></label><button type="button" disabled={loadingSeries} onClick={onLoadSeries} title="从后端读取所选节点的真实计算序列" className="min-h-10 rounded-xl bg-indigo-600 px-4 text-sm font-bold text-white disabled:opacity-40">{loadingSeries ? '读取节点序列…' : '查看节点结果'}</button></div>}{page ? <RegimeNodeSeriesChart key={`${page.run_id}:${page.node_id}:${page.port}:${page.offset}:${page.items.length}`} page={page} /> : null}{page ? <><p className="text-xs text-slate-500">已返回 {page.items.length} / {page.total} 条节点结果</p>{nodeOnly && page.items.length > 100 ? <p className="text-xs text-slate-500">图表覆盖全部数据，表格展示最近 100 条。</p> : null}<SeriesTable page={nodeOnly ? { ...page, items: page.items.slice(-100) } : page} /></> : <div className="grid min-h-36 place-items-center rounded-xl border border-dashed border-slate-300 text-center text-sm text-slate-500">选择节点并读取后端序列。</div>}<p className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-900">试算的固定签名 NJIT 执行链已通过校验。</p></div> : <div className="grid min-h-48 place-items-center rounded-xl border border-dashed border-slate-300 p-8 text-center"><div><p className="font-bold text-slate-800">{run ? '试算尚未完成' : '等待一次真实试算'}</p><p className="mt-2 text-sm text-slate-500">{run?.error ? typeof run.error === 'string' ? run.error : run.error.message : nodeOnly ? '选择节点后点击预览；其他未完成的步骤不会阻止预览。' : '完成计算图并通过检查后，从顶部命令栏启动试算。'}</p></div></div>}
      </div> : null}
    </section>
  )
}
