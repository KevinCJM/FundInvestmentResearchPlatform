...
export default function AssetAuthorizations({ value, catalog, onChange }: {
  ...
  const scope = value.strategic_universe_id ? `strategic:${value.strategic_universe_id}` : value.allocation_scope ?? ''
  const assets = value.strategic_universe_id ? catalog?.strategic_universes?.find(a => a.id === value.strategic_universe_id)?.definition.assets ?? [] : catalog?.allocations.find(a => a.alloc_name === scope)?.assets ?? []
  ...
  return <details className="border-t border-slate-200 pt-4"><summary className="cursor-pointer text-sm font-medium">限定可投大类与联合比例（可选）</summary>
    <div className="mt-4 space-y-4">
      <Field label="授权的大类方案" hint="选择后绑定此方案；后续SAA只能收紧权重边界，不能放宽。清空选择会清除对应的授权边界。"><select className={inputClass} value={scope} onChange={e => {
        const id = e.target.value.startsWith('strategic:') ? e.target.value.slice(10) : null
        const item = id ? catalog?.strategic_universes?.find(a => a.id === id)?.definition : catalog?.allocations.find(a => a.alloc_name === e.target.value)
        onChange({ strategic_universe_id: id, allocation_scope: id ? null : e.target.value || null, asset_limits: Object.fromEntries((item?.assets ?? []).map(a => [a.id, { min_weight: 0, max_weight: 1, max_abs_tilt: .1 }])), group_limits: [] })
      }}><option value="">暂不限定具体大类方案</option>{catalog?.allocations.map(a => <option key={a.alloc_name} value={a.alloc_name}>{a.alloc_name}</option>)}{catalog?.strategic_universes?.map(a => <option key={a.id} value={`strategic:${a.id}`}>战略范围：{a.name}</option>)}</select></Field>
      {assets.length > 0 && <>
