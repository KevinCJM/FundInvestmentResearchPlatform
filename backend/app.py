...
class SettingsPayload(BaseModel):
    ...
    rebalance_frequency: str = Field(..., min_length=3, max_length=32)


class SettingsResponse(BaseModel):
    parameters: SettingsPayload
...
ROLE_PRIORITY = {"basic": 0, "advanced": 1, "admin": 2}
TOKEN_TTL_HOURS = 12
...
CURRENT_PARAMETERS: Dict[str, Any] = DEFAULT_PARAMETERS.copy()


def _cleanup_tokens() -> None:
    now = datetime.utcnow()
    expired = [token for token, payload in _TOKEN_REGISTRY.items() if payload["expires"] <= now]
...
def _resolve_token(request: Request) -> Dict[str, Any]:
    _cleanup_tokens()
    token = _extract_token(request)
    if not token:
        raise HTTPException(status_code=401, detail="缺少认证信息")
    payload = _TOKEN_REGISTRY.get(token)
    if not payload or payload["expires"] <= datetime.utcnow():
        raise HTTPException(status_code=401, detail="凭证已失效，请重新登录")
    return payload


def _ensure_role(request: Request, *, allow: Optional[List[str]] = None, minimum: Optional[str] = None) -> Dict[str, Any]:
    payload = _resolve_token(request)
    role = payload.get("role", "basic")
    if allow is not None and role not in allow:
        raise HTTPException(status_code=403, detail="当前角色无权访问")
    if minimum is not None:
        if ROLE_PRIORITY.get(role, -1) < ROLE_PRIORITY.get(minimum, -1):
            raise HTTPException(status_code=403, detail="当前角色无权访问")
    return payload
...
def auth_me(request: Request) -> LoginResponse:
    payload = _resolve_token(request)
    return LoginResponse(
        token=payload["token"],
...
def get_parameters(request: Request) -> SettingsResponse:
    _ensure_role(request, minimum="advanced")
    return SettingsResponse(parameters=SettingsPayload(**CURRENT_PARAMETERS))


@app.post("/api/settings", response_model=SettingsResponse)
def update_parameters(req: SettingsPayload, request: Request) -> SettingsResponse:
    _ensure_role(request, minimum="admin")
    CURRENT_PARAMETERS.update(req.dict())
    return SettingsResponse(parameters=SettingsPayload(**CURRENT_PARAMETERS))
