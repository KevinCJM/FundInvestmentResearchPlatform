...
export default function HoldingDiagnosis() {
  ...
  if (error && !run) return <div className="mx-auto max-w-3xl p-6"><div role="alert" className="rounded-xl border border-rose-300 bg-rose-50 p-4 text-rose-800">{error}</div><Link to="/portfolio-construction" className="mt-4 inline-block text-emerald-700 underline">去构建组合</Link></div>
  ...
  return <div className="mx-auto max-w-7xl space-y-5 p-4 sm:p-6" aria-busy={scenarioLoading || exporting !== null}>
    ...
    <div className="flex flex-wrap items-end gap-2"><label className="text-xs text-slate-600">单表 CSV<select aria-label="CSV 导出表" value={exportTable} onChange={(event) => setExportTable(event.target.value as PortfolioExportTable)} className="mt-1 block rounded border border-slate-300 bg-white px-2 py-2 text-sm"><option value="summary">汇总</option><option value="components">成分</option><option value="daily-contributions">日收益贡献</option><option value="weight-path">权重路径</option><option value="covariance">协方差矩阵</option><option value="correlation">相关矩阵</option><option value="risk-contributions">风险贡献</option>{scenarioResult && <><option value="scenario-summary">情景汇总</option><option value="scenario-series">情景序列</option></>}</select></label><button type="button" onClick={() => handleExport('csv')} disabled={exporting !== null} className="rounded border border-slate-300 bg-white px-3 py-2 text-sm">{exporting === 'csv' ? '导出中…' : '导出 CSV'}</button><button type="button" onClick={() => handleExport('zip')} disabled={exporting !== null} className="rounded border border-slate-300 bg-white px-3 py-2 text-sm">{exporting === 'zip' ? '导出中…' : '导出 ZIP'}</button><span className="text-xs text-slate-500">ZIP 包含全部表{scenarioResult ? '及当前情景结果' : ''}</span><Link to="/portfolio-construction" className="rounded bg-emerald-700 px-3 py-2 text-sm text-white">新建组合研究</Link></div>
