...
候选内部单元测试以及候选新增黑盒契约在 policy-tests 的独立 runner 分别执行；控制器要求正常退出、全新完成文件、非零测试数、执行数与发现数相等、全部成功且无 skipped/expectedFailures，并限制 300 秒。它们验证候选测试可用性，不能代替 base 所有的黑盒断言。受保护父进程独立判断子进程业务输出，非零退出、空输出、非 JSON、多份 JSON 和超时均失败；空输出提前退出、已测试的恒定成功结果以及篡改子进程 unittest 均不能绕过父断言。回归实际执行工作流步骤，覆盖正常候选通过、恒定返回成功并 monkeypatch 断言的候选失败、导入期 os._exit(0) 失败，以及导入期直接输出合法 JSON 恒定成功再退出的候选失败。JSON 业务结果始终视为不可信输入；本测试不认证同进程内结果封装者、不证明指定函数确实执行，也不能排除候选识别固定测试数据后特判输出。这是进程分离的契约测试，不是同 UID 恶意代码的完整安全沙箱。规则文件另有父进程固定不变量回归：目标分支、PR-only、三个检查及 App 来源、strict、空 bypass、禁止删除/强推与零人类审批。
